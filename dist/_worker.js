...
`;for(const J of g.trends){const we=J.trend==="BULLISH"?"📈":J.trend==="BEARISH"?"📉":"➡️";$+=`${we} *${J.timeframe}*: ${J.trend} (${J.confidence.toFixed(0)}%)
`}$+=`
━━━━━━━━━━━━━━━━━━━━━━━━━
`,$+=`📈 *DAY TRADE SIGNAL*
`,$+=`━━━━━━━━━━━━━━━━━━━━━━━━━
...
`;const he=candles1h.slice(-20),de=he.map(J=>J.high).sort((J,we)=>we-J),ae=he.map(J=>J.low).sort((J,we)=>J-we),It=de.slice(0,3),_t=ae.slice(0,3);if($+=`📊 *Key Levels:*
`,$+=`🔴 *Resistance:* ${It.map(J=>`$${J.toFixed(2)}`).join(", ")}
...
`;for(const we of P.warnings)K+=`• ${we}
`;K+=`
`}K+=`💡 *Recommendation:*
${P.recommendation}
...
`,K+="🤖 Powered by Hedge Fund Grade AI",console.log("[TELEGRAM] Message 2 length:",K.length,"characters");const Be=await q({botToken:C.telegram_bot_token,chatId:C.telegram_chat_id},K);fe=gt&&Be}}catch(H){console.error("[ENHANCED] Telegram error (optional):",H.message)}return e.json({success:!0,timestamp:new Date().toISOString(),current_price:l,telegram_sent:fe,day_trade:O,swing_trade:ue,alignment:{type:g.type,score:g.score,trends:g.trends},patterns:v.length>0?v.slice(0,3):null,regime:I?{trend:I.trend,volatility:I.volatility,should_trade:I.should_trade}:null,ml_prediction:L?{direction:L.overall_direction,predictions:L.predictions}:null,profit_probability:B?{tp1:B.tp1_probability,tp2:B.tp2_probability,tp3:B.tp3_probability,expected_value:B.expected_value}:null,liquidity:P?{score:P.liquidity_score,session:P.session,time_zone:P.time_of_day_zone,volume_trend:P.volume_trend,volume_percentile:P.volume_percentile,estimated_spread_pips:P.estimated_spread_pips,price_impact_bps:P.price_impact_bps,market_depth_score:P.market_depth_score,optimal_for_trading:P.optimal_for_trading,warnings:P.warnings,recommendation:P.recommendation}:null,risk_metrics:{var_95:V,var_99:Q,drawdown_pct:ee,portfolio_heat_pct:te}})}catch(a){return console.error("[ENHANCED] Error:",a.message,a.stack),e.json({success:!1,error:a.message,stack:a.stack},500)}});const Xs=new _e;Xs.post("/simple",async e=>{var s,a,n,i;const{DB:t}=e.env;try{console.log("[SIMPLE] Starting simple signal generation");const o=await t.prepare(`
    ...
    `).all()).results||[]).map(h=>({timestamp:h.timestamp,open:h.open,high:h.high,low:h.low,close:h.close,volume:h.volume||1})).reverse(),p=ze(d,u,m,"day_trade"),g=ze(d,u,m,"swing_trade");console.log("[SIMPLE] Generated signals with liquidity:",{day:{type:p.signal_type,confidence:p.confidence,liquidity_score:p.liquidity_score,session:p.session},swing:{type:g.signal_type,confidence:g.confidence,liquidity_score:g.liquidity_score,session:g.session}});let f=!1;try{const h=await t.prepare(`
        SELECT setting_key, setting_value FROM user_settings
        WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
      `).all(),y={};for(const S of h.results||[])y[S.setting_key]=S.setting_value;if(console.log("[SIMPLE] Telegram config:",{hasToken:!!y.telegram_bot_token,hasChat:!!y.telegram_chat_id,tokenLength:((i=y.telegram_bot_token)==null?void 0:i.length)||0,chatId:y.telegram_chat_id}),y.telegram_bot_token&&y.telegram_chat_id){const S=await t.prepare(`
          SELECT high, low FROM market_data 
          WHERE timeframe = '1h'
          ORDER BY timestamp DESC 
          LIMIT 20
        `).all();let E=[],T=[];if(S.results&&S.results.length>=20){const j=S.results.map(M=>M.high).sort((M,N)=>N-M),L=S.results.map(M=>M.low).sort((M,N)=>M-N);E=j.slice(0,3),T=L.slice(0,3)}const b=p.signal_type==="BUY"?"🟢":p.signal_type==="SELL"?"🔴":"⚪",k=new Date().toLocaleString("en-US",{timeZone:"UTC"});let v=`${b} <b>GOLD/USD ${p.signal_type} SIGNAL</b> ${b}
...
`,o+="Next scan in 5 minutes...",o}async function dt(e){const t=await e.prepare(`
  ...
  `).bind(s,i).run();let o=!1,l="";s>=t.max_drawdown_pct?(o=!0,l=`Max drawdown ${s.toFixed(1)}% reached (limit ${t.max_drawdown_pct}%)`):i>=t.max_daily_loss_pct&&(o=!0,l=`Daily loss ${i.toFixed(1)}% reached (limit ${t.max_daily_loss_pct}%)`),o&&t.trading_enabled===1&&await e.prepare(`
      ...
      SET trading_enabled = 0,
          ...
          updated_at = CURRENT_TIMESTAMP
      WHERE id = 1
    `).bind(l).run()}async function aa(e){return await e.prepare("SELECT * FROM trade_stats").first()||{total_trades:0,winning_trades:0,losing_trades:0,win_rate:0,total_profit_loss:0,avg_win:0,avg_loss:0,largest_win:0,largest_loss:0,avg_confidence:0,avg_mtf_score:0}}async function na(e){return(await e.prepare("SELECT * FROM open_positions").all()).results||[]}const Se=new _e;Se.get("/limits",async e=>{try{const{DB:t}=e.env,s=await dt(t);return e.json({success:!0,limits:s})}catch(t){return e.json({success:!1,error:t.message},500)}});Se.post("/validate",async e=>{try{const{DB:t}=e.env,s=await e.req.json(),a=await Js({entry_price:s.entry_price,stop_loss:s.stop_loss,take_profit_1:s.take_profit_1,confidence:s.confidence,trade_type:s.trade_type},t);return e.json({success:!0,validation:a})}catch(t){return e.json({success:!1,error:t.message},500)}});Se.post("/open",async e=>{try{const{DB:t}=e.env,s=await e.req.json(),a={signal_id:s.signal_id,trade_type:s.trade_type,trading_style:s.trading_style||"day_trade",entry_price:s.entry_price,entry_time:s.entry_time||new Date().toISOString(),position_size:s.position_size||0,position_value:0,stop_loss:s.stop_loss,take_profit_1:s.take_profit_1,take_profit_2:s.take_profit_2,take_profit_3:s.take_profit_3,confidence:s.confidence,mtf_score:s.mtf_score,regime:s.regime,status:"OPEN",notes:s.notes},n=await ea(a,t);return n.success?e.json({success:!0,message:"Trade logged successfully",trade_id:n.trade_id}):e.json({success:!1,error:n.error},400)}catch(t){return e.json({success:!1,error:t.message},500)}});Se.post("/close/:id",async e=>{try{const{DB:t}=e.env,s=parseInt(e.req.param("id")),a=await e.req.json(),n=await ta(s,a.exit_price,a.exit_reason||"MANUAL",t);return n.success?e.json({success:!0,message:"Trade closed successfully",profit_loss:n.profit_loss}):e.json({success:!1,error:n.error},400)}catch(t){return e.json({success:!1,error:t.message},500)}});Se.get("/open",async e=>{try{const{DB:t}=e.env,s=await na(t);return e.json({success:!0,count:s.length,positions:s})}catch(t){return e.json({success:!1,error:t.message},500)}});Se.get("/history",async e=>{try{const{DB:t}=e.env,s=parseInt(e.req.query("limit")||"50"),a=await t.prepare(`
      SELECT * FROM live_trades 
      WHERE status = 'CLOSED'
      ORDER BY exit_time DESC
      LIMIT ?
    `).bind(s).all();return e.json({success:!0,count:a.results.length,trades:a.results})}catch(t){return e.json({success:!1,error:t.message},500)}});Se.get("/stats",async e=>{try{const{DB:t}=e.env,s=await aa(t),a=await dt(t);return e.json({success:!0,stats:s,account:{starting_balance:a.starting_balance,current_balance:a.current_balance,total_return:a.current_balance-a.starting_balance,total_return_pct:(a.current_balance-a.starting_balance)/a.starting_balance*100}})}catch(t){return e.json({success:!1,error:t.message},500)}});Se.get("/daily",async e=>{try{const{DB:t}=e.env,s=parseInt(e.req.query("days")||"30"),a=await t.prepare(`
      SELECT * FROM daily_performance
      ORDER BY trade_date DESC
      LIMIT ?
    `).bind(s).all();return e.json({success:!0,count:a.results.length,daily_performance:a.results})}catch(t){return e.json({success:!1,error:t.message},500)}});Se.post("/limits/update",async e=>{try{const{DB:t}=e.env,s=await e.req.json();return await t.prepare(`
      UPDATE risk_limits
      SET max_position_risk_pct = ?,
          ...
          updated_at = CURRENT_TIMESTAMP
      WHERE id = 1
    `).bind(s.max_position_risk_pct||2,s.max_portfolio_risk_pct||10,s.max_daily_loss_pct||5,s.max_drawdown_pct||10).run(),e.json({success:!0,message:"Risk limits updated"})}catch(t){return e.json({success:!1,error:t.message},500)}});Se.post("/limits/resume",async e=>{try{const{DB:t}=e.env;return await t.prepare(`
      UPDATE risk_limits
...
/help - Show this message`);return e.json({ok:!0})}catch(t){return console.error("[TELEGRAM WEBHOOK] Error:",t),e.json({ok:!0})}});const jt=new _e;jt.post("/market-analysis",async e=>await da(e));jt.get("/health",async e=>e.json({success:!0,status:"healthy",service:"ai-analysis",timestamp:new Date().toISOString()}));jt.get("/auto-ai-scan",async e=>await da(e));async function da(e){const{DB:t}=e.env;try{console.log("[AI-ANALYSIS] Starting comprehensive market analysis");const s=await t.prepare(`
    ...
    `).all();let a="";for(const R of s.results||[])R.setting_key==="twelve_data_api_key"&&(a=R.setting_value);let n=[];if(a&&a!=="your_api_key_here")try{const R=`https://api.twelvedata.com/time_series?symbol=XAU/USD&interval=1h&outputsize=100&apikey=${a}`,U=await(await fetch(R)).json();U.values&&U.values.length>=50&&(n=U.values.reverse().map(j=>({timestamp:j.datetime,open:parseFloat(j.open),high:parseFloat(j.high),low:parseFloat(j.low),close:parseFloat(j.close),volume:parseFloat(j.volume)||0})),console.log("[AI-ANALYSIS] Fresh data fetched:",n.length,"candles"))}catch{console.error("[AI-ANALYSIS] API fetch failed, falling back to database")}if(n.length===0){const R=await t.prepare(`
        ...
        ORDER BY timestamp DESC 
        LIMIT 100
      `).all();if(!R.results||R.results.length<50)return e.json({success:!1,error:"Not enough market data available"},400);n=R.results.reverse().map(I=>({timestamp:I.timestamp,open:I.open,high:I.high,low:I.low,close:I.close,volume:I.volume||0}))}const i=pe(n);if(!i)return e.json({success:!1,error:"Failed to calculate indicators"},400);const o=n[n.length-1].close,l=le(o,i,"day_trade");console.log("[AI-ANALYSIS] Current price:",o,"Signal:",l.signal_type,"Confidence:",l.confidence);const r={};for(const R of["5m","15m","1h","4h","daily"]){const I=await t.prepare(`
        SELECT * FROM multi_timeframe_indicators 
        WHERE timeframe = ?
        ORDER BY timestamp DESC 
        LIMIT 1
      `).bind(R).first();I&&(r[R]=I)}const d=Jt(r,o),c=n.slice(-50),u=c.map(R=>R.high).sort((R,I)=>I-R),_=c.map(R=>R.low).sort((R,I)=>R-I),m=[Math.max(...u.slice(0,10))],p=[Math.min(..._.slice(0,10))];o>i.sma_20?p.push(i.sma_20):m.push(i.sma_20),o>i.sma_50?p.push(i.sma_50):m.push(i.sma_50),o>i.vwap?p.push(i.vwap):m.push(i.vwap);const g=Math.round(o/10)*10;g>o?m.push(g):p.push(g);const f=[...new Set(m)].sort((R,I)=>R-I).filter(R=>R>o).slice(0,3),h=[...new Set(p)].sort((R,I)=>I-R).filter(R=>R<o).slice(0,3);console.log("[AI-ANALYSIS] Key levels - Support:",h,"Resistance:",f);const y=i.atr_14/o*100;let S="NORMAL";y>3?S="EXTREME":y>1.5?S="HIGH":y<.5&&(S="LOW");const E=[];let T=30,b=30,k=40;d.type==="ALL_BULLISH"?(T=60,b=20,k=20):d.type==="ALL_BEARISH"?(T=20,b=60,k=20):d.score>=4&&(d.trends.filter(R=>R.trend==="BULLISH").length>=4?(T=50,b=25,k=25):(T=25,b=50,k=25)),f.length>0&&E.push({name:"📈 BULLISH CONTINUATION",probability:T,description:`Price breaks above $${f[0].toFixed(2)} and rallies toward $${(f[f.length-1]||o*1.02).toFixed(2)}`,trigger:`Breakout above $${f[0].toFixed(2)} with volume`,target:f[f.length-1]||o*1.02}),h.length>0&&E.push({name:"📉 BEARISH CORRECTION",probability:b,description:`Price breaks below $${h[0].toFixed(2)} and drops toward $${(h[h.length-1]||o*.98).toFixed(2)}`,trigger:`Breakdown below $${h[0].toFixed(2)} with volume`,target:h[h.length-1]||o*.98}),E.push({name:"↔️ CONTINUED RANGING",probability:k,description:`Price oscillates between $${(h[0]||o*.99).toFixed(2)} and $${(f[0]||o*1.01).toFixed(2)} with choppy action`,trigger:S==="EXTREME"?"High volatility continues":"No clear breakout/breakdown",target:null}),E.sort((R,I)=>I.probability-R.probability);let v={action:"WAIT",reason:"Market conditions unclear - wait for better setup",entry_range:null,stop_loss:null};l.confidence>=65?l.signal_type==="BUY"?v={action:"BUY",reason:`Strong ${l.signal_type} signal with ${l.confidence}% confidence. ${d.type} MTF alignment.`,entry_range:`${(o-5).toFixed(2)}-${o.toFixed(2)}`,stop_loss:l.stop_loss.toFixed(2)}:l.signal_type==="SELL"&&(v={action:"SELL",reason:`Strong ${l.signal_type} signal with ${l.confidence}% confidence. ${d.type} MTF alignment.`,entry_range:`${o.toFixed(2)}-${(o+5).toFixed(2)}`,stop_loss:l.stop_loss.toFixed(2)}):S==="EXTREME"?v.reason=`⚠️ EXTREME volatility (ADX ${i.adx.toFixed(1)}) - Too risky to trade. Wait for market to calm down.`:(d.type==="MIXED"||d.type==="CONFLICTING")&&(v.reason=`⏰ Timeframes conflicting (${d.score}/5 aligned). Wait for ${f[0]?`breakout above $${f[0].toFixed(2)}`:h[0]?`breakdown below $${h[0].toFixed(2)}`:"clearer direction"}.`);let A=!1;if(l.confidence>=65)try{const R=await t.prepare(`
          SELECT setting_key, setting_value FROM user_settings
          WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
...
`),N+="⏰ Don't miss this AI signal!",await q({botToken:I.telegram_bot_token,chatId:I.telegram_chat_id},N)},120*1e3),setTimeout(async()=>{let N=`${U} *⚠️ FINAL: AI SIGNAL STILL VALID* ${U}
...
`;const f=1e3*t.positionMultiplier,h=f/1e3*.1,y=r*.01*h*100,S=y/1e4*100,E=t.positionMultiplier>=1.5?"🟢🟢":t.positionMultiplier>=.9?"🟢":"🟡";return p+=`${E} <b>Position:</b> ${h.toFixed(2)} lots ($${f}) [${t.positionMultiplier}x]
`,p+=`⚠️ <b>Risk:</b> $${y.toFixed(2)} (${S.toFixed(2)}%)
...
`,p+=`Signal #${s} | ${_}`,p}Rt.get("/test-alert",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
    ...
    `).bind(f.signal_type,f.price,f.stop_loss,f.take_profit_1,f.take_profit_2,f.take_profit_3,m.confidence,f.setup_type,f.trend_5m,f.trend_15m,((a=f.indicators_5m)==null?void 0:a.rsi)||50,((n=f.indicators_5m)==null?void 0:n.macd)||0,((i=f.indicators_5m)==null?void 0:i.macd_signal)||0,((o=f.indicators_5m)==null?void 0:o.adx)||0,((l=f.indicators_5m)==null?void 0:l.stochastic_k)||50,f.reason,m.grade,m.filtersPassed,m.positionMultiplier).run();const h=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings
      WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
    `).all(),y={};for(const E of h.results||[])y[E.setting_key]=E.setting_value;let S=!1;if(y.telegram_bot_token&&y.telegram_chat_id){const E=ga(f,m,g,85,"NEW_YORK");S=await q({botToken:y.telegram_bot_token,chatId:y.telegram_chat_id},E),S&&await t.prepare(`
          UPDATE micro_trade_signals 
          SET telegram_sent = 1
          WHERE id = (SELECT MAX(id) FROM micro_trade_signals)
        `).run()}return e.json({success:!0,message:`${m.grade}-grade signal generated and ${S?"sent":"saved"}`,signal:{...f,grade:m.grade,filters_passed:m.filtersPassed,position_multiplier:m.positionMultiplier},telegram_sent:S,execution_time_ms:Date.now()-s})}catch(r){return console.error("[HYBRID-MICRO] Error:",r),e.json({success:!1,error:r.message},500)}});Rt.get("/signals/recent",async e=>{var a;const{DB:t}=e.env,s=parseInt(e.req.query("limit")||"10");try{const n=await t.prepare(`
      SELECT 
        id,
        signal_type,
        price,
        ...
        <script>
            ...
            async function testTelegram() {
                try {
                    ...
                    if (res.success) {
                        alert('SUCCESS:  Telegram test message sent successfully!');
                    } else {
                        alert('ERROR:  Failed to send Telegram message. Check your settings.');
                    }
                } catch (error) {
            ...
            async function sendMicroTestAlert() {
                try {
                    if (!confirm('TARGET:  This will send a HYBRID GRADE A signal to your Telegram.
...
Continue?')) {
                    ...
                    if (res.success) {
                        alert('SUCCESS:  Hybrid Grade A signal sent!

Check your Telegram for:

Signal: HIGH-QUALITY SIGNAL [A]
                ...
                try {
                    ...
                    if (res.success) {
                        ...
                        if (scan.grade === 'A' || scan.grade === 'A+') {
                            ...
                            html += '<p class="text-sm font-bold text-yellow-200"> DECENT SETUP - WAIT FOR CONFIRMATION</p>';
                            html += '</div>';
                        } else {
                    ...
                    alert('ERROR:  Error running 5M scan: ' + error.message);
                ...
                try {
                    ...
                    if (res.success) {
                        ...
                        if (res.telegram_sent) {
                            ...
                            message += '  Telegram not configured (check settings)';
                        }
                ...
                try {
                    ...
                    if (res.success) {
                        ...
                        statusDiv.innerHTML = '<i class="fas fa-check-circle text-green-400 mr-2"></i>' +
                            ...
                            (telegram_sent ? ' |  Sent to Telegram' : ' |  Telegram not configured');
                        ...
                        if (day.isValid && day.signal_type !== 'HOLD') {
                            ...
                            html += '<div class="text-yellow-400"> Day Trade: SKIP (' + day.mtf_reason + ')</div>';
                        }
                        
                        if (swing.isValid && swing.signal_type !== 'HOLD') {
                            html += '<div class="text-green-400 font-bold"> Swing Trade: EXECUTE ' + swing.signal_type + '</div>';
                        } else {
                            html += '<div class="text-yellow-400"> Swing Trade: SKIP (' + swing.mtf_reason + ')</div>';
                        }
                        html += '</div>';
                ...
                try {
                    ...
                    if (resData.success) {
                        ...
                        if (r.win_rate >= 70 && r.profit_factor >= 2.0) {
                            ...
                            html += '<div class="text-yellow-300"> <strong>GOOD PERFORMANCE</strong> - Strategy shows promise but needs refinement.</div>';
...
Your Gold/USD Trading System is connected and working!
 ...
 Telegram alerts are active.`);return e.json({success:n})}catch(s){return e.json({success:!1,error:s.message},500)}});G.post("/api/news/fetch",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
      ...
      `).bind(m.timestamp,m.open,m.high,m.low,m.close,m.volume));await t.batch(u);const _=c.length;if(c.length>=50){const m=pe(c.reverse());if(m){await t.prepare(`
        ...
        `).bind(m.rsi_14,m.macd,m.macd_signal,m.macd_histogram,m.sma_20,m.sma_50,m.sma_200,m.ema_12,m.ema_26,m.bb_upper,m.bb_middle,m.bb_lower,m.atr_14,m.stochastic_k,m.stochastic_d,m.adx,m.plus_di,m.minus_di,m.ichimoku_tenkan,m.ichimoku_kijun,m.ichimoku_senkou_a,m.ichimoku_senkou_b,m.parabolic_sar,m.vwap,m.fib_382||0,m.fib_500||0,m.fib_618||0).run();const p=c[c.length-1].close,g=le(p,m,"day_trade"),f=le(p,m,"swing_trade"),h=70;for(const y of[g,f])if(y.confidence>=h&&y.signal_type!=="HOLD"){await t.prepare(`
            ...
            `).bind(y.signal_type,y.trading_style,y.price,y.stop_loss,y.take_profit_1,y.take_profit_2,y.take_profit_3,y.confidence,y.reason).run();const S=await t.prepare(`
              SELECT setting_key, setting_value FROM user_settings
              WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
            `).all(),E={};for(const T of S.results||[])E[T.setting_key]=T.setting_value;E.telegram_bot_token&&E.telegram_chat_id&&await q({botToken:E.telegram_bot_token,chatId:E.telegram_chat_id},lt(y))}}}return e.json({success:!0,count:_})}catch(s){return e.json({success:!1,error:s.message},500)}});G.get("/api/cron/ping",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
      SELECT close, timestamp FROM market_data 
      WHERE timeframe = '1h' 
    ...
    `).all(),a={};for(const v of s.results)a[v.setting_key]=v.setting_value;const n=a.twelve_data_api_key||"70140f57bea54c5e90768de696487d8f",i=a.telegram_bot_token,o=a.telegram_chat_id;console.log("[AUTO-FETCH] Settings loaded:",{hasApiKey:!!n,hasBotToken:!!i,botTokenLength:i?i.length:0,hasChatId:!!o,chatId:o});const d=`https://api.twelvedata.com/time_series?symbol=XAU/USD&interval=1h&apikey=${n}&outputsize=100`,u=await(await fetch(d)).json();if(u.code&&u.status==="error")return e.json({success:!1,error:u.message||"API error",telegram_sent:!1});if(!u.values||!Array.isArray(u.values))return e.json({success:!1,error:"No data available",telegram_sent:!1});const m=u.values.map(v=>({timestamp:v.datetime,open:parseFloat(v.open)||0,high:parseFloat(v.high)||0,low:parseFloat(v.low)||0,close:parseFloat(v.close)||0,volume:parseInt(v.volume||"0")||0})),p=m.map(v=>t.prepare(`
        INSERT OR REPLACE INTO market_data (timestamp, open, high, low, close, volume, timeframe)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).bind(v.timestamp,v.open,v.high,v.low,v.close,v.volume,"1h"));await t.batch(p);const g=pe(m);if(!g)return e.json({success:!0,count:m.length,message:"Data stored, but insufficient for indicators",telegram_sent:!1});let f=m[0].close,h=!1;try{console.log("[AUTO-FETCH] Fetching real-time price...");const A=await(await fetch(`https://api.twelvedata.com/price?symbol=XAU/USD&apikey=${n}`,{signal:AbortSignal.timeout(5e3)})).json();if(A.price){const R=parseFloat(A.price),I=f,j=Math.abs(R-I)/R*100;console.log(`[AUTO-FETCH] Real-time: $${R}, Last candle: $${I}, Diff: ${j.toFixed(2)}%`),j<2?(f=R,h=!0,console.log(`[AUTO-FETCH]  Using real-time price: $${R}`)):console.log(`[AUTO-FETCH]  Price diff too large (${j.toFixed(2)}%), using candle close`)}}catch(v){console.log("[AUTO-FETCH] Real-time price fetch failed, using candle close:",v.message)}const y=le(f,g,"day_trade"),S=le(f,g,"swing_trade");try{await t.prepare(`
        INSERT INTO signals 
        (timestamp, signal_type, trading_style, price, stop_loss, 
         take_profit_1, take_profit_2, take_profit_3, confidence, reason)
        VALUES (datetime('now'), ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
${I}
 ...
 <b>Stop Loss:</b> $${y.stop_loss.toFixed(2)}

 <b>Reason:</b>
${v(y.reason)}

⏰ ${new Date().toLocaleString()}`,j=await q({botToken:i,chatId:o},U);k.day_trade_send_result=j,console.log("[AUTO-FETCH] Day trade alert result:",j),j?(T=!0,b.push("Day Trade"),R&&y.signal_type!=="HOLD"&&(console.log("[AUTO-FETCH]  HIGH CONVICTION signal detected! Sending reminder alerts..."),setTimeout(async()=>{const L=`${A} <b>${ICONS.warning} REMINDER: HIGH CONVICTION SIGNAL</b>
...
⏰ Don't miss this trade!`;await q({botToken:i,chatId:o},L),console.log("[AUTO-FETCH] First reminder sent")},120*1e3),setTimeout(async()=>{const L=`${A} <b>${ICONS.warning} FINAL REMINDER: HIGH CONVICTION</b>
 ...
 TP1: $${y.take_profit_1.toFixed(2)}
 Stop: $${y.stop_loss.toFixed(2)}`;await q({botToken:i,chatId:o},L),console.log("[AUTO-FETCH] Final reminder sent")},300*1e3),b.push("High Conviction Reminders (2+5min)"))):console.error("[AUTO-FETCH] Failed to send day trade alert!")}if(k.swing_trade_checked=!0,console.log("[AUTO-FETCH] Checking swing trade...",{confidence:S.confidence,type:S.signal_type,threshold:80}),S.confidence>=80){k.swing_trade_send_attempted=!0,console.log("[AUTO-FETCH] Swing trade meets criteria! Sending alert...");const v=L=>L.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"),A=S.signal_type==="BUY"?ICONS.green:S.signal_type==="SELL"?ICONS.red:ICONS.gray,R=S.confidence>=85,I=R?"${ICONS.fire} <b>HIGH CONVICTION</b> ${ICONS.fire}":"",U=`${A} <b>GOLD/USD ${S.signal_type} SIGNAL</b>
${I}
 ...
 <b>Take Profits:</b>
   TP1: $${S.take_profit_1.toFixed(2)}
   TP2: $${S.take_profit_2.toFixed(2)}
   TP3: $${S.take_profit_3.toFixed(2)}

 <b>Stop Loss:</b> $${S.stop_loss.toFixed(2)}

 <b>Reason:</b>
${v(S.reason)}

⏰ ${new Date().toLocaleString()}`,j=await q({botToken:i,chatId:o},U);k.swing_trade_send_result=j,j&&(T=!0,b.push("Swing Trade"),R&&S.signal_type!=="HOLD"&&(console.log("[AUTO-FETCH]  HIGH CONVICTION swing signal! Sending reminder alerts..."),setTimeout(async()=>{const L=`${A} <b>${ICONS.warning} REMINDER: HIGH CONVICTION SWING</b>

 <b>${S.signal_type} Swing Trade</b>
 Current Price: $${f.toFixed(2)}
 Confidence: ${S.confidence.toFixed(1)}%
...
⏰ Don't miss this swing trade!`;await q({botToken:i,chatId:o},L),console.log("[AUTO-FETCH] Swing first reminder sent")},180*1e3),setTimeout(async()=>{const L=`${A} <b>${ICONS.warning} FINAL REMINDER: HIGH CONVICTION</b>
 ...
 <b>${S.signal_type} Swing Still Valid</b>
 Price: $${f.toFixed(2)}
 Confidence: ${S.confidence.toFixed(1)}%
 ...
 TP1: $${S.take_profit_1.toFixed(2)}
 Stop: $${S.stop_loss.toFixed(2)}`;await q({botToken:i,chatId:o},L),console.log("[AUTO-FETCH] Swing final reminder sent")},420*1e3),b.push("High Conviction Swing Reminders (3+7min)")))}}else console.log("[AUTO-FETCH] Telegram NOT configured or invalid token");return console.log(`[CRON] Processed ${m.length} candles, Telegram: ${T?"SENT":"NOT SENT"}, Alerts: ${b.join(", ")}`),e.json({success:!0,timestamp:new Date().toISOString(),data_fetched:{candles:m.length,latest_price:f,data_timestamp:m[0].timestamp},signals:{day_trade:{type:y.signal_type,confidence:y.confidence,price:f},swing_trade:{type:S.signal_type,confidence:S.confidence,price:f}},telegram:{configured:!!(i&&o),bot_token_set:!!i,chat_id_set:!!o,bot_token_valid:i!=="your_bot_token_here",sent:T,alerts:b},debug:{...k,day_trade_check:{confidence:y.confidence,min_confidence:E,meets_threshold:y.confidence>=E,signal_type:y.signal_type,sends_all_signals:!0,should_alert:y.confidence>=E},swing_trade_check:{confidence:S.confidence,min_confidence:80,meets_threshold:S.confidence>=80,signal_type:S.signal_type,sends_all_signals:!0,should_alert:S.confidence>=80}},message:T?` Alerts sent: ${b.join(", ")}`:" No alerts sent (signals below confidence threshold)"})}catch(s){return console.error("[CRON] Error:",s),e.json({success:!1,error:s.message,telegram_sent:!1},500)}});G.get("/api/test/auto-fetch-settings",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings 
      WHERE setting_key IN ('twelve_data_api_key', 'telegram_bot_token', 'telegram_chat_id')
...
${((u=m.regime)==null?void 0:u.should_trade)===!1?" <b>WARNING: Extreme volatility detected</b>":""}
 ...
 Dashboard: ${e.req.url.replace("/api/cron/hedge-fund","")}
    ...
    `).first();if(!r)return e.json({success:!1,error:"No market data available"});const d=r.close,c=s(o,d),u=o["1h"],_=le(d,u,"day_trade"),m=le(d,u,"swing_trade"),p=a(_.signal_type,c),g=a(m.signal_type,c),f={..._,base_confidence:_.confidence,mtf_confidence:p.confidence,final_confidence:Math.min(95,p.confidence),isValid:p.isValid,mtf_reason:p.reason,alignment_score:c.score,alignment_type:c.type,reason:`${_.reason}, MTF: ${p.reason}`},h={...m,base_confidence:m.confidence,mtf_confidence:g.confidence,final_confidence:Math.min(95,g.confidence),isValid:g.isValid,mtf_reason:g.reason,alignment_score:c.score,alignment_type:c.type,reason:`${m.reason}, MTF: ${g.reason}`},y=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings
      WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
    `).all(),S={};for(const b of y.results||[])S[b.setting_key]=b.setting_value;let E=!1,T=[];S.telegram_bot_token&&S.telegram_chat_id&&(f.isValid&&f.signal_type!=="HOLD"&&await q({botToken:S.telegram_bot_token,chatId:S.telegram_chat_id},` MULTI-TIMEFRAME CONFIRMED

${lt({...f,timestamp:new Date().toISOString()})}

Grade: ${n(c)}`)&&(T.push("day_trade"),E=!0),await new Promise(b=>setTimeout(b,1e3)),h.isValid&&h.signal_type!=="HOLD"&&await q({botToken:S.telegram_bot_token,chatId:S.telegram_chat_id},` MULTI-TIMEFRAME CONFIRMED

${lt({...h,timestamp:new Date().toISOString()})}

Grade: ${n(c)}`)&&(T.push("swing_trade"),E=!0));for(const b of[f,h])b.isValid&&await t.prepare(`
    ...
    `).all();let a="";for(const E of s.results||[])E.setting_key==="twelve_data_api_key"&&(a=E.setting_value);let n,i=!1;if(a&&a!=="your_api_key_here"){console.log("[GENERATE-NOW] Fetching FRESH data from Twelve Data API");try{const E=`https://api.twelvedata.com/time_series?symbol=XAU/USD&interval=1h&outputsize=100&apikey=${a}`,b=await(await fetch(E)).json();b.values&&b.values.length>=50?(n=b.values.reverse().map(k=>({timestamp:k.datetime,open:parseFloat(k.open)||0,high:parseFloat(k.high)||0,low:parseFloat(k.low)||0,close:parseFloat(k.close)||0,volume:parseFloat(k.volume)||0})),i=!0,console.log("[GENERATE-NOW]  Fresh data fetched! Price:",n[n.length-1].close)):console.log("[GENERATE-NOW]  API returned insufficient data, falling back to database")}catch(E){console.error("[GENERATE-NOW] API fetch failed:",E.message)}}if(!n||n.length===0){console.log("[GENERATE-NOW] Using database data (may be stale)");const E=await t.prepare(`
      ...
      SELECT setting_key, setting_value FROM user_settings
      WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
    `).all(),_={};for(const E of u.results||[])_[E.setting_key]=E.setting_value;let m=!1,p=[];const g=n.slice(-20),f=g.map(E=>E.high).sort((E,T)=>T-E),h=g.map(E=>E.low).sort((E,T)=>E-T),y=f.length>=3?[f[0],f[1],f[2]]:f.length>=1?[f[0]]:[],S=h.length>=3?[h[0],h[1],h[2]]:h.length>=1?[h[0]]:[];console.log("[GENERATE-NOW] S/R calculated - Resistance:",y,"Support:",S),_.telegram_bot_token&&_.telegram_chat_id&&(await q({botToken:_.telegram_bot_token,chatId:_.telegram_chat_id},lt({...d,timestamp:new Date().toISOString(),resistance:y,support:S}))&&(p.push("day_trade"),m=!0),await new Promise(b=>setTimeout(b,1e3)),await q({botToken:_.telegram_bot_token,chatId:_.telegram_chat_id},lt({...c,timestamp:new Date().toISOString(),resistance:y,support:S}))&&(p.push("swing_trade"),m=!0));for(const E of[d,c])await t.prepare(`
        INSERT INTO signals 
        (timestamp, signal_type, trading_style, price, stop_loss, 
    ...
    `).bind(a).first();if(!p)return e.json({success:!1,error:"Account not found"},404);const g=new Date().toISOString().split("T")[0],f=await t.prepare(`
      ...
      AND date(entry_time) = ?
      AND status = 'CLOSED'
    `).bind(a,g).all(),{checkDailyLossLimit:h}=await Promise.resolve().then(()=>mt),y=h(p,f.results);if(y.limit_exceeded)return e.json({success:!1,error:`Daily loss limit exceeded: ${y.current_loss_pct}% (max ${p.max_daily_loss_pct}%)`},400);const S=c*i,E=await t.prepare(`
      INSERT INTO trades (
...
${y.optimal_for_trading?" *Status:* Optimal for trading":" *Status:* Sub-optimal liquidity"}
 ...
 *LIQUIDITY ANALYSIS:*
${S.liquidity_score>=70?ICONS.green:S.liquidity_score>=50?"🟡":ICONS.red} *Score:* ${S.liquidity_score}/100
 *Session:* ${S.session} (${S.time_zone} LIQUIDITY)
 *Volume:* ${S.volume_trend} (${S.volume_percentile}%ile)
 *Spread:* ~${S.estimated_spread_pips} pips
 *Impact:* ~${S.price_impact_bps} bps ($100K)

 *POSITION SIZING:*
${S.position_size_multiplier>=1?ICONS.green:S.position_size_multiplier>=.75?"🟡":ICONS.red} *Recommended:* ${(S.position_size_multiplier*100).toFixed(0)}% of normal size
${S.optimal_for_trading?" *Status:* Optimal for trading":" *Status:* Sub-optimal liquidity"}


 *RECOMMENDATION*


${b.isValid&&b.signal_type!=="HOLD"?` Day Trade: EXECUTE ${b.signal_type}`:` Day Trade: SKIP (${b.mtf_reason})`}

${k.isValid&&k.signal_type!=="HOLD"?` Swing Trade: EXECUTE ${k.signal_type}`:` Swing Trade: SKIP (${k.mtf_reason})`}
...
${f.optimal_for_trading?" *Status:* Optimal for trading":" *Status:* Sub-optimal liquidity"}
...
${h.optimal_for_trading?" *Status:* Optimal for trading":" *Status:* Sub-optimal liquidity"}


 *RECOMMENDATION*


${E.isValid&&E.signal_type!=="HOLD"?` Day Trade: EXECUTE ${E.signal_type}`:` Day Trade: SKIP (${E.mtf_reason})`}

${T.isValid&&T.signal_type!=="HOLD"?` Swing Trade: EXECUTE ${T.signal_type}`:` Swing Trade: SKIP (${T.mtf_reason})`}
