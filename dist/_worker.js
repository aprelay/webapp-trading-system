...
🌐 Dashboard: ${e.req.url.replace("/api/automation/analyze-and-notify","")}
      `.trim(),{sendTelegramMessage:P}=await Promise.resolve().then(()=>Gs);j=await P({botToken:U.telegram_bot_token,chatId:U.telegram_chat_id},B)}if(n.steps[3].status=j?"completed":"failed",n.steps[3].data={telegramSent:j},E.isValid||S.isValid)for(const I of[E,S])I.isValid&&await t.prepare(`
            INSERT INTO mtf_signals 
            (timestamp, signal_type, trading_style, price, stop_loss, 
             take_profit_1, take_profit_2, take_profit_3, 
             base_confidence, mtf_confidence, final_confidence,
