...
`)}const es=Object.freeze(Object.defineProperty({__proto__:null,analyzeTimeframeAlignment:Jt,determineTrend:Vs,formatAlignmentReport:En,validateMultiTimeframeSignal:Zt},Symbol.toStringTag,{value:"Module"}));function ls(e,t=.95){if(e.length===0)return 0;const s=[...e].sort((o,i)=>o-i),a=Math.floor((1-t)*s.length);return Math.abs(s[a]||0)}function bn(e,t){const s=ls(e,.95),a=ls(e,.99),n=t*s,o=t*a;return{var_95:parseFloat(n.toFixed(2)),var_99:parseFloat(o.toFixed(2)),confidence_95:95,confidence_99:99,method:"historical",calculation_date:new Date().toISOString()}}function wn(e,t,s,a){const n=t-e,o=n/t*100;let i=0;for(let d=a.length-1;d>=0&&a[d].balance<t;d--)i++;const l=o<=s,r=o>s;return{current_balance:parseFloat(e.toFixed(2)),peak_balance:parseFloat(t.toFixed(2)),current_drawdown:parseFloat(n.toFixed(2)),current_drawdown_pct:parseFloat(o.toFixed(2)),max_allowed_drawdown_pct:s,is_within_limit:l,should_pause_trading:r,days_in_drawdown:i}}function Tn(e,t,s=5){let a=0;const n=[];for(const r of e){const c=Math.abs(r.entry_price-r.stop_loss)*r.position_size,p=c/t*100;a+=c,n.push({position_id:r.id,entry_price:r.entry_price,stop_loss:r.stop_loss,risk_amount:parseFloat(c.toFixed(2)),risk_pct:parseFloat(p.toFixed(2))})}const o=a/t*100,i=o<=s,l=t*(s/100)-a;return{total_open_positions:e.length,total_risk_amount:parseFloat(a.toFixed(2)),total_risk_pct:parseFloat(o.toFixed(2)),max_allowed_risk_pct:s,is_within_limit:i,available_risk:parseFloat(l.toFixed(2)),positions:n}}function Sn(e){if(e.length<30)return null;const s=e.slice(-30).map(r=>r.high),a=[];for(let r=2;r<s.length-2;r++)s[r]>s[r-1]&&s[r]>s[r-2]&&s[r]>s[r+1]&&s[r]>s[r+2]&&a.push({index:r,value:s[r]});if(a.length<3)return null;const n=a.slice(-3),[o,i,l]=n;if(i.value>o.value&&i.value>l.value&&Math.abs(o.value-l.value)/o.value<.02){const d=Math.min(o.value,l.value)*.995,c=d-(i.value-d);return{pattern_type:"Head and Shoulders",confidence:75,start_index:e.length-30+o.index,end_index:e.length-30+l.index,direction:"bearish",description:"Bearish reversal pattern with clear head and shoulders formation",target_price:parseFloat(c.toFixed(2)),invalidation_price:parseFloat(i.value.toFixed(2)),historical_win_rate:65}}return null}function vn(e){if(e.length<20)return null;const s=e.slice(-20).map(i=>i.close),a=s.slice(0,10),n=s.slice(10);if((a[a.length-1]-a[0])/a[0]>.02&&(Math.max(...n)-Math.min(...n))/n[0]<.015){const r=s[s.length-1],d=a[a.length-1]-a[0],c=r+d;return{pattern_type:"Bullish Flag",confidence:80,start_index:e.length-20,end_index:e.length-1,direction:"bullish",description:"Bullish continuation pattern - strong uptrend followed by consolidation",target_price:parseFloat(c.toFixed(2)),invalidation_price:parseFloat((r*.98).toFixed(2)),historical_win_rate:68}}return null}function xn(e){if(e.length<25)return null;const t=e.slice(-25),s=t.map(d=>d.high),a=t.map(d=>d.low),o=(Math.max(...s)-Math.min(...s))/Math.max(...s),i=a.slice(0,6),l=a.slice(-6),r=(Math.min(...l)-Math.min(...i))/Math.min(...i);if(o<.01&&r>.015){const d=Math.max(...s),c=t[t.length-1].close,p=d+(d-Math.min(...a));return{pattern_type:"Ascending Triangle",confidence:70,start_index:e.length-25,end_index:e.length-1,direction:"bullish",description:"Bullish breakout pattern with rising support and flat resistance",target_price:parseFloat(p.toFixed(2)),invalidation_price:parseFloat((c*.975).toFixed(2)),historical_win_rate:72}}return null}function kn(e){if(e.length<30)return null;const s=e.slice(-30).map(r=>r.low),a=[];for(let r=2;r<s.length-2;r++)s[r]<s[r-1]&&s[r]<s[r-2]&&s[r]<s[r+1]&&s[r]<s[r+2]&&a.push({index:r,value:s[r]});if(a.length<2)return null;const n=a.slice(-2),[o,i]=n;if(Math.abs(o.value-i.value)/o.value<.015){const r=Math.max(...s.slice(o.index,i.index))*1.005,d=r+(r-o.value);return{pattern_type:"Double Bottom",confidence:75,start_index:e.length-30+o.index,end_index:e.length-30+i.index,direction:"bullish",description:"Bullish reversal pattern with two equal lows",target_price:parseFloat(d.toFixed(2)),invalidation_price:parseFloat(o.value.toFixed(2)),historical_win_rate:66}}return null}function Rn(e){if(e.length<40)return null;const s=e.slice(-40).map(l=>l.close),a=s[0],n=Math.min(...s.slice(10,25)),o=s[25];if(Math.abs(a-o)/a<.02&&n<a*.95){const l=s.slice(25),r=Math.min(...l),d=(o-r)/o;if(d>.01&&d<.05){const c=a-n,p=o+c;return{pattern_type:"Cup and Handle",confidence:65,start_index:e.length-40,end_index:e.length-1,direction:"bullish",description:"Bullish continuation pattern with cup and handle formation",target_price:parseFloat(p.toFixed(2)),invalidation_price:parseFloat(r.toFixed(2)),historical_win_rate:61}}}return null}function Ln(e){const t=[],s=Sn(e);s&&t.push(s);const a=vn(e);a&&t.push(a);const n=xn(e);n&&t.push(n);const o=kn(e);o&&t.push(o);const i=Rn(e);i&&t.push(i);let l=0,r=0,d=0;for(const m of t)m.direction==="bullish"?(l++,d+=m.confidence):m.direction==="bearish"&&(r++,d+=m.confidence);let c="neutral",p=0;l>r?(c="bullish",p=Math.min(d/l/10,15)):r>l&&(c="bearish",p=Math.min(d/r/10,15));let g="";if(t.length===0)g="No significant chart patterns detected";else{const m=t.map(u=>u.pattern_type).join(", ");g=`Detected ${t.length} pattern(s): ${m}. Overall ${c} bias.`}return{patterns:t,overall_sentiment:c,confidence_boost:parseFloat(p.toFixed(1)),summary:g}}function In(e,t){const s=e/t*100;return s<.3?"LOW":s<.6?"MEDIUM":s<1?"HIGH":"EXTREME"}function $n(e,t){let s=0;if(s+=Math.min(e.adx,40),t>e.sma_20&&e.sma_20>e.sma_50&&e.sma_50>e.sma_200)s+=30;else if(t<e.sma_20&&e.sma_20<e.sma_50&&e.sma_50<e.sma_200)s+=30;else{const n=[t>e.sma_20,e.sma_20>e.sma_50,e.sma_50>e.sma_200].filter(Boolean).length;s+=n*10}const a=Math.abs(e.macd_histogram)/e.atr_14;return s+=Math.min(a*10,30),Math.min(Math.round(s),100)}function An(e,t){let s=0;e.rsi_14>70?s+=Math.min((e.rsi_14-70)*2,40):e.rsi_14<30&&(s+=Math.min((30-e.rsi_14)*2,40));const a=(t-e.bb_lower)/(e.bb_upper-e.bb_lower);return(a>.9||a<.1)&&(s+=30),e.stochastic_k>80?s+=Math.min((e.stochastic_k-80)*1.5,30):e.stochastic_k<20&&(s+=Math.min((20-e.stochastic_k)*1.5,30)),Math.min(Math.round(s),100)}function Dn(e,t,s){const a=In(t.atr_14,s),n=$n(t,s),o=An(t,s);let i,l,r,d,c,p;const g=e.slice(-10),m=g.map(h=>h.volume||0),u=m.reduce((h,y)=>h+y,0)/m.length,f=(g[g.length-1].volume||0)>u*1.5;return a==="EXTREME"&&f?s>t.bb_upper&&t.rsi_14>60?(i="BREAKOUT",l=75,r=!0,d="Trend-following (aggressive entry)",c=1.3,p="Explosive upside breakout with volume - ride the momentum"):s<t.bb_lower&&t.rsi_14<40?(i="BREAKDOWN",l=75,r=!1,d="Wait for stabilization",c=.5,p="Sharp breakdown in progress - avoid trading until dust settles"):(i="RANGING",l=50,r=!1,d="Wait for direction",c=.5,p="Extreme volatility without clear direction - wait for clarity"):t.adx>30&&n>70?s>t.sma_20&&t.sma_20>t.sma_50&&t.plus_di>t.minus_di?(i="STRONG_UPTREND",l=90,r=!0,d="Trend-following (buy dips, trail stops)",c=1.5,p="Strong bullish trend confirmed - ideal for aggressive long positions"):(i="STRONG_DOWNTREND",l=90,r=!1,d="Stay in cash or short",c=.3,p="Strong bearish trend - avoid long positions"):t.adx>20&&n>40?s>t.sma_50&&t.plus_di>t.minus_di?(i="WEAK_UPTREND",l=70,r=!0,d="Trend-following (selective entries)",c=1,p="Moderate bullish trend - trade with normal position sizing"):(i="WEAK_DOWNTREND",l=70,r=!1,d="Reduce exposure or stay flat",c=.5,p="Moderate bearish trend - reduce risk or wait"):(i="RANGING",l=80,o>60?(r=!0,d="Mean-reversion (fade extremes)",c=.8,p="Choppy market with mean-reversion opportunities - trade extremes only"):(r=!1,d="Wait for trend to develop",c=.5,p="Choppy market without clear opportunity - stay on sidelines")),{regime:i,confidence:l,volatility:a,trend_strength:n,mean_reversion_score:o,should_trade:r,recommended_strategy:d,risk_adjustment:c,description:p}}function Mn(e){const t=e.length;let s=0,a=0,n=0,o=0;for(let r=0;r<t;r++)s+=r,a+=e[r],n+=r*e[r],o+=r*r;const i=(t*n-s*a)/(t*o-s*s),l=(a-i*s)/t;return{slope:i,intercept:l}}function Nn(e,t,s){const a=e.map(l=>l.close),n=2/(t+1);let o=a[0];for(let l=1;l<a.length;l++)o=(a[l]-o)*n+o;const i=(a[a.length-1]-a[a.length-10])/10;return o+i*s}function On(e,t){const s=e.map(l=>l.close).slice(-20),a=[];for(let l=1;l<s.length;l++)a.push(s[l]-s[l-1]);const i=a.slice(-5).reduce((l,r)=>l+r,0)/5*t*Math.pow(.8,t);return s[s.length-1]+i}function Cn(e,t,s){const a=e[e.length-1].close;e.map(i=>i.close).slice(-20);let n=0;t.rsi_14>50?n+=t.rsi_14-50:n-=50-t.rsi_14,t.macd>t.macd_signal?n+=20:n-=20,a>t.sma_20&&(n+=10),a>t.sma_50&&(n+=10);const o=n/100*s;return a+t.atr_14*o}function Fn(e,t){const s=e.map(g=>g.close),a=s[s.length-1],n=10,o=s.slice(-n),i=Math.min(...o),l=Math.max(...o),r=o.map(g=>(g-i)/(l-i));let d={index:0,similarity:-1/0};for(let g=n;g<s.length-n-t;g++){const m=s.slice(g-n,g),u=Math.min(...m),_=Math.max(...m),f=m.map(w=>(w-u)/(_-u));let h=0;for(let w=0;w<n;w++)h+=Math.pow(r[w]-f[w],2);const y=-h;y>d.similarity&&(d={index:g,similarity:y})}const p=(s[d.index+t]-s[d.index])*(a/s[d.index]);return a+p}function Vt(e,t,s){const a=[],n=[],o=e.map(E=>E.close),{slope:i,intercept:l}=Mn(o.slice(-20)),r=i*(o.length-1+s)+l;a.push(r),n.push(1);const d=Nn(e,12,s);a.push(d),n.push(1.5);const c=On(e,s);a.push(c),n.push(1.2);const p=Cn(e,t,s);a.push(p),n.push(1.8);const g=Fn(e,s);a.push(g),n.push(1.3);const m=n.reduce((E,k)=>E+k,0),_=a.reduce((E,k,v)=>E+k*n[v],0)/m,f=a.reduce((E,k)=>E+k,0)/a.length,h=a.reduce((E,k)=>E+Math.pow(k-f,2),0)/a.length,y=Math.sqrt(h),w=e[e.length-1].close,b=1-y/w,T=Math.max(50,Math.min(95,b*100));return{prediction:_,confidence:T}}function Un(e,t){const s=e[e.length-1].close,a=[],n=Vt(e,t,1),o=n.prediction-s,i=o/s*100;a.push({timeframe:"1h",predicted_price:parseFloat(n.prediction.toFixed(2)),confidence_interval_upper:parseFloat((n.prediction+t.atr_14*.8).toFixed(2)),confidence_interval_lower:parseFloat((n.prediction-t.atr_14*.8).toFixed(2)),confidence:parseFloat(n.confidence.toFixed(1)),direction:o>.5?"UP":o<-.5?"DOWN":"NEUTRAL",expected_move_pct:parseFloat(i.toFixed(2)),method:"Ensemble (5 models)"});const l=Vt(e,t,4),r=l.prediction-s,d=r/s*100;a.push({timeframe:"4h",predicted_price:parseFloat(l.prediction.toFixed(2)),confidence_interval_upper:parseFloat((l.prediction+t.atr_14*1.5).toFixed(2)),confidence_interval_lower:parseFloat((l.prediction-t.atr_14*1.5).toFixed(2)),confidence:parseFloat(l.confidence.toFixed(1)),direction:r>2?"UP":r<-2?"DOWN":"NEUTRAL",expected_move_pct:parseFloat(d.toFixed(2)),method:"Ensemble (5 models)"});const c=Vt(e,t,24),p=c.prediction-s,g=p/s*100;a.push({timeframe:"24h",predicted_price:parseFloat(c.prediction.toFixed(2)),confidence_interval_upper:parseFloat((c.prediction+t.atr_14*2.5).toFixed(2)),confidence_interval_lower:parseFloat((c.prediction-t.atr_14*2.5).toFixed(2)),confidence:parseFloat(c.confidence.toFixed(1)),direction:p>5?"UP":p<-5?"DOWN":"NEUTRAL",expected_move_pct:parseFloat(g.toFixed(2)),method:"Ensemble (5 models)"});const m=a.filter(y=>y.direction==="UP").length,u=a.filter(y=>y.direction==="DOWN").length;let _,f=0;m>u?(_="BULLISH",f=Math.min(m*5,15)):u>m?(_="BEARISH",f=Math.min(u*5,15)):_="NEUTRAL";const h=`ML models predict ${_} movement. 1h: ${a[0].direction} (${a[0].expected_move_pct.toFixed(2)}%), 4h: ${a[1].direction} (${a[1].expected_move_pct.toFixed(2)}%), 24h: ${a[2].direction} (${a[2].expected_move_pct.toFixed(2)}%)`;return{current_price:parseFloat(s.toFixed(2)),predictions:a,overall_direction:_,confidence_boost:parseFloat(f.toFixed(1)),summary:h}}function qt(e,t,s,a,n){const i=Math.abs(t-e)/s;let l;i<1?l=80:i<2?l=65:i<3?l=50:i<4?l=35:l=20;const r=(a-50)/10;l+=r;const d=(n-1)*5;return l+=d,Math.max(5,Math.min(95,l))}function Bn(e,t,s,a,n){const i=Math.abs(e-t)/s;let l;if(i<1?l=60:i<1.5?l=40:i<2?l=25:l=15,n==="BUY"){const r=(a-50)/10;l-=r}else{const r=(a-50)/10;l-=r}return Math.max(5,Math.min(80,l))}function Hn(e,t,s,a,n,o){const i=(s-e)*.5,l=(a-e)*.3,r=(n-e)*.2,d=t-e;return o.tp1/100*i+o.tp2/100*l+o.tp3/100*r+o.sl/100*d}function Pn(e,t,s){const a=e.price,n=t.atr_14;let o=50;e.signal_type==="BUY"?(a>t.sma_20&&(o+=10),a>t.sma_50&&(o+=10),t.adx>25&&(o+=Math.min(t.adx-25,20)),t.macd>t.macd_signal&&(o+=10)):e.signal_type==="SELL"&&(a<t.sma_20&&(o+=10),a<t.sma_50&&(o+=10),t.adx>25&&(o+=Math.min(t.adx-25,20)),t.macd<t.macd_signal&&(o+=10)),o=Math.min(100,o);const i=s.slice(-50),l=[];for(let w=14;w<i.length;w++){const b=i.slice(w-14,w);let T=0;for(let E=1;E<b.length;E++){const k=Math.max(b[E].high-b[E].low,Math.abs(b[E].high-b[E-1].close),Math.abs(b[E].low-b[E-1].close));T+=k}l.push(T/14)}const r=l.reduce((w,b)=>w+b,0)/l.length,d=n/r,c=qt(a,e.take_profit_1,n,o,d),p=qt(a,e.take_profit_2,n,o,d),g=qt(a,e.take_profit_3,n,o,d),m=Bn(a,e.stop_loss,n,o,e.signal_type),u=Hn(a,e.stop_loss,e.take_profit_1,e.take_profit_2,e.take_profit_3,{tp1:c,tp2:p,tp3:g,sl:m}),f=(c+p+g)/3/m;let h;c>70&&u>5&&f>2?h="STRONG_TRADE":c>60&&u>0&&f>1.5?h="GOOD_TRADE":c>50&&u>-2?h="MARGINAL_TRADE":h="AVOID_TRADE";const y=`TP1 has ${c.toFixed(0)}% chance of hitting. Expected value: $${u.toFixed(2)}. Risk-adjusted R:R: ${f.toFixed(2)}:1. Recommendation: ${h.replace(/_/g," ")}`;return{tp1_probability:parseFloat(c.toFixed(1)),tp2_probability:parseFloat(p.toFixed(1)),tp3_probability:parseFloat(g.toFixed(1)),stop_loss_probability:parseFloat(m.toFixed(1)),expected_value:parseFloat(u.toFixed(2)),risk_reward_adjusted:parseFloat(f.toFixed(2)),recommendation:h,summary:y}}const ts=["2025-01-28","2025-01-29","2025-03-18","2025-03-19","2025-04-29","2025-04-30","2025-06-17","2025-06-18","2025-07-29","2025-07-30","2025-09-16","2025-09-17","2025-10-28","2025-10-29","2025-12-09","2025-12-10"];function jn(e,t){let s=1;for(;s<=7;){if(new Date(e,t,s).getDay()===5)return s;s++}return 1}function Pt(e=30){const t=[],s=new Date;for(const n of ts){const o=new Date(n),i=Math.floor((o.getTime()-s.getTime())/(1e3*60*60*24));i>=0&&i<=e&&(t.push({date:n,time:"19:00",title:"FOMC Interest Rate Decision",country:"USD",impact:"high",source:"static"}),t.push({date:n,time:"19:30",title:"FOMC Press Conference (Powell)",country:"USD",impact:"high",source:"static"}))}for(let n=0;n<=e;n++){const o=new Date(s.getTime()+n*24*60*60*1e3),i=o.getFullYear(),l=o.getMonth(),r=o.getDate(),d=o.getDay();if(r===jn(i,l)&&d===5){const c=o.toISOString().split("T")[0];t.push({date:c,time:"13:30",title:"US Non-Farm Payrolls (NFP)",country:"USD",impact:"high",source:"static"}),t.push({date:c,time:"13:30",title:"US Unemployment Rate",country:"USD",impact:"high",source:"static"})}r===10&&t.push({date:o.toISOString().split("T")[0],time:"13:30",title:"US Consumer Price Index (CPI)",country:"USD",impact:"high",source:"static"}),r===11&&t.push({date:o.toISOString().split("T")[0],time:"13:30",title:"US Producer Price Index (PPI)",country:"USD",impact:"high",source:"static"}),r===15&&t.push({date:o.toISOString().split("T")[0],time:"13:30",title:"US Retail Sales",country:"USD",impact:"high",source:"static"}),(r===1||r<=3&&d>=1&&d<=5)&&t.push({date:o.toISOString().split("T")[0],time:"15:00",title:"US ISM Manufacturing PMI",country:"USD",impact:"high",source:"static"}),(r===3||r<=5&&d>=1&&d<=5)&&t.push({date:o.toISOString().split("T")[0],time:"15:00",title:"US ISM Services PMI",country:"USD",impact:"high",source:"static"})}return t.filter((n,o,i)=>o===i.findIndex(l=>l.date===n.date&&l.time===n.time&&l.title===n.title)).sort((n,o)=>{const i=new Date(`${n.date}T${n.time}:00Z`),l=new Date(`${o.date}T${o.time}:00Z`);return i.getTime()-l.getTime()})}function Rt(e=new Date,t=[]){const s=[...Pt(7),...t],a=s.filter(i=>new Date(`${i.date}T${i.time}:00Z`)>e).slice(0,10),n=e.toISOString().split("T")[0];if(s.filter(i=>i.date===n&&i.impact==="high"),ts.includes(n))return{shouldTrade:!1,reason:"🚨 FOMC Meeting Day - No trading recommended",riskLevel:"danger",upcomingEvents:a,nextSafeTime:Wn(n)};new Date(e.getTime()+7200*1e3);for(const i of s){const l=new Date(`${i.date}T${i.time}:00Z`),r=(l.getTime()-e.getTime())/(1e3*60);if(i.impact==="high"&&r>0&&r<=30)return{shouldTrade:!1,reason:`🚨 HIGH IMPACT: ${i.title} in ${Math.round(r)} minutes`,riskLevel:"danger",upcomingEvents:a,nextSafeTime:new Date(l.getTime()+1800*1e3).toISOString()};if(i.impact==="high"&&r>30&&r<=120)return{shouldTrade:!0,reason:`⚠️ CAUTION: ${i.title} in ${Math.round(r)} minutes`,riskLevel:"caution",upcomingEvents:a,nextSafeTime:void 0}}const o=new Date(e.getTime()-1800*1e3);for(const i of s){const l=new Date(`${i.date}T${i.time}:00Z`);if(i.impact==="high"&&l>o&&l<e){const r=(e.getTime()-l.getTime())/6e4;return{shouldTrade:!1,reason:`🚨 ${i.title} just happened ${Math.round(r)} min ago - Wait for volatility to settle`,riskLevel:"danger",upcomingEvents:a,nextSafeTime:new Date(l.getTime()+1800*1e3).toISOString()}}}return{shouldTrade:!0,reason:"✅ No major economic events - Safe to trade",riskLevel:"safe",upcomingEvents:a}}function Wn(e){const t=new Date(e);return t.setDate(t.getDate()+1),t.toISOString().split("T")[0]}function Ot(e){const s=new Date(`${e.date}T${e.time}:00Z`).toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit",timeZone:"UTC"});let a="🔴";return e.impact==="medium"&&(a="🟡"),e.impact==="low"&&(a="🟢"),`${a} ${e.date} ${s} UTC - ${e.title}`}function Yn(e){const t=e.toISOString().split("T")[0];return ts.includes(t)?!0:Pt(30).filter(n=>n.date===t&&n.impact==="high").length>=2}function Gn(){const e=new Date().toISOString().split("T")[0];return Pt(7).filter(s=>s.date===e)}function qs(e=new Date){const t=Rt(e);return t.riskLevel==="danger"?{adjustment:-30,reason:t.reason}:t.riskLevel==="caution"?{adjustment:-15,reason:t.reason}:{adjustment:0,reason:t.reason}}const zs=new ge;zs.post("/enhanced",async e=>{var s;const{DB:t}=e.env;try{console.log("[ENHANCED] Starting request, DB:",!!t),console.log("[ENHANCED] Step 1: Fetching MTF data");const a={},n={};for(const H of["5m","15m","1h","4h","daily"]){const F=await t.prepare(`
    ...
    `).first(),d=(r==null?void 0:r.setting_value)||"70140f57bea54c5e90768de696487d8f";try{console.log("[ENHANCED] Fetching real-time price...");const F=await(await fetch(`https://api.twelvedata.com/price?symbol=XAU/USD&apikey=${d}`,{signal:AbortSignal.timeout(5e3)})).json();if(F.price){const X=parseFloat(F.price),A=l,de=Math.abs(X-A)/X*100;console.log(`[ENHANCED] Real-time: $${X}, Last candle: $${A}, Diff: ${de.toFixed(2)}%`),de<2?(l=X,console.log(`[ENHANCED] ✅ Using real-time price: $${X}`)):console.log(`[ENHANCED] ⚠️ Price diff too large (${de.toFixed(2)}%), using candle close`)}}catch(H){console.log("[ENHANCED] Real-time price fetch failed, using candle close:",H.message)}if(i!=null&&i.timestamp){const H=new Date(i.timestamp).getTime(),F=(Date.now()-H)/(1e3*60);F>60&&o.push(`⚠️ WARNING: Price data is ${F.toFixed(0)} minutes old`),console.log(`[ENHANCED] Price freshness: ${F.toFixed(1)} minutes old`)}console.log("[ENHANCED] Step 1.5: Checking economic calendar");const c=Rt(),p=qs();let g=null,m=!1;c.riskLevel==="danger"?(m=!0,g=c.reason,console.log("[ENHANCED] ⚠️ CALENDAR DANGER:",c.reason)):c.riskLevel==="caution"?(g=c.reason,console.log("[ENHANCED] ⚠️ CALENDAR CAUTION:",c.reason)):console.log("[ENHANCED] ✅ Calendar safe:",c.reason);const u=a["1h"];if(!u)return e.json({success:!1,error:`1h indicators not found. Available timeframes: ${Object.keys(a).join(", ")}`},400);const _=Jt(a,l),f=le(l,u,"day_trade"),h=le(l,u,"swing_trade"),y=Zt(f.signal_type,_),w=Zt(h.signal_type,_),b={...f,base_confidence:f.confidence,mtf_confidence:y.confidence,final_confidence:Math.min(95,y.confidence),isValid:y.isValid,mtf_reason:y.reason,alignment_score:_.score,alignment_type:_.type},T={...h,base_confidence:h.confidence,mtf_confidence:w.confidence,final_confidence:Math.min(95,w.confidence),isValid:w.isValid,mtf_reason:w.reason,alignment_score:_.score,alignment_type:_.type};let E=0,k="",v=[];if(n["1h"]&&Array.isArray(n["1h"])&&n["1h"].length>=20){try{const F=Ln(n["1h"]);v=(F==null?void 0:F.patterns)||[]}catch(F){console.error("[ENHANCED] Pattern detection error:",F.message)}const H=v.filter(F=>F.confidence>=70&&F.endIndex>=n["1h"].length-5);for(const F of H)F.type==="bullish"&&b.signal_type==="BUY"?(E+=F.confidence*.1,k+=`${F.name} (${F.confidence.toFixed(0)}%), `):F.type==="bearish"&&b.signal_type==="SELL"&&(E+=F.confidence*.1,k+=`${F.name} (${F.confidence.toFixed(0)}%), `);E=Math.min(15,E)}let $=0,R="",L=null;if(n["1h"]&&n["1h"].length>=50){const H=pe(n["1h"]);H&&(L=Dn(n["1h"],H),L.trend==="STRONG_UPTREND"&&b.signal_type==="BUY"?($=10,R="Strong Uptrend"):L.trend==="UPTREND"&&b.signal_type==="BUY"?($=5,R="Uptrend"):L.trend==="STRONG_DOWNTREND"&&b.signal_type==="SELL"?($=10,R="Strong Downtrend"):L.trend==="DOWNTREND"&&b.signal_type==="SELL"&&($=5,R="Downtrend"))}let O=0,W="",I=null;if(n["1h"]&&Array.isArray(n["1h"])&&n["1h"].length>=50)try{I=Un(n["1h"],l),I.overall_direction==="BULLISH"&&b.signal_type==="BUY"?(O=I.confidence_boost,W=`ML predicts +${((I.predictions[0].predicted_price/l-1)*100).toFixed(2)}% in 1h`):I.overall_direction==="BEARISH"&&b.signal_type==="SELL"&&(O=I.confidence_boost,W=`ML predicts ${((I.predictions[0].predicted_price/l-1)*100).toFixed(2)}% in 1h`)}catch(H){console.error("[ENHANCED] ML prediction error:",H.message)}let C=0,D="",B=null;if(n["1h"]&&Array.isArray(n["1h"])&&n["1h"].length>=50)try{const H=pe(n["1h"]);H&&(B=Pn(b,H,n["1h"]),B.tp1_probability>70?(C=10,D=`PoP: TP1 ${B.tp1_probability.toFixed(0)}%`):B.tp1_probability>60&&(C=5,D=`PoP: TP1 ${B.tp1_probability.toFixed(0)}%`))}catch(H){console.error("[ENHANCED] Probability of Profit error:",H.message)}let P=null,N=0,U=0;if(n["1h"]&&Array.isArray(n["1h"])&&n["1h"].length>=20)try{P=Ht(n["1h"]),P.liquidity_score>=80?N=5:P.liquidity_score>=70?N=0:P.liquidity_score>=50?U=-5:U=-10,console.log(`[LIQUIDITY] Score: ${P.liquidity_score}/100, Session: ${P.session}, Adjust: ${N+U}%`)}catch(H){console.error("[ENHANCED] Liquidity Analysis error:",H.message)}let V=0,Q=0,ee=0,te=0,se="";try{const H=await t.prepare(`
      ...
      `).all(),X=await t.prepare(`
        ...
        WHERE account_id = 1
      `).all();if(H&&F.results&&F.results.length>=10){const A=bn(F.results,H.balance);V=A.var_95,Q=A.var_99;const he=wn(H.balance,F.results);if(ee=he.current_drawdown_pct,he.is_within_limit||(se+=`⚠️ Drawdown ${ee.toFixed(1)}% exceeds limit. `),X.results){const de=Tn(X.results,H.balance);te=de.total_risk_pct,de.is_within_limit||(se+=`⚠️ Portfolio heat ${te.toFixed(1)}% exceeds limit. `)}}}catch(H){console.error("[ENHANCED] Risk metrics error (optional):",H.message)}const x=E+$+O+C+N+U,M={...b,pattern_boost:E,regime_boost:$,ml_boost:O,pop_boost:C,total_boost:x,enhanced_confidence:Math.min(98,b.final_confidence+x),var_95:V,var_99:Q,current_drawdown_pct:ee,portfolio_heat_pct:te,risk_warning:se||null},ue={...T,pattern_boost:E,regime_boost:$,ml_boost:O,pop_boost:C,total_boost:x,enhanced_confidence:Math.min(98,T.final_confidence+x),var_95:V,var_99:Q,current_drawdown_pct:ee,portfolio_heat_pct:te,risk_warning:se||null};m?(M.signal_type="HOLD",ue.signal_type="HOLD",M.enhanced_confidence=50,ue.enhanced_confidence=50,M.reasoning=g||"Economic event nearby - trading paused",ue.reasoning=g||"Economic event nearby - trading paused",console.log("[ENHANCED] ⚠️ SIGNALS FORCED TO HOLD due to calendar")):p.adjustment<0&&(M.enhanced_confidence=Math.max(50,M.enhanced_confidence+p.adjustment),ue.enhanced_confidence=Math.max(50,ue.enhanced_confidence+p.adjustment),console.log("[ENHANCED] Applied calendar adjustment:",p.adjustment)),M.calendar_check={risk_level:c.riskLevel,should_trade:c.shouldTrade,reason:c.reason,confidence_adjustment:p.adjustment,upcoming_events:c.upcomingEvents.slice(0,3).map(H=>Ot(H))},ue.calendar_check=M.calendar_check;let fe=!1;try{const H=await t.prepare(`
        SELECT setting_key, setting_value FROM user_settings
        WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
      `).all(),F={};for(const X of H.results||[])F[X.setting_key]=X.setting_value;if(F.telegram_bot_token&&F.telegram_chat_id){const X=new Date().toLocaleString("en-US",{timeZone:"UTC"});let A=`🏦 *HEDGE FUND GRADE SIGNAL*
⏰ ${X} UTC
...
`,A+=`${_.type} (${_.score}/5 timeframes)
`,A+=`Confidence Boost: +${_.confidenceBoost}%

`;for(const J of _.trends){const Te=J.trend==="BULLISH"?"📈":J.trend==="BEARISH"?"📉":"➡️";A+=`${Te} *${J.timeframe}*: ${J.trend} (${J.confidence.toFixed(0)}%)
`}A+=`
━━━━━━━━━━━━━━━━━━━━━━━━━
`,A+=`📈 *DAY TRADE SIGNAL*
`,A+=`━━━━━━━━━━━━━━━━━━━━━━━━━
...
`;const he=candles1h.slice(-20),de=he.map(J=>J.high).sort((J,Te)=>Te-J),ae=he.map(J=>J.low).sort((J,Te)=>J-Te),Lt=de.slice(0,3),_t=ae.slice(0,3);if(A+=`📊 *Key Levels:*
...
`,E>0&&(A+=`Pattern: +${E.toFixed(0)}%
`),$>0&&(A+=`Regime: +${$.toFixed(0)}%
`),O>0&&(A+=`ML: +${O.toFixed(0)}%
`),C>0&&(A+=`PoP: +${C.toFixed(0)}%
`),N!==0||U!==0){const J=N+U;A+=`Liquidity: ${J>=0?"+":""}${J.toFixed(0)}%
...
`),A+=`
🌐 Dashboard: ${e.req.url.replace("/api/signals/enhanced/enhanced","")}`,console.log("[TELEGRAM] Message 1 length:",A.length,"characters");const ft=await q({botToken:F.telegram_bot_token,chatId:F.telegram_chat_id},A);let K=`📊 *ADDITIONAL ANALYSIS*
━━━━━━━━━━━━━━━━━━━━━━━━━
...
`,K+="🤖 Powered by Hedge Fund Grade AI",console.log("[TELEGRAM] Message 2 length:",K.length,"characters");const Be=await q({botToken:F.telegram_bot_token,chatId:F.telegram_chat_id},K);fe=ft&&Be}}catch(H){console.error("[ENHANCED] Telegram error (optional):",H.message)}return e.json({success:!0,timestamp:new Date().toISOString(),current_price:l,telegram_sent:fe,day_trade:M,swing_trade:ue,alignment:{type:_.type,score:_.score,trends:_.trends},patterns:v.length>0?v.slice(0,3):null,regime:L?{trend:L.trend,volatility:L.volatility,should_trade:L.should_trade}:null,ml_prediction:I?{direction:I.overall_direction,predictions:I.predictions}:null,profit_probability:B?{tp1:B.tp1_probability,tp2:B.tp2_probability,tp3:B.tp3_probability,expected_value:B.expected_value}:null,liquidity:P?{score:P.liquidity_score,session:P.session,time_zone:P.time_of_day_zone,volume_trend:P.volume_trend,volume_percentile:P.volume_percentile,estimated_spread_pips:P.estimated_spread_pips,price_impact_bps:P.price_impact_bps,market_depth_score:P.market_depth_score,optimal_for_trading:P.optimal_for_trading,warnings:P.warnings,recommendation:P.recommendation}:null,risk_metrics:{var_95:V,var_99:Q,drawdown_pct:ee,portfolio_heat_pct:te}})}catch(a){return console.error("[ENHANCED] Error:",a.message,a.stack),e.json({success:!1,error:a.message,stack:a.stack},500)}});const Xs=new ge;Xs.post("/simple",async e=>{var s,a,n,o;const{DB:t}=e.env;try{console.log("[SIMPLE] Starting simple signal generation");const i=await t.prepare(`
      SELECT * FROM multi_timeframe_indicators 
      WHERE timeframe = '1h'
      ORDER BY timestamp DESC 
      LIMIT 1
    ...
    `).all()).results||[]).map(h=>({timestamp:h.timestamp,open:h.open,high:h.high,low:h.low,close:h.close,volume:h.volume||1})).reverse(),u=ze(d,p,m,"day_trade"),_=ze(d,p,m,"swing_trade");console.log("[SIMPLE] Generated signals with liquidity:",{day:{type:u.signal_type,confidence:u.confidence,liquidity_score:u.liquidity_score,session:u.session},swing:{type:_.signal_type,confidence:_.confidence,liquidity_score:_.liquidity_score,session:_.session}});let f=!1;try{const h=await t.prepare(`
      ...
      `).all(),y={};for(const w of h.results||[])y[w.setting_key]=w.setting_value;if(console.log("[SIMPLE] Telegram config:",{hasToken:!!y.telegram_bot_token,hasChat:!!y.telegram_chat_id,tokenLength:((o=y.telegram_bot_token)==null?void 0:o.length)||0,chatId:y.telegram_chat_id}),y.telegram_bot_token&&y.telegram_chat_id){const w=await t.prepare(`
          ...
          ORDER BY timestamp DESC 
          LIMIT 20
        `).all();let b=[],T=[];if(w.results&&w.results.length>=20){const W=w.results.map(C=>C.high).sort((C,D)=>D-C),I=w.results.map(C=>C.low).sort((C,D)=>C-D);b=W.slice(0,3),T=I.slice(0,3)}const E=u.signal_type==="BUY"?"🟢":u.signal_type==="SELL"?"🔴":"⚪",k=new Date().toLocaleString("en-US",{timeZone:"UTC"});let v=`${E} <b>GOLD/USD ${u.signal_type} SIGNAL</b> ${E}
...
`,b.length>0&&(v+=`📊 <b>Key Levels:</b>
`,v+=`🔴 <b>Resistance:</b> ${b.map(W=>`$${W.toFixed(2)}`).join(", ")}
`,v+=`🟢 <b>Support:</b> ${T.map(W=>`$${W.toFixed(2)}`).join(", ")}

`),v+=`💧 <b>LIQUIDITY ANALYSIS:</b>
`;const $=u.liquidity_score&&u.liquidity_score>=70?"🟢":u.liquidity_score&&u.liquidity_score>=50?"🟡":"🔴";v+=`${$} <b>Score:</b> ${u.liquidity_score||50}/100
...
`;const R=u.position_size_multiplier||1,L=R>=1?"🟢":R>=.75?"🟡":"🔴";if(v+=`
💼 <b>POSITION SIZING:</b>
...
`),u.liquidity_warnings&&u.liquidity_warnings!=="[]")try{const W=JSON.parse(u.liquidity_warnings);W.length>0&&(v+=`
⚠️ <b>WARNINGS:</b>
`,W.slice(0,2).forEach(I=>{const C=I.replace(/[⚠️🔴]/g,"").trim();v+=`• ${C}
`}))}catch{}v+=`
`,v+=`📝 <b>Reason:</b>
`;const O=String(u.reason).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");v+=O+`
...
`,v+=`⏰ ${k}`,console.log("[SIMPLE] Sending Telegram message, length:",v.length),f=await q({botToken:y.telegram_bot_token,chatId:y.telegram_chat_id},v),console.log("[SIMPLE] Telegram sent:",f),f||console.log("[SIMPLE] Telegram send failed - checking response")}else console.log("[SIMPLE] Telegram not configured")}catch(h){console.error("[SIMPLE] Telegram error:",h.message)}try{await t.prepare(`
    ...
    `).all()).results.map(u=>({timestamp:u.timestamp,open:Number(u.open),high:Number(u.high),low:Number(u.low),close:Number(u.close),volume:Number(u.volume)||0})).reverse(),d=r[r.length-1].close;console.log("[5M-SCANNER] Step 7: Starting 7-layer analysis");const c=await Zs(t,n,o,i,r,d);console.log("[5M-SCANNER] Step 8: Analysis complete:",{grade:c.grade,score:c.score,signal:c.signal}),console.log("[5M-SCANNER] Step 9: Saving to database");const p=new Date().toISOString();console.log("[5M-SCANNER] Step 10: Timestamp created:",p),await t.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(p,"5m",c.signal,c.grade,c.score,d,c.stopLoss,c.tp1,c.tp2,c.tp3,c.confidence,c.layersPassed,c.liquidityScore,c.session,0).run();let g=!1;if(c.grade==="A"||c.grade==="A+")try{const u=await t.prepare(`
          SELECT setting_key, setting_value FROM user_settings
          WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
        `).all(),_={};for(const f of u.results||[])_[f.setting_key]=f.setting_value;if(_.telegram_bot_token&&_.telegram_chat_id){const f=Qs(c,d);g=await q({botToken:_.telegram_bot_token,chatId:_.telegram_chat_id},f),await t.prepare(`
            UPDATE scanner_history 
            SET telegram_sent = ?
            WHERE timestamp = (SELECT MAX(timestamp) FROM scanner_history)
          `).bind(g?1:0).run(),console.log("[5M-SCANNER] Telegram alert sent:",g)}}catch(u){console.error("[5M-SCANNER] Telegram error:",u.message)}const m=new Date;return console.log("[5M-SCANNER] Scan complete, returning results"),e.json({success:!0,timestamp:m.toISOString(),scan_result:{grade:c.grade,score:c.score,signal:c.signal,confidence:c.confidence,entry:d,stop_loss:c.stopLoss,targets:[c.tp1,c.tp2,c.tp3],layers_passed:c.layersPassed,telegram_sent:g}})}catch(s){console.error("[5M-SCANNER] Error caught:",s);const a=(s==null?void 0:s.message)||String(s);return console.error("[5M-SCANNER] Error message:",a),e.json({success:!1,error:a},500)}});dt.get("/scan",async e=>{const{DB:t}=e.env;try{console.log("[5M-SCANNER-GET] Step 1: Creating Date object");const s=new Date;console.log("[5M-SCANNER-GET] Step 2: Date object created:",typeof s),console.log("[5M-SCANNER-GET] Step 3: Converting to ISO string");const a=s.toISOString();console.log("[5M-SCANNER-GET] Starting scan at",a),console.log("[5M-SCANNER-GET] Step 4: Creating table"),await t.prepare(`
      CREATE TABLE IF NOT EXISTS scanner_history (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        timestamp DATETIME NOT NULL,
        timeframe TEXT NOT NULL,
    ...
    `).first();if(!n||!o||!i)return console.log("[5M-SCANNER-GET] Missing indicators:",{has5m:!!n,has15m:!!o,has1h:!!i}),e.json({success:!1,error:"Insufficient data for scan. Please run /api/market/fetch-mtf first."});const r=(await t.prepare(`
      ...
      ORDER BY timestamp DESC 
      LIMIT 100
    `).all()).results.map(m=>({timestamp:m.timestamp,open:Number(m.open),high:Number(m.high),low:Number(m.low),close:Number(m.close),volume:Number(m.volume)||0})).reverse();if(!r||r.length===0)return e.json({success:!1,error:"No 5m market data available"});const d=r[r.length-1].close,c=await Zs(t,n,o,i,r,d),p=new Date().toISOString();await t.prepare(`
      INSERT INTO scanner_history 
      (timestamp, timeframe, signal_type, grade, score, 
       entry_price, stop_loss, take_profit_1, take_profit_2, take_profit_3,
       confidence, layers_passed, liquidity_score, session, telegram_sent)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(p,"5m",c.signal,c.grade,c.score,d,c.stopLoss,c.tp1,c.tp2,c.tp3,c.confidence,c.layersPassed,c.liquidityScore,c.session,0).run();let g=!1;if(c.grade==="A"||c.grade==="A+")try{const m=await t.prepare(`
          SELECT setting_key, setting_value FROM user_settings
          WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
        `).all(),u={};for(const h of m.results||[]){const y=h;u[y.setting_key]=y.setting_value}const _=u.telegram_bot_token,f=u.telegram_chat_id;if(_&&f&&_!=="your_bot_token_here"&&f!=="your_chat_id_here"){const h=`
...
⏰ ${new Date().toLocaleString("en-US",{timeZone:"UTC"})} UTC
          `.trim();await q(_,f,h),g=!0,console.log("[5M-SCANNER-GET] Telegram alert sent for",c.grade,"grade")}}catch(m){console.error("[5M-SCANNER-GET] Telegram error:",m)}return e.json({success:!0,timestamp:p,scan_result:{grade:c.grade,score:c.score,signal:c.signal,confidence:c.confidence,entry:d,stop_loss:c.stopLoss,targets:[c.tp1,c.tp2,c.tp3],layers_passed:c.layersPassed,telegram_sent:g}})}catch(s){console.error("[5M-SCANNER-GET] Fatal error:",s);const a=(s==null?void 0:s.message)||String(s);return console.error("[5M-SCANNER-GET] Error message:",a),e.json({success:!1,error:a},500)}});dt.get("/stats",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
    ...
    `).all();return e.json({success:!0,history:s.results})}catch(s){return e.json({success:!1,error:s.message},500)}});dt.post("/test-alert",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings
      WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
    `).all(),a={};for(const d of s.results||[])a[d.setting_key]=d.setting_value;if(!a.telegram_bot_token||!a.telegram_chat_id)return e.json({success:!1,error:"Telegram not configured. Please set Bot Token and Chat ID in settings."});const n=4386.5,o=15,i={grade:"A",signal:"BUY",confidence:87,session:"LONDON",layersPassed:6,layers:["✅ Layer 1: Trend Aligned (BULLISH)","✅ Layer 2: RSI 54, MACD bullish crossover","✅ Layer 3: Volume spike 1.9x average","✅ Layer 4: Broke above resistance","✅ Layer 5: Liquidity 89/100 (LONDON session)","✅ Layer 6: No major news","❌ Layer 7: ADX 72.3 (extreme, reversal risk)"],stopLoss:n-o,tp1:n+o*2,tp2:n+o*3,tp3:n+o*4,liquidityScore:89,adx:72.3,rsi:54.2,volumeRatio:1.9},l=Qs(i,n),r=await q({botToken:a.telegram_bot_token,chatId:a.telegram_chat_id},l);return e.json({success:r,message:r?"Test A-grade alert sent to Telegram!":"Failed to send alert. Check your Telegram settings."})}catch(s){return console.error("[TEST-ALERT] Error:",s),e.json({success:!1,error:s.message},500)}});async function Zs(e,t,s,a,n,o){console.log("[ANALYZE] Starting analysis");let i=0,l=0;const r=[],d=(ne,Ze)=>{const $t=parseFloat(String(ne));return isNaN($t)?Ze:$t};console.log("[ANALYZE] parseNum defined");const c={ema20:d(t.ema_12,o),rsi:d(t.rsi_14,50),macd:d(t.macd,0),macd_signal:d(t.macd_signal,0),macd_histogram:d(t.macd_histogram,0),adx:d(t.adx,25)},p={ema50:d(s.ema_26,o)},g={sma200:d(a.sma_200,o)},m=o>c.ema20&&o>p.ema50&&o>g.sma200,u=o<c.ema20&&o<p.ema50&&o<g.sma200;m||u?(i+=20,l++,r.push(`✅ Layer 1: Trend Aligned (${m?"BULLISH":"BEARISH"})`)):r.push("❌ Layer 1: Trend NOT aligned (conflicting)");const _=c.rsi>=40&&c.rsi<=60,f=c.macd>c.macd_signal&&c.macd_histogram>0,h=c.macd<c.macd_signal&&c.macd_histogram<0;_&&(m?f:h)?(i+=15,l++,r.push(`✅ Layer 2: RSI ${c.rsi.toFixed(0)}, MACD ${m?"bullish":"bearish"} crossover`)):r.push(`❌ Layer 2: RSI ${c.rsi.toFixed(0)}, MACD ${_?"no crossover":"extreme"}`);const y=n.slice(-20).reduce((ne,Ze)=>ne+Ze.volume,0)/20,w=n[n.length-1].volume;w>y*1.5?(i+=15,l++,r.push(`✅ Layer 3: Volume spike ${(w/y).toFixed(1)}x average`)):r.push(`❌ Layer 3: Volume ${(w/y).toFixed(1)}x (need 1.5x+)`);const T=Math.max(...n.slice(-20).map(ne=>ne.high)),E=Math.min(...n.slice(-20).map(ne=>ne.low)),k=o>T*.999,v=o<E*1.001;m&&k||u&&v?(i+=15,l++,r.push(`✅ Layer 4: ${m?"Broke above resistance":"Broke below support"}`)):r.push("❌ Layer 4: No clear breakout"),console.log("[ANALYZE] Layer 5: Calculating liquidity");let $=null;try{$=await Ht(n),console.log("[ANALYZE] Liquidity calculated successfully")}catch(ne){console.log("[5M-SCANNER] Liquidity calc failed:",ne)}const R=($==null?void 0:$.liquidity_score)||50,L=($==null?void 0:$.session)||"UNKNOWN";R>=70?(i+=15,l++,r.push(`✅ Layer 5: Liquidity ${R}/100 (${L} session)`)):r.push(`❌ Layer 5: Liquidity ${R}/100 (too low)`),console.log("[ANALYZE] Layer 6: Checking economic calendar");const W=Rt();console.log("[ANALYZE] Calendar check complete"),W.riskLevel==="safe"?(i+=10,l++,r.push("✅ Layer 6: No major news")):r.push(`❌ Layer 6: ${W.reason}`);const C=c.adx>25,D=c.adx>70;C&&!D?(i+=10,l++,r.push(`✅ Layer 7: ADX ${c.adx.toFixed(1)} (strong trend)`)):D?r.push(`⚠️ Layer 7: ADX ${c.adx.toFixed(1)} (extreme, reversal risk)`):r.push(`❌ Layer 7: ADX ${c.adx.toFixed(1)} (weak trend)`);let B="HOLD";(m||u)&&l>=5&&(B=m?"BUY":"SELL");const P=new Date,N=Vn(P);N.hasBoost?(i+=8,l++,r.push(`✅ Layer 8: ${N.reason} (+${N.boost}% win)`)):r.push(`ℹ️ Layer 8: ${N.reason}`);const U=qn(P);U.hasBoost?(i+=5,l++,r.push(`✅ Layer 9: ${U.reason} (+${U.boost}% win)`)):r.push(`ℹ️ Layer 9: ${U.reason}`);const V=d(t.atr_14,o*.01),Q=n.slice(-20).reduce((ne,Ze)=>{const $t=Ze.high-Ze.low;return ne+$t},0)/20;if(zn(V,Q)){i+=7,l++;const ne=((V/Q-1)*100).toFixed(1);r.push(`✅ Layer 10: ATR expanding ${ne}% (high volatility)`)}else{const ne=((1-V/Q)*100).toFixed(1);r.push(`❌ Layer 10: ATR compressed ${ne}% (skip low volatility)`)}const te=Xn(n.slice(-20));Ks(te,B)&&te.strength>=60&&(i+=10,l++),r.push(Kn(te,B));const x=Zn(n.slice(-3)),{aligned:M,strongestPattern:ue}=lo(x,B);M&&ue?(i+=12,l++,r.push(`✅ Layer 12: ${ue.name} (${ue.strength}/100)`)):x.length>0&&x[0].type==="INDECISION"?r.push(`⚠️ Layer 12: ${x[0].name} (indecision, wait)`):r.push("❌ Layer 12: No clear candlestick pattern");const fe=co(n,o);_o(fe,B)&&fe.nearZone?(i+=8,l++,r.push(`✅ Layer 13: ${fe.description}`)):fe.nearZone?r.push(`⚠️ Layer 13: ${fe.description} (not aligned)`):r.push("ℹ️ Layer 13: No key zones nearby");const X=(await e.prepare(`
    SELECT rsi_14 as rsi, macd, macd_histogram
    FROM multi_timeframe_indicators
    WHERE timeframe = '5m'
    ORDER BY timestamp DESC
    LIMIT 10
  `).all()).results.map(ne=>({rsi:parseFloat(String(ne.rsi))||50,macd:parseFloat(String(ne.macd))||0,macd_histogram:parseFloat(String(ne.macd_histogram))||0})).reverse(),A=fo(X,n.slice(-10)),de=bo(A,B,m?"BULLISH":u?"BEARISH":"NEUTRAL");de&&A.strength>=70&&(i+=9,l++),r.push(wo(A,de));const ae=To(t,s,a,o),Lt=So(ae,B);Lt&&(ae.allAligned||ae.twoAligned)&&(i+=6,l++),r.push(vo(ae,Lt));const _t=await e.prepare(`
    SELECT setting_value FROM user_settings
    WHERE setting_key = 'twelve_data_api_key'
  `).first(),ft=(_t==null?void 0:_t.setting_value)||"70140f57bea54c5e90768de696487d8f",K=await Ao(e,ft,15),Be=xo(K);ko(Be,B)&&Be.strength>=60?(i+=5,l++,r.push(`✅ Layer 18: ${Be.description}`)):Be.goldSignalSupport!=="NEUTRAL"?r.push(`⚠️ Layer 18: ${Be.description} (not aligned)`):r.push("ℹ️ Layer 18: DXY flat (neutral for Gold)");const Te=await ms(e,ft,"SILVER",15),ba=await ms(e,ft,"OIL",15),Ke=Do(Te,ba,B);if(Ke.aligned&&Ke.alignmentCount>=1){const ne=Ke.alignmentCount===2?5:3;i+=ne,l++,r.push(`✅ Layer 19: ${Ke.description} (${Ke.strength}/100)`)}else r.push(`❌ Layer 19: ${Ke.description}`);const wa=await Go(e)||Vo(),Le=Uo(wa);if(Ho(Le,B)&&Le.strength>=70){const ne=Le.positioning.includes("EXTREME")?7:4;i+=ne,l++,r.push(`✅ Layer 20: ${Le.description} (${Le.strength}/100)`)}else Le.goldSignalSupport!=="NEUTRAL"?r.push(`⚠️ Layer 20: ${Le.description} (not aligned)`):r.push(`ℹ️ Layer 20: ${Le.description}`);let It="C";i>=162?It="A+":i>=144?It="A":i>=126&&(It="B"),(m||u)&&l>=7&&(B=m?"BUY":"SELL");const Ie=Math.max(V*1.5,o*.003),Ta=B==="BUY"?o-Ie:o+Ie,Sa=B==="BUY"?o+Ie*2:o-Ie*2,va=B==="BUY"?o+Ie*3:o-Ie*3,xa=B==="BUY"?o+Ie*4:o-Ie*4;return{grade:It,score:i,signal:B,confidence:i,layersPassed:l,layers:r,stopLoss:Ta,tp1:Sa,tp2:va,tp3:xa,liquidityScore:R,session:L,adx:c.adx,rsi:c.rsi,volumeRatio:w/y}}function Qs(e,t){const s=e.signal==="BUY"?"🟢":"🔴",a=e.grade==="A+"?"💎":e.grade==="A"?"⭐":"📊",n=new Date,o=`${n.getUTCHours().toString().padStart(2,"0")}:${n.getUTCMinutes().toString().padStart(2,"0")}`;let i=`🚨 <b>${e.grade}-GRADE 5M SETUP DETECTED!</b> 🚨
...
`,i+="Next scan in 5 minutes...",i}async function mt(e){const t=await e.prepare(`
  ...
  `).bind(t).all();if(s.results.length===0)return;const a=s.results,n=a.length,o=a.filter(m=>m.win===1).length,i=a.filter(m=>m.win===0).length,l=o/n*100,r=a.reduce((m,u)=>m+(u.profit_loss||0),0),d=Math.max(...a.map(m=>m.profit_loss||0)),c=Math.min(...a.map(m=>m.profit_loss||0)),p=a.reduce((m,u)=>m+(u.confidence||0),0)/n,g=a.reduce((m,u)=>m+(u.mtf_score||0),0)/n;await e.prepare(`
    ...
    ON CONFLICT(trade_date) DO UPDATE SET
      ...
      updated_at = CURRENT_TIMESTAMP
  `).bind(t,n,o,i,l,r,d,c,p,g).run()}async function Xo(e){const t=await mt(e),s=(t.starting_balance-t.current_balance)/t.starting_balance*100,a=new Date().toISOString().split("T")[0],n=await e.prepare(`
  ...
  `).bind(s,o).run();let i=!1,l="";s>=t.max_drawdown_pct?(i=!0,l=`Max drawdown ${s.toFixed(1)}% reached (limit ${t.max_drawdown_pct}%)`):o>=t.max_daily_loss_pct&&(i=!0,l=`Daily loss ${o.toFixed(1)}% reached (limit ${t.max_daily_loss_pct}%)`),i&&t.trading_enabled===1&&await e.prepare(`
    ...
    `).bind(s).run(),e.json({success:!0,message:"Event deleted"})}catch(t){return e.json({success:!1,error:t.message},500)}});function oa(e,t,s){const a=s.find(h=>t.confidence>=h.confidence_min&&t.confidence<=h.confidence_max);if(!a)return{units:0,value:0,risk_amount:0,risk_pct:0,position_pct:0,stop_loss_distance:0,reward_risk_ratio:0,is_valid:!1,warning:`No position sizing rule found for confidence ${t.confidence}%`};const n=Math.abs(t.entry_price-t.stop_loss),i=e.current_balance*(a.risk_pct/100)/n,l=i*t.entry_price;l/e.current_balance*100;const r=e.current_balance*(a.max_position_pct/100);let d=i,c=l,p=a.risk_pct,g;l>r&&(c=r,d=r/t.entry_price,p=d*n/e.current_balance*100,g=`Position reduced to ${a.max_position_pct}% max position size`);const u=Math.abs(t.take_profit_1-t.entry_price)/n;let _=!0;const f=[];return g&&f.push(g),u<1.5&&f.push(`Low reward:risk ratio (${u.toFixed(2)}:1). Recommended: >1.5:1`),p>e.max_daily_loss_pct&&(_=!1,f.push(`Risk ${p.toFixed(2)}% exceeds max daily loss ${e.max_daily_loss_pct}%`)),d<.01&&(_=!1,f.push("Position size too small (minimum 0.01 lots)")),{units:parseFloat(d.toFixed(2)),value:parseFloat(c.toFixed(2)),risk_amount:parseFloat((d*n).toFixed(2)),risk_pct:parseFloat(p.toFixed(2)),position_pct:parseFloat((c/e.current_balance*100).toFixed(2)),stop_loss_distance:parseFloat(n.toFixed(2)),reward_risk_ratio:parseFloat(u.toFixed(2)),is_valid:_,warning:f.length>0?f.join("; "):void 0}}function ia(e,t,s,a,n=0){let o;a==="BUY"?o=(t-e)*s:o=(e-t)*s,o-=n;const i=(t-e)/e*100,l=Math.abs(t-e)/.01;return{profit_loss:parseFloat(o.toFixed(2)),profit_loss_pct:parseFloat(i.toFixed(2)),pips:parseFloat(l.toFixed(1))}}function Ko(e,t){const s=t.filter(l=>l.profit_loss<0).reduce((l,r)=>l+r.profit_loss,0),a=Math.abs(s/e.current_balance)*100,n=a>=e.max_daily_loss_pct,i=e.current_balance*(e.max_daily_loss_pct/100)-Math.abs(s);return{current_loss:parseFloat(s.toFixed(2)),current_loss_pct:parseFloat(a.toFixed(2)),limit_exceeded:n,remaining:parseFloat(i.toFixed(2))}}function Zo(e){const t=e.filter(_=>_.status==="CLOSED"),s=t.filter(_=>_.profit_loss>0),a=t.filter(_=>_.profit_loss<0),n=s.reduce((_,f)=>_+f.profit_loss,0),o=Math.abs(a.reduce((_,f)=>_+f.profit_loss,0)),i=n-o,l=s.length>0?n/s.length:0,r=a.length>0?o/a.length:0,d=t.length>0?s.length/t.length*100:0,c=o>0?n/o:n,p=100-d,g=d/100*l-p/100*r,m=s.length>0?Math.max(...s.map(_=>_.profit_loss)):0,u=a.length>0?Math.min(...a.map(_=>_.profit_loss)):0;return{total_trades:t.length,winning_trades:s.length,losing_trades:a.length,win_rate:parseFloat(d.toFixed(2)),total_profit:parseFloat(n.toFixed(2)),total_loss:parseFloat(o.toFixed(2)),net_profit:parseFloat(i.toFixed(2)),avg_win:parseFloat(l.toFixed(2)),avg_loss:parseFloat(r.toFixed(2)),profit_factor:parseFloat(c.toFixed(2)),expectancy:parseFloat(g.toFixed(2)),largest_win:parseFloat(m.toFixed(2)),largest_loss:parseFloat(u.toFixed(2))}}function Qo(e){return e.is_valid?`
...
${e.warning?`⚠️ ${e.warning}`:"✅ Valid position"}
  `.trim():`❌ Invalid: ${e.warning}`}const pt=Object.freeze(Object.defineProperty({__proto__:null,calculatePortfolioMetrics:Zo,calculatePositionSize:oa,calculateProfitLoss:ia,checkDailyLossLimit:Ko,formatPositionSize:Qo},Symbol.toStringTag,{value:"Module"}));async function ra(e,t,s){const a=Date.now(),n=[],o=[];let i=t.starting_balance,l=t.starting_balance;const r=e.filter(N=>{const U=new Date(N.timestamp);return U>=new Date(t.start_date)&&U<=new Date(t.end_date)});if(r.length<200)throw new Error(`Not enough candles for backtest. Need at least 200, got ${r.length}`);const d={current_balance:i,max_daily_loss_pct:2};for(let N=200;N<r.length;N++){const U=r.slice(N-200,N),V=pe(U);if(!V)continue;const Q=r[N],ee=Q.close,te=le(ee,V,"day_trade"),se=le(ee,V,"swing_trade");for(const x of[te,se]){if(x.signal_type==="HOLD"||x.confidence<t.min_confidence)continue;d.current_balance=i;const M=oa(d,{entry_price:x.price,stop_loss:x.stop_loss,take_profit_1:x.take_profit_1,take_profit_2:x.take_profit_2,take_profit_3:x.take_profit_3,confidence:x.confidence,signal_type:x.signal_type,trading_style:x.trading_style},s);if(!M.is_valid)continue;const ue=Q.timestamp,fe=x.price;let H=null,F=null,X="UNKNOWN";const A=Math.min(50,r.length-N-1);for(let de=1;de<=A;de++){const ae=r[N+de];if(x.signal_type==="BUY"){if(ae.low<=x.stop_loss){H=x.stop_loss,F=ae.timestamp,X="STOP_LOSS";break}if(ae.high>=x.take_profit_3){H=x.take_profit_3,F=ae.timestamp,X="TP3";break}if(ae.high>=x.take_profit_2){H=x.take_profit_2,F=ae.timestamp,X="TP2";break}if(ae.high>=x.take_profit_1){H=x.take_profit_1,F=ae.timestamp,X="TP1";break}}else{if(ae.high>=x.stop_loss){H=x.stop_loss,F=ae.timestamp,X="STOP_LOSS";break}if(ae.low<=x.take_profit_3){H=x.take_profit_3,F=ae.timestamp,X="TP3";break}if(ae.low<=x.take_profit_2){H=x.take_profit_2,F=ae.timestamp,X="TP2";break}if(ae.low<=x.take_profit_1){H=x.take_profit_1,F=ae.timestamp,X="TP1";break}}}if(!H||!F)continue;const he=ia(fe,H,M.units,x.signal_type,t.commission_per_trade);i+=he.profit_loss,i>l&&(l=i),n.push({entry_time:ue,entry_price:fe,exit_time:F,exit_price:H,signal_type:x.signal_type,trading_style:x.trading_style,position_size:M.units,profit_loss:he.profit_loss,profit_loss_pct:he.profit_loss_pct,exit_reason:X,confidence:x.confidence}),o.push({date:F,balance:i})}}const c=n.filter(N=>N.profit_loss>0),p=n.filter(N=>N.profit_loss<0),g=c.reduce((N,U)=>N+U.profit_loss,0),m=Math.abs(p.reduce((N,U)=>N+U.profit_loss,0)),u=i-t.starting_balance,_=n.length>0?c.length/n.length*100:0,f=c.length>0?g/c.length:0,h=p.length>0?m/p.length:0,y=c.length>0?Math.max(...c.map(N=>N.profit_loss)):0,w=p.length>0?Math.min(...p.map(N=>N.profit_loss)):0,b=m>0?g/m:g,T=100-_,E=_/100*f-T/100*h;let k=0,v=0,$=t.starting_balance;for(const N of o){N.balance>$&&($=N.balance);const U=$-N.balance,V=U/$*100;U>k&&(k=U,v=V)}const R=n.map(N=>N.profit_loss_pct),L=R.reduce((N,U)=>N+U,0)/R.length,O=Math.sqrt(R.reduce((N,U)=>N+Math.pow(U-L,2),0)/R.length),W=O>0?L/O:0;let I=0,C=0,D=0,B=0;for(const N of n)N.profit_loss>0?(D++,B=0,I=Math.max(I,D)):(B++,D=0,C=Math.max(C,B));const P=Date.now()-a;return{config:t,total_trades:n.length,winning_trades:c.length,losing_trades:p.length,win_rate:parseFloat(_.toFixed(2)),net_profit:parseFloat(u.toFixed(2)),total_return_pct:parseFloat((u/t.starting_balance*100).toFixed(2)),avg_win:parseFloat(f.toFixed(2)),avg_loss:parseFloat(h.toFixed(2)),largest_win:parseFloat(y.toFixed(2)),largest_loss:parseFloat(w.toFixed(2)),max_drawdown:parseFloat(k.toFixed(2)),max_drawdown_pct:parseFloat(v.toFixed(2)),profit_factor:parseFloat(b.toFixed(2)),sharpe_ratio:parseFloat(W.toFixed(2)),expectancy:parseFloat(E.toFixed(2)),max_consecutive_wins:I,max_consecutive_losses:C,starting_balance:t.starting_balance,ending_balance:parseFloat(i.toFixed(2)),peak_balance:parseFloat(l.toFixed(2)),trades:n,equity_curve:o,execution_time_ms:P}}function la(e){return["🎯 BACKTEST RESULTS","","📊 Configuration:",`   Period: ${e.config.start_date} to ${e.config.end_date}`,`   Starting Balance: $${e.starting_balance.toLocaleString()}`,`   Min Confidence: ${e.config.min_confidence}%`,`   Commission: $${e.config.commission_per_trade} per trade`,"","📈 Performance:",`   Total Trades: ${e.total_trades}`,`   Winning Trades: ${e.winning_trades} (${e.win_rate}%)`,`   Losing Trades: ${e.losing_trades}`,`   Net Profit: $${e.net_profit.toLocaleString()} (${e.total_return_pct}%)`,`   Ending Balance: $${e.ending_balance.toLocaleString()}`,"","💰 Profit Metrics:",`   Average Win: $${e.avg_win.toLocaleString()}`,`   Average Loss: $${e.avg_loss.toLocaleString()}`,`   Largest Win: $${e.largest_win.toLocaleString()}`,`   Largest Loss: $${e.largest_loss.toLocaleString()}`,`   Profit Factor: ${e.profit_factor}`,`   Expectancy: $${e.expectancy.toFixed(2)} per trade`,"","⚠️ Risk Metrics:",`   Max Drawdown: $${e.max_drawdown.toLocaleString()} (${e.max_drawdown_pct}%)`,`   Sharpe Ratio: ${e.sharpe_ratio.toFixed(2)}`,`   Max Consecutive Wins: ${e.max_consecutive_wins}`,`   Max Consecutive Losses: ${e.max_consecutive_losses}`,"",`⏱️ Execution Time: ${e.execution_time_ms}ms`].join(`
`)}const Jo=Object.freeze(Object.defineProperty({__proto__:null,formatBacktestResults:la,runBacktest:ra},Symbol.toStringTag,{value:"Module"})),ut=new ge;ut.post("/run",async e=>{try{const{DB:t}=e.env,s=await e.req.json(),a=await t.prepare(`
      SELECT timestamp, open, high, low, close, volume
      FROM market_data
      WHERE timeframe = '1h'
    ...
    `).bind(s.run_name||`Backtest ${new Date().toISOString()}`,o.start_date,o.end_date,o.starting_balance,o.min_confidence,o.use_mtf_confirmation?1:0,o.use_news_filter?1:0,o.timeframe,l.total_trades,l.winning_trades,l.win_rate,l.net_profit,l.total_return_pct,l.max_drawdown_pct,l.profit_factor,l.sharpe_ratio,JSON.stringify(l.trades),JSON.stringify(l.equity_curve)).run();let d=!1;try{const c=await t.prepare(`
        SELECT setting_key, setting_value FROM user_settings 
        WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
      `).all(),p={};if(c.results.forEach(g=>{g.setting_key==="telegram_bot_token"&&(p.telegram_bot_token=g.setting_value),g.setting_key==="telegram_chat_id"&&(p.telegram_chat_id=g.setting_value)}),p.telegram_bot_token&&p.telegram_chat_id){const g=l;let m="",u="";g.total_trades<10?(m="⏳ INSUFFICIENT DATA",u="⏳"):g.total_trades<50?(m="⚠️ SMALL SAMPLE SIZE",u="⚠️"):g.win_rate>=70&&g.profit_factor>=2?(m="✅ STRATEGY VALIDATED",u="✅"):g.win_rate>=60?(m="⚠️ GOOD PERFORMANCE",u="⚠️"):(m="❌ NEEDS IMPROVEMENT",u="❌");const _=`
🎯 *BACKTEST COMPLETE*
...
📅 Backtest ID: ${r.meta.last_row_id}
    ...
    `).bind(s).run(),e.json({success:!0,message:"Backtest deleted"})}catch(t){return e.json({success:!1,error:t.message},500)}});ut.get("/data-availability",async e=>{try{const{DB:t}=e.env,s=await t.prepare(`
      ...
      FROM market_data
      WHERE timeframe = '1h'
    `).first();if(!s||s.total_candles===0)return e.json({success:!0,available:!1,message:"No historical data available. Fetch market data first.",total_candles:0});const a=new Date(s.earliest_date),n=new Date(s.latest_date),o=Math.floor((n.getTime()-a.getTime())/(1e3*60*60*24));return e.json({success:!0,available:s.total_candles>=200,total_candles:s.total_candles,earliest_date:s.earliest_date,latest_date:s.latest_date,days_covered:o,min_required:200,ready_for_backtest:s.total_candles>=200})}catch(t){return e.json({success:!1,error:t.message},500)}});const ca=new ge;ca.post("/webhook",async e=>{try{const{DB:t}=e.env,s=await e.req.json(),a=s.message||s.edited_message;if(!a||!a.text)return e.json({ok:!0});const n=a.chat.id,o=a.text.trim(),i=await t.prepare(`
      SELECT setting_value FROM user_settings WHERE setting_key = 'telegram_bot_token'
    `).first();if(!i)return e.json({ok:!0});const l={botToken:i.setting_value,chatId:n.toString()};if(o.startsWith("/log_trade")){const r=o.split(" ");if(r.length<5)return await q(l,"❌ Usage: /log_trade <BUY|SELL> <entry> <stop> <tp1>"),e.json({ok:!0});const d=r[1].toUpperCase(),c=parseFloat(r[2]),p=parseFloat(r[3]),g=parseFloat(r[4]),m=await ea({trade_type:d,trading_style:"day_trade",entry_price:c,entry_time:new Date().toISOString(),position_size:0,position_value:0,stop_loss:p,take_profit_1:g,take_profit_2:g*1.002,take_profit_3:g*1.003,status:"OPEN",confidence:85},t);m.success?await q(l,`✅ *Trade #${m.trade_id} Logged*

${d} @ $${c}
Stop: $${p}
TP1: $${g}`):await q(l,`❌ Error: ${m.error}`)}else if(o.startsWith("/close_trade")){const r=o.split(" ");if(r.length<4)return await q(l,"❌ Usage: /close_trade <id> <exit_price> <reason>"),e.json({ok:!0});const d=parseInt(r[1]),c=parseFloat(r[2]),p=r[3],g=await ta(d,c,p,t);if(g.success){const m=g.profit_loss||0,u=m>0?"💰":"❌";await q(l,`${u} *Trade #${d} Closed*

Exit: $${c}
P&L: ${m>0?"+":""}$${m.toFixed(2)}
Result: ${m>0?"WIN ✅":"LOSS ❌"}`)}else await q(l,`❌ Error: ${g.error}`)}else if(o==="/open"){const r=await na(t);if(r.length===0)await q(l,"📊 No open positions");else{let d=`📊 *Open Positions (${r.length})*
...
/help - Show this message`);return e.json({ok:!0})}catch(t){return console.error("[TELEGRAM WEBHOOK] Error:",t),e.json({ok:!0})}});const jt=new ge;jt.post("/market-analysis",async e=>await da(e));jt.get("/health",async e=>e.json({success:!0,status:"healthy",service:"ai-analysis",timestamp:new Date().toISOString()}));jt.get("/auto-ai-scan",async e=>await da(e));async function da(e){const{DB:t}=e.env;try{console.log("[AI-ANALYSIS] Starting comprehensive market analysis");const s=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings
      WHERE setting_key = 'twelve_data_api_key'
    `).all();let a="";for(const R of s.results||[])R.setting_key==="twelve_data_api_key"&&(a=R.setting_value);let n=[];if(a&&a!=="your_api_key_here")try{const R=`https://api.twelvedata.com/time_series?symbol=XAU/USD&interval=1h&outputsize=100&apikey=${a}`,O=await(await fetch(R)).json();O.values&&O.values.length>=50&&(n=O.values.reverse().map(W=>({timestamp:W.datetime,open:parseFloat(W.open),high:parseFloat(W.high),low:parseFloat(W.low),close:parseFloat(W.close),volume:parseFloat(W.volume)||0})),console.log("[AI-ANALYSIS] Fresh data fetched:",n.length,"candles"))}catch{console.error("[AI-ANALYSIS] API fetch failed, falling back to database")}if(n.length===0){const R=await t.prepare(`
      ...
      `).all();if(!R.results||R.results.length<50)return e.json({success:!1,error:"Not enough market data available"},400);n=R.results.reverse().map(L=>({timestamp:L.timestamp,open:L.open,high:L.high,low:L.low,close:L.close,volume:L.volume||0}))}const o=pe(n);if(!o)return e.json({success:!1,error:"Failed to calculate indicators"},400);const i=n[n.length-1].close,l=le(i,o,"day_trade");console.log("[AI-ANALYSIS] Current price:",i,"Signal:",l.signal_type,"Confidence:",l.confidence);const r={};for(const R of["5m","15m","1h","4h","daily"]){const L=await t.prepare(`
        ...
        ORDER BY timestamp DESC 
        LIMIT 1
      `).bind(R).first();L&&(r[R]=L)}const d=Jt(r,i),c=n.slice(-50),p=c.map(R=>R.high).sort((R,L)=>L-R),g=c.map(R=>R.low).sort((R,L)=>R-L),m=[Math.max(...p.slice(0,10))],u=[Math.min(...g.slice(0,10))];i>o.sma_20?u.push(o.sma_20):m.push(o.sma_20),i>o.sma_50?u.push(o.sma_50):m.push(o.sma_50),i>o.vwap?u.push(o.vwap):m.push(o.vwap);const _=Math.round(i/10)*10;_>i?m.push(_):u.push(_);const f=[...new Set(m)].sort((R,L)=>R-L).filter(R=>R>i).slice(0,3),h=[...new Set(u)].sort((R,L)=>L-R).filter(R=>R<i).slice(0,3);console.log("[AI-ANALYSIS] Key levels - Support:",h,"Resistance:",f);const y=o.atr_14/i*100;let w="NORMAL";y>3?w="EXTREME":y>1.5?w="HIGH":y<.5&&(w="LOW");const b=[];let T=30,E=30,k=40;d.type==="ALL_BULLISH"?(T=60,E=20,k=20):d.type==="ALL_BEARISH"?(T=20,E=60,k=20):d.score>=4&&(d.trends.filter(R=>R.trend==="BULLISH").length>=4?(T=50,E=25,k=25):(T=25,E=50,k=25)),f.length>0&&b.push({name:"📈 BULLISH CONTINUATION",probability:T,description:`Price breaks above $${f[0].toFixed(2)} and rallies toward $${(f[f.length-1]||i*1.02).toFixed(2)}`,trigger:`Breakout above $${f[0].toFixed(2)} with volume`,target:f[f.length-1]||i*1.02}),h.length>0&&b.push({name:"📉 BEARISH CORRECTION",probability:E,description:`Price breaks below $${h[0].toFixed(2)} and drops toward $${(h[h.length-1]||i*.98).toFixed(2)}`,trigger:`Breakdown below $${h[0].toFixed(2)} with volume`,target:h[h.length-1]||i*.98}),b.push({name:"↔️ CONTINUED RANGING",probability:k,description:`Price oscillates between $${(h[0]||i*.99).toFixed(2)} and $${(f[0]||i*1.01).toFixed(2)} with choppy action`,trigger:w==="EXTREME"?"High volatility continues":"No clear breakout/breakdown",target:null}),b.sort((R,L)=>L.probability-R.probability);let v={action:"WAIT",reason:"Market conditions unclear - wait for better setup",entry_range:null,stop_loss:null};l.confidence>=65?l.signal_type==="BUY"?v={action:"BUY",reason:`Strong ${l.signal_type} signal with ${l.confidence}% confidence. ${d.type} MTF alignment.`,entry_range:`${(i-5).toFixed(2)}-${i.toFixed(2)}`,stop_loss:l.stop_loss.toFixed(2)}:l.signal_type==="SELL"&&(v={action:"SELL",reason:`Strong ${l.signal_type} signal with ${l.confidence}% confidence. ${d.type} MTF alignment.`,entry_range:`${i.toFixed(2)}-${(i+5).toFixed(2)}`,stop_loss:l.stop_loss.toFixed(2)}):w==="EXTREME"?v.reason=`⚠️ EXTREME volatility (ADX ${o.adx.toFixed(1)}) - Too risky to trade. Wait for market to calm down.`:(d.type==="MIXED"||d.type==="CONFLICTING")&&(v.reason=`⏰ Timeframes conflicting (${d.score}/5 aligned). Wait for ${f[0]?`breakout above $${f[0].toFixed(2)}`:h[0]?`breakdown below $${h[0].toFixed(2)}`:"clearer direction"}.`);let $=!1;if(l.confidence>=65)try{const R=await t.prepare(`
          SELECT setting_key, setting_value FROM user_settings
          WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
        `).all(),L={};for(const O of R.results||[])L[O.setting_key]=O.setting_value;if(L.telegram_bot_token&&L.telegram_chat_id&&L.telegram_bot_token!=="your_bot_token_here"){const O=l.signal_type==="BUY"?"🟢":l.signal_type==="SELL"?"🔴":"⚪",W=l.confidence>=85,I=W?`🔥 *HIGH CONVICTION AI* 🔥
`:"";let C=`${O} *AI MARKET ANALYSIS* ${O}
`;C+=I,C+=`⏰ ${new Date().toLocaleString("en-US",{timeZone:"UTC"})} UTC
...
`,C+=`🔴 *Resistance:* ${f.length>0?f.map(D=>`$${D.toFixed(2)}`).join(", "):"N/A"}
`,C+=`🟢 *Support:* ${h.length>0?h.map(D=>`$${D.toFixed(2)}`).join(", "):"N/A"}

`,C+=`*Top Scenario:* ${b[0].name} (${b[0].probability}%)
`,C+=`${b[0].description}

`,C+=`💡 *Recommendation:* ${v.action==="WAIT"?"⏰":v.action==="BUY"?"📈":"📉"} ${v.action}
`,C+=`${v.reason}

`,v.entry_range&&(C+=`🎯 *Entry Range:* $${v.entry_range}
`,C+=`🛡️ *Stop Loss:* $${v.stop_loss}`),$=await q({botToken:L.telegram_bot_token,chatId:L.telegram_chat_id},C),console.log("[AI-ANALYSIS] Telegram alert sent:",$,"for",l.signal_type,l.confidence+"%"),$&&W&&(l.signal_type==="BUY"||l.signal_type==="SELL")&&(console.log("[AI-ANALYSIS] 🔥 HIGH CONVICTION AI signal! Scheduling reminders..."),setTimeout(async()=>{let D=`${O} *⚠️ REMINDER: AI HIGH CONVICTION* ${O}
...
`),D+="⏰ Don't miss this AI signal!",await q({botToken:L.telegram_bot_token,chatId:L.telegram_chat_id},D)},120*1e3),setTimeout(async()=>{let D=`${O} *⚠️ FINAL: AI SIGNAL STILL VALID* ${O}
...
⏰ ${new Date().toUTCString()}`;return await q(p,l,r),!0}}return!1}catch(i){return console.error("[MONITORING] Failed to send alert:",i),!1}}Ue.get("/health-check",async e=>{const{DB:t}=e.env;try{const s=await ma(t),a=e.req.url.replace("/api/monitoring/health-check",""),n=new Date().toISOString(),o=await ei(t);console.log("[MONITORING] Starting comprehensive health check..."),console.log("[MONITORING] Tables exist:",o);const i=[{name:"auto-fetch",url:"/api/cron/auto-fetch"},{name:"ai-analysis",url:"/api/ai/health"},{name:"scanner",url:"/api/scanner/scan"},{name:"signals-recent",url:"/api/signals/recent"},{name:"indicators-latest",url:"/api/indicators/latest"}],l=[],r=s.telegram_alerts_enabled==="1",d=parseInt(s.slow_response_threshold_ms||"5000"),c=parseInt(s.max_failure_count||"3");console.log("[MONITORING] Fast mode: Checking 5 endpoints (MTF skipped)");for(const h of i){const y=await ti(t,h.name,h.url,a);let w=0,b="unknown",T=y.status==="down"?1:0;if(o)try{const E=await t.prepare(`
        ...
        `).bind(g,m,u,_,l.reduce((h,y)=>h+y.response_time_ms,0)/l.length).run()}catch(h){console.log("[MONITORING] Could not save metrics:",h)}return console.log(`[MONITORING] Health check complete: ${f}`),console.log(`[MONITORING] Tables exist: ${o}, Alerts enabled: ${r}`),e.json({success:!0,timestamp:n,overall_status:f,summary:{endpoints:{healthy:g,degraded:m,down:u,total:l.length},data:{fresh:p.length-_,stale:_,total:p.length}},endpoints:l,data_freshness:p,config:{stale_threshold_minutes:s.data_stale_threshold_minutes,slow_response_threshold_ms:s.slow_response_threshold_ms,max_failure_count:s.max_failure_count,telegram_alerts_enabled:r}})}catch(s){return console.error("[MONITORING] Health check failed:",s),e.json({success:!1,error:s.message,timestamp:new Date().toISOString()},500)}});Ue.get("/status",async e=>{const{DB:t}=e.env;try{let s,a,n;try{s=await t.prepare(`
        SELECT 
          endpoint_name,
          status,
          response_time_ms,
      ...
      `).all()}catch{const r=new URL(e.req.url),d=`${r.protocol}//${r.host}`,c=[{name:"auto-fetch",url:"/api/cron/auto-fetch"},{name:"ai-analysis",url:"/api/ai/health"},{name:"scanner",url:"/api/scanner/scan"},{name:"signals-recent",url:"/api/signals/recent"},{name:"indicators-latest",url:"/api/indicators/latest"}];s={results:await Promise.all(c.map(async({name:g,url:m})=>{try{const u=Date.now(),_=await fetch(`${d}${m}`,{method:"GET",signal:AbortSignal.timeout(1e4)}),f=Date.now()-u;return{endpoint_name:g,status:_.ok?"healthy":"degraded",response_time_ms:f,last_check_at:new Date().toISOString()}}catch{return{endpoint_name:g,status:"down",response_time_ms:0,last_check_at:new Date().toISOString()}}}))};try{const g=await t.prepare(`
        ...
        `).all(),u=await t.prepare(`
          SELECT 
            ...
            datetime('now') as last_fetch_at
          FROM signals
        `).first();a={results:[...g?[g]:[],...m.results||[],...u?[u]:[]]}}catch(g){console.log("[MONITORING] Data freshness check error:",g.message),a={results:[]}}n={results:[]}}const o=(s.results||[]).every(l=>l.status==="healthy"),i=(a.results||[]).every(l=>l.is_stale===0);return e.json({success:!0,overall_status:o&&i?"healthy":"degraded",endpoints:s.results,data_sources:a.results,unresolved_alerts:n.results,alert_count:(n.results||[]).length})}catch(s){return e.json({success:!1,error:s.message},500)}});Ue.get("/alerts",async e=>{const{DB:t}=e.env,s=e.req.query("resolved")==="true";try{const a=await t.prepare(`
...
`,h+=`📊 Volume: ${a} (${n}%ile)
`,h+=`💰 Spread: ~${o} pips | R:R 1:${u.toFixed(1)}
...
`,h+=`Signal #${l} | ${f}`,h}gt.get("/scan",async e=>{var a;const{DB:t}=e.env,s={success:!1,message:"",data:{}};try{console.log("[MICRO] Starting 5-minute scan...");const n=await li(t);if(!n.allowed)return console.log("[MICRO] Daily limit check failed:",n.reason),s.message=n.reason,s.data={signals_sent_today:n.signals_sent_today,max_signals:n.max_signals},e.json(s);console.log(`[MICRO] Daily limit check passed: ${n.signals_sent_today}/${n.max_signals} signals sent`);const o=await t.prepare(`
      SELECT timestamp, open, high, low, close, volume 
      FROM market_data 
      WHERE timeframe = '5m'
      ORDER BY timestamp DESC 
    ...
    `).all();if(!o.results||o.results.length<20)return s.message="Insufficient 5m candle data",e.json(s);const i=o.results.map(E=>({timestamp:E.timestamp,open:E.open,high:E.high,low:E.low,close:E.close,volume:E.volume||1})).reverse(),l=await t.prepare(`
      ...
      ORDER BY timestamp DESC 
      LIMIT 60
    `).all();if(!l.results||l.results.length<50)return s.message=`Insufficient 15m candle data (have ${((a=l.results)==null?void 0:a.length)||0}, need 50)`,e.json(s);const r=l.results.map(E=>({timestamp:E.timestamp,open:E.open,high:E.high,low:E.low,close:E.close,volume:E.volume||1})).reverse(),d=pe(i),c=pe(r);if(!d)return console.error("[MICRO] Failed to calculate 5m indicators - data length:",i.length),s.message="Failed to calculate 5m indicators",s.data={candles5m_count:i.length,first_candle:i[0],last_candle:i[i.length-1]},e.json(s);if(!c)return console.error("[MICRO] Failed to calculate 15m indicators - data length:",r.length),s.message="Failed to calculate 15m indicators",s.data={candles15m_count:r.length},e.json(s);const p=i[i.length-1].close,g=pa(i,d,r,c);if(!g)return console.log("[MICRO] No valid setup detected"),s.success=!0,s.message="No signal - waiting for setup",s.data={currentPrice:p},e.json(s);if(console.log(`[MICRO] Signal detected: ${g.signal_type} ${g.setup_type} (${g.confidence}%)`),g.confidence<60)return console.log(`[MICRO] Confidence too low: ${g.confidence}%`),s.success=!0,s.message=`Signal below threshold (${g.confidence}% < 60%)`,s.data={signal:g},e.json(s);const m=Ht(i);if(m.liquidity_score<60)return console.log(`[MICRO] Liquidity too low: ${m.liquidity_score}`),s.success=!0,s.message=`Low liquidity (${m.liquidity_score} < 60)`,s.data={signal:g,liquidity:m},e.json(s);const u=ci(g,m.liquidity_score,m.optimal_for_trading),_=new Date(Date.now()+900*1e3).toISOString(),f=new Date().toISOString().split("T")[0],h=await t.prepare(`
    ...
    `).bind(g.signal_type,g.price,g.stop_loss,g.take_profit_1,g.take_profit_2,g.take_profit_3,g.confidence,g.setup_type,g.trend_5m,g.trend_15m,g.indicators_5m.rsi,g.indicators_5m.macd,g.indicators_5m.macd_signal,g.indicators_5m.macd_histogram,g.indicators_5m.adx,g.indicators_5m.stochastic_k,g.indicators_5m.stochastic_d,g.indicators_5m.ema_20,g.indicators_5m.volume,m.liquidity_score,m.session,m.time_zone,m.volume_trend,m.volume_percentile,m.estimated_spread_pips,m.price_impact_bps,m.market_depth_score,m.optimal_for_trading?1:0,u.multiplier,u.lots,u.risk_amount,u.risk_percent,_,g.reason).run(),console.log("[MICRO] Signal saved to database");const w=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings
      WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
    `).all(),b={};for(const E of w.results||[])b[E.setting_key]=E.setting_value;let T=!1;if(b.telegram_bot_token&&b.telegram_chat_id){const E=ua(g,m.liquidity_score,m.session,m.volume_trend,m.volume_percentile,m.estimated_spread_pips,u,y);T=await q({botToken:b.telegram_bot_token,chatId:b.telegram_chat_id},E),T&&(await t.prepare(`
        ...
        `).bind(f).run(),await t.prepare(`
          ...
          ON CONFLICT(date) DO UPDATE SET
            ...
            updated_at = datetime('now')
        `).bind(f,g.signal_type==="BUY"?1:0,g.signal_type==="SELL"?1:0,g.confidence,m.liquidity_score,m.session==="ASIA"?1:0,m.session==="LONDON"?1:0,m.session==="NEW_YORK"?1:0,m.session==="OVERLAP"?1:0,g.setup_type==="BREAKOUT"?1:0,g.setup_type==="CONTINUATION"?1:0,g.setup_type==="REVERSAL"?1:0,g.setup_type==="BOUNCE"?1:0,g.setup_type==="PATTERN"?1:0).run(),console.log("[MICRO] Telegram alert sent successfully"))}return s.success=!0,s.message=T?"Signal generated and sent to Telegram":"Signal generated (Telegram not configured)",s.data={signal:g,liquidity:m,position:u,signalNumber:y,limitsCheck:n,telegramSent:T},e.json(s)}catch(n){return console.error("[MICRO] Error:",n),s.message=n.message||"Unknown error",e.json(s,500)}});gt.get("/signals/recent",async e=>{const{DB:t}=e.env,s=parseInt(e.req.query("limit")||"20");try{const a=await t.prepare(`
      SELECT * FROM micro_trade_signals 
      ORDER BY created_at DESC 
      LIMIT ?
    `).bind(s).all();return e.json({success:!0,signals:a.results})}catch(a){return e.json({success:!1,error:a.message},500)}});gt.get("/stats/daily",async e=>{const{DB:t}=e.env,s=e.req.query("date")||new Date().toISOString().split("T")[0];try{const a=await t.prepare(`
      SELECT * FROM micro_trade_daily_stats WHERE date = ?
    `).bind(s).first(),n=await t.prepare(`
      SELECT * FROM micro_trade_limits WHERE date = ?
    `).bind(s).first();return e.json({success:!0,date:s,stats:a||null,limits:n||null})}catch(a){return e.json({success:!1,error:a.message},500)}});gt.get("/debug/data-check",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
    ...
    `).first(),i=await t.prepare(`
      ...
      GROUP BY timeframe
      ORDER BY timeframe
    `).all();return e.json({success:!0,data:{candles_5m:{count:(s==null?void 0:s.count)||0,latest:(a==null?void 0:a.timestamp)||null,price:(a==null?void 0:a.close)||null},candles_15m:{count:(n==null?void 0:n.count)||0,latest:(o==null?void 0:o.timestamp)||null,price:(o==null?void 0:o.close)||null},all_timeframes:i.results||[]}})}catch(s){return e.json({success:!1,error:s.message},500)}});gt.get("/test-alert",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings
      WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
...
`;const f=1e3*t.positionMultiplier,h=f/1e3*.1,y=r*.01*h*100,w=y/1e4*100,b=t.positionMultiplier>=1.5?"🟢🟢":t.positionMultiplier>=.9?"🟢":"🟡";return u+=`${b} <b>Position:</b> ${h.toFixed(2)} lots ($${f}) [${t.positionMultiplier}x]
...
`,u+=`Signal #${s} | ${g}`,u}Xe.get("/test-alert",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
    ...
    `).first(),l=((i==null?void 0:i.count)||0)+1,r=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings
      WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
    `).all(),d={};for(const g of r.results||[])d[g.setting_key]=g.setting_value;if(!d.telegram_bot_token||!d.telegram_chat_id)return e.json({success:!1,error:"Telegram not configured"});const c=_a(n,o,l,85,"NEW_YORK"),p=await q({botToken:d.telegram_bot_token,chatId:d.telegram_chat_id},c);return await t.prepare(`
      INSERT INTO micro_trade_signals (
    ...
    `).bind(n.signal_type,n.price,n.stop_loss,n.take_profit_1,n.take_profit_2,n.take_profit_3,o.confidence,n.setup_type,n.trend_5m,n.trend_15m,n.indicators_5m.rsi,n.indicators_5m.macd,n.indicators_5m.macd_signal,n.indicators_5m.adx,n.indicators_5m.stochastic_k,n.reason,o.grade,o.filtersPassed,o.positionMultiplier,p?1:0).run(),e.json({success:!0,message:"Test Grade A hybrid alert sent to Telegram and stored in database!",signal:{grade:o.grade,filters_passed:o.filtersPassed,confidence:o.confidence,position_multiplier:o.positionMultiplier,signal_type:n.signal_type,entry:n.price,stop_loss:n.stop_loss,telegram_sent:p,signal_number:l}})}catch(s){return e.json({success:!1,error:s.message})}});Xe.get("/scan",async e=>fa(e));Xe.post("/scan",async e=>fa(e));async function fa(e){var a,n,o,i,l;const{DB:t}=e.env,s=Date.now();try{console.log("[HYBRID-MICRO] Starting hybrid scan...");const r={};for(const T of["5m","15m","1h","4h"]){const E=await t.prepare(`
        SELECT timestamp, open, high, low, close, volume 
        FROM market_data 
        WHERE timeframe = ?
        ORDER BY timestamp DESC 
        LIMIT 100
      `).bind(T).all();if(E.results&&E.results.length>=20){const k=E.results.map($=>({timestamp:$.timestamp,open:$.open,high:$.high,low:$.low,close:$.close,volume:$.volume||1})).reverse(),v=await pe(k);if(!v){console.log(`[HYBRID-MICRO] Skipping ${T} - insufficient data for indicators`);continue}r[T]={candles:k,indicators:v,trend:v.ema_20>v.ema_50?"BULLISH":"BEARISH"}}}if(!r["5m"]||!r["15m"])return e.json({success:!1,error:"Insufficient data for hybrid analysis"});const d=await pa(r["5m"].candles,r["5m"].indicators,r["15m"].candles,r["15m"].indicators);if(!d)return e.json({success:!1,message:"No micro trade setup detected"});const c=r["5m"].candles[r["5m"].candles.length-1].close,p=r["5m"].indicators;if(!p||!p.rsi_14)return e.json({success:!1,error:"Technical indicators not available",debug:{hasIndicators:!!p,indicatorKeys:p?Object.keys(p).slice(0,5):[]}});const g=r["5m"].candles.slice(-100).map((T,E,k)=>{if(E<14)return 0;const v=k.slice(Math.max(0,E-14),E),$=v.map((R,L)=>{if(L===0)return R.high-R.low;const O=v[L-1].close;return Math.max(R.high-R.low,Math.abs(R.high-O),Math.abs(R.low-O))});return $.reduce((R,L)=>R+L,0)/$.length}).filter(T=>T>0),m=await ga(d.signal_type,c,r["5m"].candles,p,r,{atrHistory:g});if(console.log(`[HYBRID-MICRO] Signal graded: ${m.grade} (${m.filtersPassed}/10 filters)`),m.grade==="REJECT")return e.json({success:!1,message:`Signal rejected (only ${m.filtersPassed}/10 filters passed)`,grade:m.grade,filters_passed:m.filtersPassed});const u=await t.prepare(`
      SELECT COUNT(*) as count FROM micro_trade_signals 
      WHERE DATE(created_at) = DATE('now')
    `).first(),_=((u==null?void 0:u.count)||0)+1,f={...d,confidence:m.confidence};await t.prepare(`
      INSERT INTO micro_trade_signals (
        signal_type, price, stop_loss, 
        take_profit_1, take_profit_2, take_profit_3,
        confidence, setup_type, trend_5m, trend_15m,
    ...
    `).bind(f.signal_type,f.price,f.stop_loss,f.take_profit_1,f.take_profit_2,f.take_profit_3,m.confidence,f.setup_type,f.trend_5m,f.trend_15m,((a=f.indicators_5m)==null?void 0:a.rsi)||50,((n=f.indicators_5m)==null?void 0:n.macd)||0,((o=f.indicators_5m)==null?void 0:o.macd_signal)||0,((i=f.indicators_5m)==null?void 0:i.adx)||0,((l=f.indicators_5m)==null?void 0:l.stochastic_k)||50,f.reason,m.grade,m.filtersPassed,m.positionMultiplier).run();const h=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings
      WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
    `).all(),y={};for(const T of h.results||[])y[T.setting_key]=T.setting_value;let w=!1;if(y.telegram_bot_token&&y.telegram_chat_id&&m.confidence>=60){const T=_a(f,m,_,85,"NEW_YORK");w=await q({botToken:y.telegram_bot_token,chatId:y.telegram_chat_id},T),w&&await t.prepare(`
          UPDATE micro_trade_signals 
          SET telegram_sent = 1
          WHERE id = (SELECT MAX(id) FROM micro_trade_signals)
        `).run()}const b=m.confidence>=60?w?"sent to Telegram":"saved (Telegram failed)":"saved (confidence < 60%, no alert)";return e.json({success:!0,message:`${m.grade}-grade signal generated and ${b}`,signal:{...f,grade:m.grade,filters_passed:m.filtersPassed,confidence:m.confidence,position_multiplier:m.positionMultiplier},telegram_sent:w,telegram_eligible:m.confidence>=60,execution_time_ms:Date.now()-s})}catch(r){return console.error("[HYBRID-MICRO] Error:",r),e.json({success:!1,error:r.message},500)}}Xe.get("/signals/recent",async e=>{var a;const{DB:t}=e.env,s=parseInt(e.req.query("limit")||"10");try{const n=await t.prepare(`
      SELECT 
        id,
        signal_type,
        price,
    ...
    `).bind(s).first();return e.json({success:!0,date:s,total:(a==null?void 0:a.total)||0,telegram_sent:(n==null?void 0:n.sent)||0})}catch(a){return console.error("[HYBRID-MICRO] Error fetching today stats:",a),e.json({success:!1,error:a.message},500)}});Xe.get("/test",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
      ...
      ORDER BY timestamp DESC 
      LIMIT 50
    `).all();if(!s.results||s.results.length<20)return e.json({success:!1,error:"Insufficient data for testing"});const a=s.results.map(l=>({timestamp:l.timestamp,open:l.open,high:l.high,low:l.low,close:l.close,volume:l.volume||1})).reverse(),n=await pe(a),o=a[a.length-1].close,i=await ga("BUY",o,a,n,{"5m":{trend:"BULLISH",indicators:n}});return e.json({success:!0,test_results:{grade:i.grade,filters_passed:i.filtersPassed,confidence:i.confidence,position_multiplier:i.positionMultiplier,filter_details:i.filterResults},current_price:o,indicators:{rsi:n.rsi,macd:n.macd,adx:n.adx}})}catch(s){return e.json({success:!1,error:s.message},500)}});const j=new ge;j.use("/api/*",cn());j.route("/api/signals/enhanced",zs);j.route("/api/signals/simple",Xs);j.route("/api/scanner",dt);j.route("/api/trades",we);j.route("/api/calendar",Fe);j.route("/api/backtest",ut);j.route("/api/telegram",ca);j.route("/api/ai",jt);j.route("/api/monitoring",Ue);j.route("/api/micro",gt);j.route("/api/hybrid-micro",Xe);j.get("/",e=>e.html(`
    <!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
...
';
  ...
  `));j.get("/api/signals/recent",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
    ...
    `).first(),a=await t.prepare(`
      ...
      LIMIT 10
    `).all();return e.json({success:!0,sentiment:s||{overall_sentiment:"neutral",sentiment_score:0},articles:a.results||[]})}catch(s){return e.json({success:!1,error:s.message},500)}});j.get("/api/news/events",async e=>{try{const{getEconomicEvents:t}=await Promise.resolve().then(()=>Ea),s=await t();return e.json({success:!0,events:s})}catch(t){return e.json({success:!1,error:t.message},500)}});j.get("/api/market/fetch",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
      SELECT setting_value FROM user_settings 
      WHERE setting_key = 'twelve_data_api_key'
    `).first(),i=`https://api.twelvedata.com/time_series?symbol=XAU/USD&interval=1h&apikey=${(s==null?void 0:s.setting_value)||"70140f57bea54c5e90768de696487d8f"}&outputsize=100`,r=await(await fetch(i)).json();if(r.code&&r.status==="error")return e.json({success:!1,error:r.message||"Twelve Data API error",count:0});if(!r.values||!Array.isArray(r.values))return e.json({success:!1,error:"No data available from Twelve Data API",count:0});const d=r.values,c=d.map(u=>t.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).bind(u.datetime,parseFloat(u.open)||0,parseFloat(u.high)||0,parseFloat(u.low)||0,parseFloat(u.close)||0,parseInt(u.volume||"0")||0,"1h"));await t.batch(c);const p=d.length,g=d[0],m=parseFloat(g.close)||0;return e.json({success:!0,count:p,price:m,message:"Data fetched successfully from cron job"})}catch(s){return console.error("Cron fetch error:",s),e.json({success:!1,error:s.message,count:0},500)}});j.post("/api/market/fetch",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
      SELECT setting_value FROM user_settings 
      WHERE setting_key = 'twelve_data_api_key'
    `).first();let a=(s==null?void 0:s.setting_value)||"70140f57bea54c5e90768de696487d8f";if(!a||a==="your_key_here"||a==="")return e.json({success:!1,error:"Twelve Data API key not configured. Please add it in settings.",count:0});const i=`https://api.twelvedata.com/time_series?symbol=XAU/USD&interval=1h&apikey=${a}&outputsize=100`,r=await(await fetch(i)).json();if(r.code&&r.status==="error")return e.json({success:!1,error:r.message||"Twelve Data API error",count:0});if(!r.values||!Array.isArray(r.values))return e.json({success:!1,error:"No data available from Twelve Data API",count:0});const c=r.values.map(m=>({timestamp:m.datetime,open:parseFloat(m.open)||0,high:parseFloat(m.high)||0,low:parseFloat(m.low)||0,close:parseFloat(m.close)||0,volume:0})),p=c.map(m=>t.prepare(`
      ...
      `).bind(m.timestamp,m.open,m.high,m.low,m.close,m.volume));await t.batch(p);const g=c.length;if(c.length>=50){const m=pe(c.reverse());if(m){await t.prepare(`
        ...
        `).bind(m.rsi_14,m.macd,m.macd_signal,m.macd_histogram,m.sma_20,m.sma_50,m.sma_200,m.ema_12,m.ema_26,m.bb_upper,m.bb_middle,m.bb_lower,m.atr_14,m.stochastic_k,m.stochastic_d,m.adx,m.plus_di,m.minus_di,m.ichimoku_tenkan,m.ichimoku_kijun,m.ichimoku_senkou_a,m.ichimoku_senkou_b,m.parabolic_sar,m.vwap,m.fib_382||0,m.fib_500||0,m.fib_618||0).run();const u=c[c.length-1].close,_=le(u,m,"day_trade"),f=le(u,m,"swing_trade"),h=70;for(const y of[_,f])if(y.confidence>=h&&y.signal_type!=="HOLD"){await t.prepare(`
            ...
            `).bind(y.signal_type,y.trading_style,y.price,y.stop_loss,y.take_profit_1,y.take_profit_2,y.take_profit_3,y.confidence,y.reason).run();const w=await t.prepare(`
              SELECT setting_key, setting_value FROM user_settings
              WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
            `).all(),b={};for(const T of w.results||[])b[T.setting_key]=T.setting_value;b.telegram_bot_token&&b.telegram_chat_id&&await q({botToken:b.telegram_bot_token,chatId:b.telegram_chat_id},ct(y))}}}return e.json({success:!0,count:g})}catch(s){return e.json({success:!1,error:s.message},500)}});j.get("/api/cron/ping",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
      SELECT close, timestamp FROM market_data 
      WHERE timeframe = '1h' 
    ...
    `).all(),a={};for(const v of s.results)a[v.setting_key]=v.setting_value;const n=a.twelve_data_api_key||"70140f57bea54c5e90768de696487d8f",o=a.telegram_bot_token,i=a.telegram_chat_id;console.log("[AUTO-FETCH] Settings loaded:",{hasApiKey:!!n,hasBotToken:!!o,botTokenLength:o?o.length:0,hasChatId:!!i,chatId:i});const d=`https://api.twelvedata.com/time_series?symbol=XAU/USD&interval=1h&apikey=${n}&outputsize=100`,p=await(await fetch(d)).json();if(p.code&&p.status==="error")return e.json({success:!1,error:p.message||"API error",telegram_sent:!1});if(!p.values||!Array.isArray(p.values))return e.json({success:!1,error:"No data available",telegram_sent:!1});const m=p.values.map(v=>({timestamp:v.datetime,open:parseFloat(v.open)||0,high:parseFloat(v.high)||0,low:parseFloat(v.low)||0,close:parseFloat(v.close)||0,volume:parseInt(v.volume||"0")||0})),u=m.map(v=>t.prepare(`
        INSERT OR REPLACE INTO market_data (timestamp, open, high, low, close, volume, timeframe)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).bind(v.timestamp,v.open,v.high,v.low,v.close,v.volume,"1h"));await t.batch(u);const _=pe(m);if(!_)return e.json({success:!0,count:m.length,message:"Data stored, but insufficient for indicators",telegram_sent:!1});let f=m[0].close,h=!1;try{console.log("[AUTO-FETCH] Fetching real-time price...");const $=await(await fetch(`https://api.twelvedata.com/price?symbol=XAU/USD&apikey=${n}`,{signal:AbortSignal.timeout(5e3)})).json();if($.price){const R=parseFloat($.price),L=f,W=Math.abs(R-L)/R*100;console.log(`[AUTO-FETCH] Real-time: $${R}, Last candle: $${L}, Diff: ${W.toFixed(2)}%`),W<2?(f=R,h=!0,console.log(`[AUTO-FETCH] ✅ Using real-time price: $${R}`)):console.log(`[AUTO-FETCH] ⚠️ Price diff too large (${W.toFixed(2)}%), using candle close`)}}catch(v){console.log("[AUTO-FETCH] Real-time price fetch failed, using candle close:",v.message)}const y=le(f,_,"day_trade"),w=le(f,_,"swing_trade");try{await t.prepare(`
        INSERT INTO signals 
        (timestamp, signal_type, trading_style, price, stop_loss, 
         take_profit_1, take_profit_2, take_profit_3, confidence, reason)
        VALUES (datetime('now'), ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
⏰ ${new Date().toLocaleString()}`,W=await q({botToken:o,chatId:i},O);k.day_trade_send_result=W,console.log("[AUTO-FETCH] Day trade alert result:",W),W?(T=!0,E.push("Day Trade"),R&&y.signal_type!=="HOLD"&&(console.log("[AUTO-FETCH] 🔥 HIGH CONVICTION signal detected! Sending reminder alerts..."),setTimeout(async()=>{const I=`${$} <b>⚠️ REMINDER: HIGH CONVICTION SIGNAL</b> ${$}
...
⏰ ${new Date().toLocaleString()}`,W=await q({botToken:o,chatId:i},O);k.swing_trade_send_result=W,W&&(T=!0,E.push("Swing Trade"),R&&w.signal_type!=="HOLD"&&(console.log("[AUTO-FETCH] 🔥 HIGH CONVICTION swing signal! Sending reminder alerts..."),setTimeout(async()=>{const I=`${$} <b>⚠️ REMINDER: HIGH CONVICTION SWING</b> ${$}
...
🎯 TP1: $${w.take_profit_1.toFixed(2)}
🛡️ Stop: $${w.stop_loss.toFixed(2)}`;await q({botToken:o,chatId:i},I),console.log("[AUTO-FETCH] Swing final reminder sent")},420*1e3),E.push("High Conviction Swing Reminders (3+7min)")))}}else console.log("[AUTO-FETCH] Telegram NOT configured or invalid token");return console.log(`[CRON] Processed ${m.length} candles, Telegram: ${T?"SENT":"NOT SENT"}, Alerts: ${E.join(", ")}`),e.json({success:!0,timestamp:new Date().toISOString(),data_fetched:{candles:m.length,latest_price:f,data_timestamp:m[0].timestamp},signals:{day_trade:{type:y.signal_type,confidence:y.confidence,price:f},swing_trade:{type:w.signal_type,confidence:w.confidence,price:f}},telegram:{configured:!!(o&&i),bot_token_set:!!o,chat_id_set:!!i,bot_token_valid:o!=="your_bot_token_here",sent:T,alerts:E},debug:{...k,day_trade_check:{confidence:y.confidence,min_confidence:b,meets_threshold:y.confidence>=b,signal_type:y.signal_type,sends_all_signals:!0,should_alert:y.confidence>=b},swing_trade_check:{confidence:w.confidence,min_confidence:80,meets_threshold:w.confidence>=80,signal_type:w.signal_type,sends_all_signals:!0,should_alert:w.confidence>=80}},message:T?`✅ Alerts sent: ${E.join(", ")}`:"⚪ No alerts sent (signals below confidence threshold)"})}catch(s){return console.error("[CRON] Error:",s),e.json({success:!1,error:s.message,telegram_sent:!1},500)}});j.get("/api/test/auto-fetch-settings",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings 
      WHERE setting_key IN ('twelve_data_api_key', 'telegram_bot_token', 'telegram_chat_id')
    `).all(),a={};for(const l of s.results)a[l.setting_key]=l.setting_value;const n=a.twelve_data_api_key||"70140f57bea54c5e90768de696487d8f",o=a.telegram_bot_token,i=a.telegram_chat_id;return e.json({success:!0,raw_results:s.results,config_object:a,extracted:{apiKey:n?`${n.substring(0,10)}...`:null,telegramBotToken:o?`${o.substring(0,10)}...`:null,telegramChatId:i,is_configured:!!(o&&i&&o!=="your_bot_token_here")}})}catch(s){return e.json({success:!1,error:s.message},500)}});j.get("/api/cron/auto-ai-scan",async e=>{var s,a,n;const{DB:t}=e.env;try{console.log("[AI-AUTO-SCAN] Starting automatic AI market analysis");const o=await t.prepare(`
      SELECT setting_value FROM user_settings 
      WHERE setting_key = 'ai_auto_scan_enabled'
    `).first();if(!((o==null?void 0:o.setting_value)==="1"||(o==null?void 0:o.setting_value)==="true"))return console.log("[AI-AUTO-SCAN] Disabled in settings"),e.json({success:!0,message:"AI auto-scan is disabled",ai_scan_enabled:!1});const l=await((a=(s=e.env.app)==null?void 0:s.fetch)==null?void 0:a.call(s,new Request(new URL("/api/ai/market-analysis",e.req.url).toString(),{method:"POST"})));if(l){const r=await l.json();return console.log("[AI-AUTO-SCAN] Analysis complete:",r.success?"Success":"Failed"),e.json({success:!0,ai_scan_enabled:!0,analysis:r,message:(n=r.analysis)!=null&&n.telegram_sent?"🤖 AI analysis complete - Telegram alert sent":"🤖 AI analysis complete - No alert (confidence < 65% or HOLD)"})}return e.json({success:!1,error:"Failed to run AI analysis"},500)}catch(o){return console.error("[AI-AUTO-SCAN] Error:",o),e.json({success:!1,error:o.message},500)}});j.get("/api/cron/hedge-fund",async e=>{var s,a,n,o,i,l,r,d,c,p;const t=Date.now();try{console.log("[HEDGE-FUND-CRON] Starting hedge fund analysis");const g=await fetch(`${e.req.url.replace("/api/cron/hedge-fund","/api/signals/enhanced/enhanced")}`,{method:"POST",headers:{"Content-Type":"application/json"}});if(!g.ok)throw new Error(`Enhanced endpoint returned ${g.status}`);const m=await g.json();if(!m.success)return e.json({success:!1,error:m.error||"Enhanced signal generation failed",execution_time_ms:Date.now()-t});const u=m.day_trade,_=m.swing_trade,f=(u==null?void 0:u.enhanced_confidence)>=80||(_==null?void 0:_.enhanced_confidence)>=80;console.log("[HEDGE-FUND-CRON] Signal confidence:",{day:u==null?void 0:u.enhanced_confidence,swing:_==null?void 0:_.enhanced_confidence,shouldAlert:f});let h=!1;if(f){const{DB:w}=e.env,b=await w.prepare(`
        SELECT setting_key, setting_value FROM user_settings
        WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
      `).all(),T={};for(const E of b.results||[])T[E.setting_key]=E.setting_value;if(T.telegram_bot_token&&T.telegram_chat_id){const E=`
...
• Drawdown: ${((a=u.current_drawdown_pct)==null?void 0:a.toFixed(1))||0}%
• Portfolio Heat: ${((n=u.portfolio_heat_pct)==null?void 0:n.toFixed(1))||0}%
...
<b>Entry:</b> $${_.price.toFixed(2)}
<b>Stop Loss:</b> $${_.stop_loss.toFixed(2)}
<b>TP1:</b> $${_.take_profit_1.toFixed(2)}
<b>TP2:</b> $${_.take_profit_2.toFixed(2)}
<b>TP3:</b> $${_.take_profit_3.toFixed(2)}

📊 <b>Risk Metrics:</b>
• VaR(99%): $${((d=_.var_99)==null?void 0:d.toFixed(2))||0}
• Max Drawdown: ${((c=_.current_drawdown_pct)==null?void 0:c.toFixed(1))||0}%
...
🌐 Dashboard: ${e.req.url.replace("/api/cron/hedge-fund","")}
        `.trim(),{sendTelegramMessage:k}=await Promise.resolve().then(()=>Gs);h=await k({botToken:T.telegram_bot_token,chatId:T.telegram_chat_id},E),console.log("[HEDGE-FUND-CRON] Telegram alert sent:",h)}}const y=Date.now()-t;return e.json({success:!0,message:f?`Hedge fund signal generated and ${h?"sent":"failed to send"} to Telegram`:"Signal confidence below 80% threshold - no alert sent",confidence:{day_trade:(u==null?void 0:u.enhanced_confidence)||0,swing_trade:(_==null?void 0:_.enhanced_confidence)||0},telegram_sent:h,threshold:80,execution_time_ms:y,timestamp:new Date().toISOString()})}catch(g){return console.error("[HEDGE-FUND-CRON] Error:",g),e.json({success:!1,error:g.message,execution_time_ms:Date.now()-t},500)}});j.get("/api/cron/micro-trade",async e=>{const t=Date.now();try{console.log("[MICRO-CRON] Starting micro trade scan");const s=await fetch(`${e.req.url.replace("/api/cron/micro-trade","/api/micro/scan")}`,{method:"GET"});if(!s.ok)throw new Error(`Micro scanner returned ${s.status}`);const a=await s.json(),n=Date.now()-t;return e.json({...a,execution_time_ms:n,timestamp:new Date().toISOString()})}catch(s){return console.error("[MICRO-CRON] Error:",s),e.json({success:!1,error:s.message,execution_time_ms:Date.now()-t},500)}});j.get("/api/cron/fetch-mtf",async e=>{try{const s=`${new URL(e.req.url).origin}/api/market/fetch-mtf`;return fetch(s,{method:"POST",headers:{"Content-Type":"application/json"}}).catch(a=>console.error("[CRON-MTF] Background fetch error:",a)),e.json({success:!0,message:"MTF data fetch triggered successfully",timestamp:new Date().toISOString(),note:"Processing in background (~20 seconds)"})}catch(t){return e.json({success:!1,error:t.message},500)}});j.post("/api/market/fetch-mtf",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
      SELECT setting_value FROM user_settings 
      WHERE setting_key = 'twelve_data_api_key'
    `).first();let a=(s==null?void 0:s.setting_value)||"70140f57bea54c5e90768de696487d8f";if(!a||a==="your_key_here"||a==="")return e.json({success:!1,error:"Twelve Data API key not configured.",count:0});const n="XAU/USD",o=[{interval:"5min",dbKey:"5m",outputsize:100},{interval:"15min",dbKey:"15m",outputsize:100},{interval:"1h",dbKey:"1h",outputsize:100},{interval:"4h",dbKey:"4h",outputsize:100},{interval:"1day",dbKey:"daily",outputsize:100}];let i=0;const l={};for(const r of o){const d=`https://api.twelvedata.com/time_series?symbol=${n}&interval=${r.interval}&apikey=${a}&outputsize=${r.outputsize}`,p=await(await fetch(d)).json();if(p.code&&p.status==="error"){l[r.dbKey]={success:!1,error:p.message,count:0};continue}if(!p.values||!Array.isArray(p.values)){l[r.dbKey]={success:!1,error:"No data",count:0};continue}const g=p.values;let m=0;const u=[];for(const _ of g){const f={timestamp:_.datetime,open:parseFloat(_.open)||0,high:parseFloat(_.high)||0,low:parseFloat(_.low)||0,close:parseFloat(_.close)||0,volume:0};u.push(f),await t.prepare(`
          INSERT INTO market_data (timestamp, open, high, low, close, volume, timeframe)
          VALUES (?, ?, ?, ?, ?, ?, ?)
          ON CONFLICT DO NOTHING
        `).bind(f.timestamp,f.open,f.high,f.low,f.close,f.volume,r.dbKey).run(),m++}if(u.length>=50){const _=pe(u.reverse());_&&await t.prepare(`
            ...
            (timestamp, timeframe, rsi_14, macd, macd_signal, macd_histogram,
             ...
             parabolic_sar, vwap, fib_382, fib_500, fib_618)
            VALUES (datetime('now'), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `).bind(r.dbKey,_.rsi_14,_.macd,_.macd_signal,_.macd_histogram,_.sma_20,_.sma_50,_.sma_200,_.ema_12,_.ema_26,_.bb_upper,_.bb_middle,_.bb_lower,_.atr_14,_.stochastic_k,_.stochastic_d,_.adx,_.plus_di,_.minus_di,_.ichimoku_tenkan,_.ichimoku_kijun,_.ichimoku_senkou_a,_.ichimoku_senkou_b,_.parabolic_sar,_.vwap,_.fib_382,_.fib_500,_.fib_618).run()}l[r.dbKey]={success:!0,count:m},i+=m,await new Promise(_=>setTimeout(_,500))}return e.json({success:!0,totalCount:i,timeframes:l,message:"Multi-timeframe data fetched successfully"})}catch(s){return e.json({success:!1,error:s.message},500)}});j.post("/api/signals/generate",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
      SELECT * FROM market_data 
      WHERE timeframe = '1h'
      ORDER BY timestamp DESC 
      LIMIT 200
    `).all();if(!s.results||s.results.length<50)return e.json({success:!1,error:"Not enough data. Please fetch market data first (need at least 50 candles)."});const a=s.results.reverse().map(r=>({timestamp:r.timestamp,open:r.open,high:r.high,low:r.low,close:r.close,volume:r.volume})),n=pe(a);if(!n)return e.json({success:!1,error:"Failed to calculate indicators"});const o=a[a.length-1].close,i=le(o,n,"day_trade"),l=le(o,n,"swing_trade");return e.json({success:!0,signals:{day_trade:i,swing_trade:l}})}catch(s){return e.json({success:!1,error:s.message},500)}});j.post("/api/signals/generate-mtf",async e=>{const{DB:t}=e.env;try{const{analyzeTimeframeAlignment:s,validateMultiTimeframeSignal:a,formatAlignmentReport:n}=await Promise.resolve().then(()=>es),o=["5m","15m","1h","4h","daily"],i={};for(const E of o){const k=await t.prepare(`
        SELECT * FROM multi_timeframe_indicators 
        WHERE timeframe = ?
        ORDER BY timestamp DESC 
        LIMIT 1
      ...
      ORDER BY timestamp DESC 
      LIMIT 1
    `).first();if(!r)return e.json({success:!1,error:"No market data available"});const d=r.close,c=s(i,d),p=i["1h"],g=le(d,p,"day_trade"),m=le(d,p,"swing_trade"),u=a(g.signal_type,c),_=a(m.signal_type,c),f={...g,base_confidence:g.confidence,mtf_confidence:u.confidence,final_confidence:Math.min(95,u.confidence),isValid:u.isValid,mtf_reason:u.reason,alignment_score:c.score,alignment_type:c.type,reason:`${g.reason}, MTF: ${u.reason}`},h={...m,base_confidence:m.confidence,mtf_confidence:_.confidence,final_confidence:Math.min(95,_.confidence),isValid:_.isValid,mtf_reason:_.reason,alignment_score:c.score,alignment_type:c.type,reason:`${m.reason}, MTF: ${_.reason}`},y=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings
      WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
    `).all(),w={};for(const E of y.results||[])w[E.setting_key]=E.setting_value;let b=!1,T=[];w.telegram_bot_token&&w.telegram_chat_id&&(f.isValid&&f.signal_type!=="HOLD"&&await q({botToken:w.telegram_bot_token,chatId:w.telegram_chat_id},`🎯 MULTI-TIMEFRAME CONFIRMED\\n\\n${ct({...f,timestamp:new Date().toISOString()})}\\n\\n📊 ${n(c)}`)&&(T.push("day_trade"),b=!0),await new Promise(E=>setTimeout(E,1e3)),h.isValid&&h.signal_type!=="HOLD"&&await q({botToken:w.telegram_bot_token,chatId:w.telegram_chat_id},`🎯 MULTI-TIMEFRAME CONFIRMED\\n\\n${ct({...h,timestamp:new Date().toISOString()})}\\n\\n📊 ${n(c)}`)&&(T.push("swing_trade"),b=!0));for(const E of[f,h])E.isValid&&await t.prepare(`
          ...
          VALUES (datetime('now'), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).bind(E.signal_type,E.trading_style,E.price,E.stop_loss,E.take_profit_1,E.take_profit_2,E.take_profit_3,E.base_confidence,E.mtf_confidence,E.final_confidence,E.alignment_score,E.alignment_type,E.reason,b?1:0).run();return e.json({success:!0,signals:{day_trade:f,swing_trade:h},alignment:c,alignment_report:n(c),telegram_sent:b,sent_to_telegram:T,available_timeframes:Object.keys(i)})}catch(s){return e.json({success:!1,error:s.message,stack:s.stack},500)}});j.post("/api/signals/generate-now",async e=>{const{DB:t}=e.env;try{console.log("[GENERATE-NOW] Step 1: Fetching FRESH data from Twelve Data API");const s=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings
      WHERE setting_key IN ('twelve_data_api_key')
    `).all();let a="";for(const b of s.results||[])b.setting_key==="twelve_data_api_key"&&(a=b.setting_value);let n,o=!1;if(a&&a!=="your_api_key_here"){console.log("[GENERATE-NOW] Fetching FRESH data from Twelve Data API");try{const b=`https://api.twelvedata.com/time_series?symbol=XAU/USD&interval=1h&outputsize=100&apikey=${a}`,E=await(await fetch(b)).json();E.values&&E.values.length>=50?(n=E.values.reverse().map(k=>({timestamp:k.datetime,open:parseFloat(k.open)||0,high:parseFloat(k.high)||0,low:parseFloat(k.low)||0,close:parseFloat(k.close)||0,volume:parseFloat(k.volume)||0})),o=!0,console.log("[GENERATE-NOW] ✅ Fresh data fetched! Price:",n[n.length-1].close)):console.log("[GENERATE-NOW] ⚠️ API returned insufficient data, falling back to database")}catch(b){console.error("[GENERATE-NOW] API fetch failed:",b.message)}}if(!n||n.length===0){console.log("[GENERATE-NOW] Using database data (may be stale)");const b=await t.prepare(`
        SELECT * FROM market_data 
        WHERE timeframe = '1h'
        ORDER BY timestamp DESC 
        LIMIT 200
      `).all();if(!b.results||b.results.length<50)return e.json({success:!1,error:"Not enough data. Please configure Twelve Data API key or fetch market data first."});n=b.results.reverse().map(T=>({timestamp:T.timestamp,open:T.open,high:T.high,low:T.low,close:T.close,volume:T.volume}))}const i=pe(n);if(!i)return e.json({success:!1,error:"Failed to calculate indicators"});let l=n[n.length-1].close,r=!1;if(a&&a!=="your_api_key_here")try{console.log("[GENERATE-NOW] Fetching real-time price...");const T=await(await fetch(`https://api.twelvedata.com/price?symbol=XAU/USD&apikey=${a}`,{signal:AbortSignal.timeout(5e3)})).json();if(T.price){const E=parseFloat(T.price),k=l,$=Math.abs(E-k)/E*100;console.log(`[GENERATE-NOW] Real-time: $${E}, Last candle: $${k}, Diff: ${$.toFixed(2)}%`),l=E,r=!0}}catch(b){console.log("[GENERATE-NOW] Real-time price fetch failed, using candle close:",b.message)}const d=le(l,i,"day_trade"),c=le(l,i,"swing_trade");console.log("[GENERATE-NOW] Signals generated - Day:",d.signal_type,"Swing:",c.signal_type);const p=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings
      WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
    `).all(),g={};for(const b of p.results||[])g[b.setting_key]=b.setting_value;let m=!1,u=[];const _=n.slice(-20),f=_.map(b=>b.high).sort((b,T)=>T-b),h=_.map(b=>b.low).sort((b,T)=>b-T),y=f.length>=3?[f[0],f[1],f[2]]:f.length>=1?[f[0]]:[],w=h.length>=3?[h[0],h[1],h[2]]:h.length>=1?[h[0]]:[];console.log("[GENERATE-NOW] S/R calculated - Resistance:",y,"Support:",w),g.telegram_bot_token&&g.telegram_chat_id&&(await q({botToken:g.telegram_bot_token,chatId:g.telegram_chat_id},ct({...d,timestamp:new Date().toISOString(),resistance:y,support:w}))&&(u.push("day_trade"),m=!0),await new Promise(E=>setTimeout(E,1e3)),await q({botToken:g.telegram_bot_token,chatId:g.telegram_chat_id},ct({...c,timestamp:new Date().toISOString(),resistance:y,support:w}))&&(u.push("swing_trade"),m=!0));for(const b of[d,c])await t.prepare(`
        INSERT INTO signals 
        (timestamp, signal_type, trading_style, price, stop_loss, 
         take_profit_1, take_profit_2, take_profit_3, confidence, reason, telegram_sent)
        VALUES (datetime('now'), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).bind(b.signal_type,b.trading_style,b.price,b.stop_loss,b.take_profit_1,b.take_profit_2,b.take_profit_3,b.confidence,b.reason,m?1:0).run();return e.json({success:!0,signals:{day_trade:d,swing_trade:c},telegram_sent:m,sent_to_telegram:u})}catch(s){return e.json({success:!1,error:s.message},500)}});j.get("/api/trading/account/:id",async e=>{const{DB:t}=e.env,s=e.req.param("id");try{const a=await t.prepare(`
      SELECT * FROM trading_accounts WHERE id = ?
    `).bind(s).first();return a?e.json({success:!0,account:a}):e.json({success:!1,error:"Account not found"},404)}catch(a){return e.json({success:!1,error:a.message},500)}});j.post("/api/trading/calculate-position",async e=>{const{DB:t}=e.env;try{const s=await e.req.json(),{account_id:a,signal:n}=s,o=await t.prepare(`
      SELECT * FROM trading_accounts WHERE id = ?
    `).bind(a).first();if(!o)return e.json({success:!1,error:"Account not found"},404);const i=await t.prepare(`
      SELECT * FROM position_sizing_rules 
      WHERE account_id = ? AND is_active = 1
      ORDER BY confidence_min DESC
    `).bind(a).all(),{calculatePositionSize:l,formatPositionSize:r}=await Promise.resolve().then(()=>pt),d=l(o,n,i.results);return e.json({success:!0,position:d,formatted:r(d)})}catch(s){return e.json({success:!1,error:s.message},500)}});j.post("/api/trading/execute",async e=>{const{DB:t}=e.env;try{const s=await e.req.json(),{account_id:a,signal_id:n,entry_price:o,stop_loss:i,take_profit_1:l,take_profit_2:r,take_profit_3:d,position_size:c,signal_type:p,trading_style:g,confidence:m}=s,u=await t.prepare(`
      SELECT * FROM trading_accounts WHERE id = ?
    `).bind(a).first();if(!u)return e.json({success:!1,error:"Account not found"},404);const _=new Date().toISOString().split("T")[0],f=await t.prepare(`
      SELECT profit_loss FROM trades 
      WHERE account_id = ? 
      AND date(entry_time) = ?
      AND status = 'CLOSED'
    `).bind(a,_).all(),{checkDailyLossLimit:h}=await Promise.resolve().then(()=>pt),y=h(u,f.results);if(y.limit_exceeded)return e.json({success:!1,error:`Daily loss limit exceeded: ${y.current_loss_pct}% (max ${u.max_daily_loss_pct}%)`},400);const w=c*o,b=await t.prepare(`
      INSERT INTO trades (
    ...
    `).bind(a,n||null,p,g,o,c,w,i,l,r,d,m).run();return e.json({success:!0,trade_id:b.meta.last_row_id,message:"Trade executed successfully"})}catch(s){return e.json({success:!1,error:s.message},500)}});j.post("/api/trading/close/:trade_id",async e=>{const{DB:t}=e.env,s=e.req.param("trade_id");try{const a=await e.req.json(),{exit_price:n,exit_reason:o}=a,i=await t.prepare(`
    ...
    `).bind(n,o,r.profit_loss,r.profit_loss_pct,r.pips,s).run(),await t.prepare(`
      ...
      SET current_balance = current_balance + ?,
          updated_at = datetime('now')
      WHERE id = ?
    `).bind(r.profit_loss,i.account_id).run(),e.json({success:!0,profit_loss:r.profit_loss,profit_loss_pct:r.profit_loss_pct,pips:r.pips})}catch(a){return e.json({success:!1,error:a.message},500)}});j.get("/api/trading/trades/open",async e=>{const{DB:t}=e.env,s=e.req.query("account_id")||"1";try{const a=await t.prepare(`
      SELECT * FROM trades 
    ...
    `).bind(s).all(),{calculatePortfolioMetrics:n}=await Promise.resolve().then(()=>pt),o=n(a.results);return e.json({success:!0,stats:o})}catch(a){return e.json({success:!1,error:a.message},500)}});j.post("/api/trading/backtest",async e=>{var s;const{DB:t}=e.env;try{const a=await e.req.json(),n=await t.prepare(`
      SELECT timestamp, open, high, low, close, volume 
      FROM market_data 
      WHERE timeframe = ?
      ORDER BY timestamp ASC
    ...
    `).bind(a.run_name||`Backtest ${new Date().toISOString()}`,r.config.start_date,r.config.end_date,r.starting_balance,r.config.min_confidence,r.config.use_mtf_confirmation?1:0,r.config.use_news_filter?1:0,r.config.timeframe,r.total_trades,r.winning_trades,r.win_rate,r.net_profit,r.total_return_pct,r.max_drawdown_pct,r.profit_factor,r.sharpe_ratio,JSON.stringify(r.trades),JSON.stringify(r.equity_curve)).run(),e.json({success:!0,result:r,formatted:l(r)})}catch(a){return e.json({success:!1,error:a.message,stack:a.stack},500)}});j.get("/api/trading/backtest/history",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
    ...
    `).first(),i=!a&&(o==null?void 0:o.count)>0;let l=0;if(i)l=0,n.steps[0].cached=!0;else{n.steps[0].name="Fetching Fresh MTF Data (5 timeframes × 100 candles)",n.steps[0].fetching=!0;const D=await t.prepare(`
        SELECT setting_value FROM user_settings 
        WHERE setting_key = 'twelve_data_api_key'
      `).first(),B=(D==null?void 0:D.setting_value)||"70140f57bea54c5e90768de696487d8f",P=[{interval:"5min",dbKey:"5m"},{interval:"15min",dbKey:"15m"},{interval:"1h",dbKey:"1h"},{interval:"4h",dbKey:"4h"},{interval:"1day",dbKey:"daily"}];for(const N of P)try{const U=`https://api.twelvedata.com/time_series?symbol=XAU/USD&interval=${N.interval}&apikey=${B}&outputsize=100`,V=new AbortController,Q=setTimeout(()=>V.abort(),1e4),ee=await fetch(U,{signal:V.signal});clearTimeout(Q);const te=await ee.json();if(te.values&&Array.isArray(te.values)){const se=[];for(const x of te.values)se.push({timestamp:x.datetime,open:parseFloat(x.open)||0,high:parseFloat(x.high)||0,low:parseFloat(x.low)||0,close:parseFloat(x.close)||0,volume:0});for(const x of se)await t.prepare(`
                INSERT INTO market_data (timestamp, open, high, low, close, volume, timeframe)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT DO NOTHING
              `).bind(x.timestamp,x.open,x.high,x.low,x.close,x.volume,N.dbKey).run();if(se.length>=50){const x=pe(se.reverse());x&&await t.prepare(`
                  ...
                  (timestamp, timeframe, rsi_14, macd, macd_signal, macd_histogram,
                   ...
                   parabolic_sar, vwap, fib_382, fib_500, fib_618)
                  VALUES (datetime('now'), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                `).bind(N.dbKey,x.rsi_14,x.macd,x.macd_signal,x.macd_histogram,x.sma_20,x.sma_50,x.sma_200,x.ema_12,x.ema_26,x.bb_upper,x.bb_middle,x.bb_lower,x.atr_14,x.stochastic_k,x.stochastic_d,x.adx,x.plus_di,x.minus_di,x.ichimoku_tenkan,x.ichimoku_kijun,x.ichimoku_senkou_a,x.ichimoku_senkou_b,x.parabolic_sar,x.vwap,x.fib_382,x.fib_500,x.fib_618).run()}l+=te.values.length}await new Promise(se=>setTimeout(se,100))}catch(U){console.error(`[MTF] Error fetching ${N.dbKey}:`,U)}}n.steps[0].status="completed",n.steps[0].data={totalCandles:l},n.steps.push({step:2,name:"Generate MTF Signal",status:"running"});const{analyzeTimeframeAlignment:r,validateMultiTimeframeSignal:d,formatAlignmentReport:c}=await Promise.resolve().then(()=>es),p={};for(const D of["5m","15m","1h","4h","daily"]){const B=await t.prepare(`
        SELECT * FROM multi_timeframe_indicators 
        WHERE timeframe = ?
        ORDER BY timestamp DESC 
        LIMIT 1
      `).bind(D).first();B&&(p[D]=B)}const g=await t.prepare(`
      SELECT * FROM market_data 
      WHERE timeframe = '1h'
      ORDER BY timestamp DESC 
      LIMIT 1
    `).first(),m=(g==null?void 0:g.close)||0,_=((await t.prepare(`
      SELECT timestamp, open, high, low, close, volume FROM market_data 
      WHERE timeframe = '1h'
      ORDER BY timestamp DESC 
      LIMIT 20
    `).all()).results||[]).map(D=>({timestamp:D.timestamp,open:D.open,high:D.high,low:D.low,close:D.close,volume:D.volume||1})).reverse(),f=r(p,m),h=p["1h"],y=ze(m,h,_,"day_trade"),w=ze(m,h,_,"swing_trade"),b=d(y.signal_type,f),T=d(w.signal_type,f),E={...y,final_confidence:Math.min(95,b.confidence),isValid:b.isValid,mtf_reason:b.reason,alignment_score:f.score,alignment_type:f.type},k={...w,final_confidence:Math.min(95,T.confidence),isValid:T.isValid,mtf_reason:T.reason,alignment_score:f.score,alignment_type:f.type};n.steps[1].status="completed",n.steps[1].data={dayTrade:E,swingTrade:k,alignment:f},n.steps.push({step:3,name:"Calculate Position Sizes",status:"running"});const v=await t.prepare(`
      SELECT * FROM trading_accounts WHERE id = 1
    `).first(),$=await t.prepare(`
      SELECT * FROM position_sizing_rules 
      WHERE account_id = 1 AND is_active = 1
      ORDER BY confidence_min DESC
    `).all(),{calculatePositionSize:R}=await Promise.resolve().then(()=>pt),L=R(v,{entry_price:E.price,stop_loss:E.stop_loss,take_profit_1:E.take_profit_1,take_profit_2:E.take_profit_2,take_profit_3:E.take_profit_3,confidence:E.final_confidence,signal_type:E.signal_type,trading_style:E.trading_style},$.results),O=R(v,{entry_price:k.price,stop_loss:k.stop_loss,take_profit_1:k.take_profit_1,take_profit_2:k.take_profit_2,take_profit_3:k.take_profit_3,confidence:k.final_confidence,signal_type:k.signal_type,trading_style:k.trading_style},$.results);n.steps[2].status="completed",n.steps[2].data={dayPosition:L,swingPosition:O},n.steps.push({step:4,name:"Send Telegram Alert",status:"running"});const W=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings
      WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
    `).all(),I={};for(const D of W.results||[])I[D.setting_key]=D.setting_value;let C=!1;if(I.telegram_bot_token&&I.telegram_chat_id){const D=await t.prepare(`
        SELECT high, low FROM market_data 
        WHERE timeframe = '1h'
        ORDER BY timestamp DESC 
        LIMIT 20
...
🌐 Dashboard: ${e.req.url.replace("/api/automation/analyze-and-notify","")}
      `.trim();C=await q({botToken:I.telegram_bot_token,chatId:I.telegram_chat_id},N)}if(n.steps[3].status=C?"completed":"failed",n.steps[3].data={telegramSent:C},E.isValid||k.isValid)for(const D of[E,k])D.isValid&&await t.prepare(`
            INSERT INTO mtf_signals 
            (timestamp, signal_type, trading_style, price, stop_loss, 
             take_profit_1, take_profit_2, take_profit_3, 
             base_confidence, mtf_confidence, final_confidence,
        ...
        `).first(),B=(D==null?void 0:D.setting_value)||"70140f57bea54c5e90768de696487d8f",P=[{interval:"5min",dbKey:"5m"},{interval:"15min",dbKey:"15m"},{interval:"1h",dbKey:"1h"},{interval:"4h",dbKey:"4h"},{interval:"1day",dbKey:"daily"}];for(const N of P)try{const U=`https://api.twelvedata.com/time_series?symbol=XAU/USD&interval=${N.interval}&apikey=${B}&outputsize=100`,V=new AbortController,Q=setTimeout(()=>V.abort(),1e4),ee=await fetch(U,{signal:V.signal});clearTimeout(Q);const te=await ee.json();if(te.values&&Array.isArray(te.values)){const se=[];for(const x of te.values)se.push({timestamp:x.datetime,open:parseFloat(x.open)||0,high:parseFloat(x.high)||0,low:parseFloat(x.low)||0,close:parseFloat(x.close)||0,volume:0});for(const x of se)await t.prepare(`
                ...
                `).bind(x.timestamp,x.open,x.high,x.low,x.close,x.volume,N.dbKey).run();if(se.length>=50){const{calculateIndicators:x}=await Promise.resolve().then(()=>rs),M=x(se.reverse());M&&await t.prepare(`
                    ...
                    (timestamp, timeframe, rsi_14, macd, macd_signal, macd_histogram,
                     ...
                     parabolic_sar, vwap, fib_382, fib_500, fib_618)
                    VALUES (datetime('now'), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                  `).bind(N.dbKey,M.rsi_14,M.macd,M.macd_signal,M.macd_histogram,M.sma_20,M.sma_50,M.sma_200,M.ema_12,M.ema_26,M.bb_upper,M.bb_middle,M.bb_lower,M.atr_14,M.stochastic_k,M.stochastic_d,M.adx,M.plus_di,M.minus_di,M.ichimoku_tenkan,M.ichimoku_kijun,M.ichimoku_senkou_a,M.ichimoku_senkou_b,M.parabolic_sar,M.vwap,M.fib_382,M.fib_500,M.fib_618).run()}o+=te.values.length}await new Promise(se=>setTimeout(se,100))}catch(U){console.error(`[MTF] Error fetching ${N.dbKey}:`,U)}}else o=0,n.steps[0].cached=!0}n.steps[0].status="completed",n.steps[0].data={totalCandles:o},n.steps.push({step:2,name:"Generate MTF Signal",status:"running"});const{analyzeTimeframeAlignment:i,validateMultiTimeframeSignal:l}=await Promise.resolve().then(()=>es),{generateSignal:r}=await Promise.resolve().then(()=>rs),d={};for(const I of["5m","15m","1h","4h","daily"]){const C=await t.prepare(`
        SELECT * FROM multi_timeframe_indicators 
        WHERE timeframe = ?
        ORDER BY timestamp DESC 
        LIMIT 1
      `).bind(I).first();C&&(d[I]=C)}const c=await t.prepare(`
      SELECT * FROM market_data 
      WHERE timeframe = '1h'
      ORDER BY timestamp DESC 
      LIMIT 1
    `).first(),p=(c==null?void 0:c.close)||0,m=((await t.prepare(`
      ...
      ORDER BY timestamp DESC 
      LIMIT 20
    `).all()).results||[]).map(I=>({timestamp:I.timestamp,open:I.open,high:I.high,low:I.low,close:I.close,volume:I.volume||1})).reverse(),u=i(d,p),_=d["1h"],f=ze(p,_,m,"day_trade"),h=ze(p,_,m,"swing_trade"),y=l(f.signal_type,u),w=l(h.signal_type,u),b={...f,final_confidence:Math.min(95,y.confidence),isValid:y.isValid,mtf_reason:y.reason,alignment_score:u.score,alignment_type:u.type},T={...h,final_confidence:Math.min(95,w.confidence),isValid:w.isValid,mtf_reason:w.reason,alignment_score:u.score,alignment_type:u.type};n.steps[1].status="completed",n.steps[1].data={dayTrade:b,swingTrade:T,alignment:u},n.steps.push({step:3,name:"Calculate Position Sizes",status:"running"});const E=await t.prepare(`
      SELECT * FROM trading_accounts WHERE id = 1
    `).first(),k=await t.prepare(`
      SELECT * FROM position_sizing_rules 
      WHERE account_id = 1 AND is_active = 1
      ORDER BY confidence_min DESC
    `).all(),{calculatePositionSize:v}=await Promise.resolve().then(()=>pt),$=v(E,{entry_price:b.price,stop_loss:b.stop_loss,take_profit_1:b.take_profit_1,take_profit_2:b.take_profit_2,take_profit_3:b.take_profit_3,confidence:b.final_confidence,signal_type:b.signal_type,trading_style:b.trading_style},k.results),R=v(E,{entry_price:T.price,stop_loss:T.stop_loss,take_profit_1:T.take_profit_1,take_profit_2:T.take_profit_2,take_profit_3:T.take_profit_3,confidence:T.final_confidence,signal_type:T.signal_type,trading_style:T.trading_style},k.results);n.steps[2].status="completed",n.steps[2].data={dayPosition:$,swingPosition:R},n.steps.push({step:4,name:"Send Telegram Alert",status:"running"});const L=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings
      WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
    `).all(),O={};for(const I of L.results||[])O[I.setting_key]=I.setting_value;let W=!1;if(O.telegram_bot_token&&O.telegram_chat_id){const I=await t.prepare(`
        SELECT high, low FROM market_data 
        WHERE timeframe = '1h'
        ORDER BY timestamp DESC 
        LIMIT 20
      `).all();let C=[],D=[];if(I.results&&I.results.length>=20){const U=I.results.map(Q=>Q.high).sort((Q,ee)=>ee-Q),V=I.results.map(Q=>Q.low).sort((Q,ee)=>Q-ee);C=U.slice(0,3),D=V.slice(0,3)}const B=`
🤖 *AUTOMATED DAILY ANALYSIS*
⏰ ${new Date().toLocaleString("en-US",{timeZone:"UTC"})} UTC
...
🔴 *Resistance:* ${C.map(U=>`$${U.toFixed(2)}`).join(", ")}
🟢 *Support:* ${D.map(U=>`$${U.toFixed(2)}`).join(", ")}
...
🌐 Dashboard: ${e.req.url.replace("/api/automation/analyze-and-notify","")}
      `.trim(),{sendTelegramMessage:P}=await Promise.resolve().then(()=>Gs);W=await P({botToken:O.telegram_bot_token,chatId:O.telegram_chat_id},B)}if(n.steps[3].status=W?"completed":"failed",n.steps[3].data={telegramSent:W},b.isValid||T.isValid)for(const I of[b,T])I.isValid&&await t.prepare(`
            INSERT INTO mtf_signals 
            (timestamp, signal_type, trading_style, price, stop_loss, 
             take_profit_1, take_profit_2, take_profit_3, 
             base_confidence, mtf_confidence, final_confidence,
