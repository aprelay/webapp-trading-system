...
`;if(i.length>0){A+=`⚠️ *DATA FRESHNESS WARNING*
`;for(const J of i)A+=`${J}
...
`,v+=`⏰ ${k}`,console.log("[SIMPLE] Sending Telegram message, length:",v.length),f=await q({botToken:y.telegram_bot_token,chatId:y.telegram_chat_id},v),console.log("[SIMPLE] Telegram sent:",f),f||console.log("[SIMPLE] Telegram send failed - checking response")}else console.log("[SIMPLE] Telegram not configured")}catch(h){console.error("[SIMPLE] Telegram error:",h.message)}try{await t.prepare(`
    ...
    `).run()}catch(s){console.error("[DXY] Error storing DXY data:",s.message)}}async function Li(e){try{const t=await e.prepare(`
      SELECT timestamp, close
      FROM dxy_cache
      ORDER BY timestamp DESC
      LIMIT 10
    `).all();return!t.results||t.results.length===0?[]:t.results.map(s=>({close:s.close,timestamp:s.timestamp})).reverse()}catch(t){return console.error("[DXY] Error fetching cached DXY data:",t.message),[]}}async function Ii(e,t,s=15){const a=await Li(e);if(a.length>0){const i=new Date(a[a.length-1].timestamp),l=(new Date().getTime()-i.getTime())/6e4;if(l<s)return console.log(`[DXY] Using cached data (${l.toFixed(1)}min old)`),a}console.log("[DXY] Fetching fresh DXY data from API...");const n=await ki(t);return n.length>0?(await Ri(e,n),n):(console.log("[DXY] Fetch failed, using stale cache"),a)}function $i(e,t,s){const a=ds("Silver (XAG/USD)",e),n=ds("Crude Oil (WTI)",t);let i=0;a&&Ft(a.trend,s)&&i++,n&&Ft(n.trend,s)&&i++;let o=0;const l=i>=1;i===2?o=95:i===1?o=70:o=30;const r=Ai(a,n,i,s);return{silver:a,oil:n,aligned:l,alignmentCount:i,strength:o,description:r}}function ds(e,t){if(t.length<2)return null;const s=t[t.length-1],a=t[0],n=s.close,i=(s.close-a.close)/a.close*100;let o="FLAT";i>.2?o="UP":i<-.2&&(o="DOWN");const l=Math.abs(i);let r=0;return l>1?r=90:l>.5?r=75:l>.2?r=60:r=40,{symbol:e,price:n,change:i,trend:o,strength:r}}function Ft(e,t){return t==="HOLD"||e==="FLAT"?!1:t==="BUY"&&e==="UP"||t==="SELL"&&e==="DOWN"}function Ai(e,t,s,a){if(s===2)return`Silver & Oil both ${a==="BUY"?"up":"down"} (strong confirmation)`;if(s===1){if(e&&Ft(e.trend,a))return`Silver ${e.trend.toLowerCase()} confirms Gold ${a}`;if(t&&Ft(t.trend,a))return`Oil ${t.trend.toLowerCase()} confirms Gold ${a}`}const n=e?`Silver ${e.trend.toLowerCase()}`:"Silver N/A",i=t?`Oil ${t.trend.toLowerCase()}`:"Oil N/A";return`${n}, ${i} (mixed signals)`}async function Di(e){try{const t=`https://api.twelvedata.com/time_series?symbol=XAG/USD&interval=5min&outputsize=10&apikey=${e}`,a=await(await fetch(t)).json();return!a.values||a.values.length===0?(console.error("[SILVER] No data returned from API"),[]):a.values.map(i=>({close:parseFloat(i.close),timestamp:i.datetime})).reverse()}catch(t){return console.error("[SILVER] Error fetching Silver data:",t.message),[]}}async function Mi(e){try{const t=`https://api.twelvedata.com/time_series?symbol=WTI/USD&interval=5min&outputsize=10&apikey=${e}`,a=await(await fetch(t)).json();return!a.values||a.values.length===0?(console.error("[OIL] No data returned from API"),[]):a.values.map(i=>({close:parseFloat(i.close),timestamp:i.datetime})).reverse()}catch(t){return console.error("[OIL] Error fetching Oil data:",t.message),[]}}async function Ni(e,t,s){try{const a=t==="SILVER"?"silver_cache":"oil_cache";await e.prepare(`
...
⏰ ${new Date().toLocaleString("en-US",{timeZone:"UTC"})} UTC
    ...
    `).all();return e.json({success:!0,history:s.results})}catch(s){return e.json({success:!1,error:s.message},500)}});ct.post("/test-alert",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings
      WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
    `).all(),a={};for(const d of s.results||[])a[d.setting_key]=d.setting_value;if(!a.telegram_bot_token||!a.telegram_chat_id)return e.json({success:!1,error:"Telegram not configured. Please set Bot Token and Chat ID in settings."});const n=4386.5,i=15,o={grade:"A",signal:"BUY",confidence:87,session:"LONDON",layersPassed:6,layers:["✅ Layer 1: Trend Aligned (BULLISH)","✅ Layer 2: RSI 54, MACD bullish crossover","✅ Layer 3: Volume spike 1.9x average","✅ Layer 4: Broke above resistance","✅ Layer 5: Liquidity 89/100 (LONDON session)","✅ Layer 6: No major news","❌ Layer 7: ADX 72.3 (extreme, reversal risk)"],stopLoss:n-i,tp1:n+i*2,tp2:n+i*3,tp3:n+i*4,liquidityScore:89,adx:72.3,rsi:54.2,volumeRatio:1.9},l=Qs(o,n),r=await q({botToken:a.telegram_bot_token,chatId:a.telegram_chat_id},l);return e.json({success:r,message:r?"Test A-grade alert sent to Telegram!":"Failed to send alert. Check your Telegram settings."})}catch(s){return console.error("[TEST-ALERT] Error:",s),e.json({success:!1,error:s.message},500)}});async function Zs(e,t,s,a,n,i){console.log("[ANALYZE] Starting analysis");let o=0,l=0;const r=[],d=(ne,Ke)=>{const $t=parseFloat(String(ne));return isNaN($t)?Ke:$t};console.log("[ANALYZE] parseNum defined");const c={ema20:d(t.ema_12,i),rsi:d(t.rsi_14,50),macd:d(t.macd,0),macd_signal:d(t.macd_signal,0),macd_histogram:d(t.macd_histogram,0),adx:d(t.adx,25)},u={ema50:d(s.ema_26,i)},_={sma200:d(a.sma_200,i)},m=i>c.ema20&&i>u.ema50&&i>_.sma200,p=i<c.ema20&&i<u.ema50&&i<_.sma200;m||p?(o+=20,l++,r.push(`✅ Layer 1: Trend Aligned (${m?"BULLISH":"BEARISH"})`)):r.push("❌ Layer 1: Trend NOT aligned (conflicting)");const g=c.rsi>=40&&c.rsi<=60,f=c.macd>c.macd_signal&&c.macd_histogram>0,h=c.macd<c.macd_signal&&c.macd_histogram<0;g&&(m?f:h)?(o+=15,l++,r.push(`✅ Layer 2: RSI ${c.rsi.toFixed(0)}, MACD ${m?"bullish":"bearish"} crossover`)):r.push(`❌ Layer 2: RSI ${c.rsi.toFixed(0)}, MACD ${g?"no crossover":"extreme"}`);const y=n.slice(-20).reduce((ne,Ke)=>ne+Ke.volume,0)/20,w=n[n.length-1].volume;w>y*1.5?(o+=15,l++,r.push(`✅ Layer 3: Volume spike ${(w/y).toFixed(1)}x average`)):r.push(`❌ Layer 3: Volume ${(w/y).toFixed(1)}x (need 1.5x+)`);const S=Math.max(...n.slice(-20).map(ne=>ne.high)),b=Math.min(...n.slice(-20).map(ne=>ne.low)),k=i>S*.999,v=i<b*1.001;m&&k||p&&v?(o+=15,l++,r.push(`✅ Layer 4: ${m?"Broke above resistance":"Broke below support"}`)):r.push("❌ Layer 4: No clear breakout"),console.log("[ANALYZE] Layer 5: Calculating liquidity");let $=null;try{$=await Ht(n),console.log("[ANALYZE] Liquidity calculated successfully")}catch(ne){console.log("[5M-SCANNER] Liquidity calc failed:",ne)}const R=($==null?void 0:$.liquidity_score)||50,L=($==null?void 0:$.session)||"UNKNOWN";R>=70?(o+=15,l++,r.push(`✅ Layer 5: Liquidity ${R}/100 (${L} session)`)):r.push(`❌ Layer 5: Liquidity ${R}/100 (too low)`),console.log("[ANALYZE] Layer 6: Checking economic calendar");const j=kt();console.log("[ANALYZE] Calendar check complete"),j.riskLevel==="safe"?(o+=10,l++,r.push("✅ Layer 6: No major news")):r.push(`❌ Layer 6: ${j.reason}`);const O=c.adx>25,D=c.adx>70;O&&!D?(o+=10,l++,r.push(`✅ Layer 7: ADX ${c.adx.toFixed(1)} (strong trend)`)):D?r.push(`⚠️ Layer 7: ADX ${c.adx.toFixed(1)} (extreme, reversal risk)`):r.push(`❌ Layer 7: ADX ${c.adx.toFixed(1)} (weak trend)`);let B="HOLD";(m||p)&&l>=5&&(B=m?"BUY":"SELL");const P=new Date,N=Gn(P);N.hasBoost?(o+=8,l++,r.push(`✅ Layer 8: ${N.reason} (+${N.boost}% win)`)):r.push(`ℹ️ Layer 8: ${N.reason}`);const F=Vn(P);F.hasBoost?(o+=5,l++,r.push(`✅ Layer 9: ${F.reason} (+${F.boost}% win)`)):r.push(`ℹ️ Layer 9: ${F.reason}`);const V=d(t.atr_14,i*.01),Q=n.slice(-20).reduce((ne,Ke)=>{const $t=Ke.high-Ke.low;return ne+$t},0)/20;if(qn(V,Q)){o+=7,l++;const ne=((V/Q-1)*100).toFixed(1);r.push(`✅ Layer 10: ATR expanding ${ne}% (high volatility)`)}else{const ne=((1-V/Q)*100).toFixed(1);r.push(`❌ Layer 10: ATR compressed ${ne}% (skip low volatility)`)}const te=zn(n.slice(-20));Ks(te,B)&&te.strength>=60&&(o+=10,l++),r.push(Xn(te,B));const x=Kn(n.slice(-3)),{aligned:M,strongestPattern:ue}=ri(x,B);M&&ue?(o+=12,l++,r.push(`✅ Layer 12: ${ue.name} (${ue.strength}/100)`)):x.length>0&&x[0].type==="INDECISION"?r.push(`⚠️ Layer 12: ${x[0].name} (indecision, wait)`):r.push("❌ Layer 12: No clear candlestick pattern");const fe=li(n,i);ui(fe,B)&&fe.nearZone?(o+=8,l++,r.push(`✅ Layer 13: ${fe.description}`)):fe.nearZone?r.push(`⚠️ Layer 13: ${fe.description} (not aligned)`):r.push("ℹ️ Layer 13: No key zones nearby");const X=(await e.prepare(`
    SELECT rsi_14 as rsi, macd, macd_histogram
    FROM multi_timeframe_indicators
    WHERE timeframe = '5m'
    ORDER BY timestamp DESC
    LIMIT 10
  `).all()).results.map(ne=>({rsi:parseFloat(String(ne.rsi))||50,macd:parseFloat(String(ne.macd))||0,macd_histogram:parseFloat(String(ne.macd_histogram))||0})).reverse(),A=_i(X,n.slice(-10)),de=yi(A,B,m?"BULLISH":p?"BEARISH":"NEUTRAL");de&&A.strength>=70&&(o+=9,l++),r.push(Ei(A,de));const ae=bi(t,s,a,i),Lt=wi(ae,B);Lt&&(ae.allAligned||ae.twoAligned)&&(o+=6,l++),r.push(Ti(ae,Lt));const _t=await e.prepare(`
    SELECT setting_value FROM user_settings
    WHERE setting_key = 'twelve_data_api_key'
  `).first(),gt=(_t==null?void 0:_t.setting_value)||"70140f57bea54c5e90768de696487d8f",K=await Ii(e,gt,15),Be=Si(K);vi(Be,B)&&Be.strength>=60?(o+=5,l++,r.push(`✅ Layer 18: ${Be.description}`)):Be.goldSignalSupport!=="NEUTRAL"?r.push(`⚠️ Layer 18: ${Be.description} (not aligned)`):r.push("ℹ️ Layer 18: DXY flat (neutral for Gold)");const Te=await ms(e,gt,"SILVER",15),Ea=await ms(e,gt,"OIL",15),Xe=$i(Te,Ea,B);if(Xe.aligned&&Xe.alignmentCount>=1){const ne=Xe.alignmentCount===2?5:3;o+=ne,l++,r.push(`✅ Layer 19: ${Xe.description} (${Xe.strength}/100)`)}else r.push(`❌ Layer 19: ${Xe.description}`);const ba=await Wi(e)||Yi(),Le=Ci(ba);if(Ui(Le,B)&&Le.strength>=70){const ne=Le.positioning.includes("EXTREME")?7:4;o+=ne,l++,r.push(`✅ Layer 20: ${Le.description} (${Le.strength}/100)`)}else Le.goldSignalSupport!=="NEUTRAL"?r.push(`⚠️ Layer 20: ${Le.description} (not aligned)`):r.push(`ℹ️ Layer 20: ${Le.description}`);let It="C";o>=162?It="A+":o>=144?It="A":o>=126&&(It="B"),(m||p)&&l>=7&&(B=m?"BUY":"SELL");const Ie=Math.max(V*1.5,i*.003),wa=B==="BUY"?i-Ie:i+Ie,Ta=B==="BUY"?i+Ie*2:i-Ie*2,Sa=B==="BUY"?i+Ie*3:i-Ie*3,va=B==="BUY"?i+Ie*4:i-Ie*4;return{grade:It,score:o,signal:B,confidence:o,layersPassed:l,layers:r,stopLoss:wa,tp1:Ta,tp2:Sa,tp3:va,liquidityScore:R,session:L,adx:c.adx,rsi:c.rsi,volumeRatio:w/y}}function Qs(e,t){const s=e.signal==="BUY"?"🟢":"🔴",a=e.grade==="A+"?"💎":e.grade==="A"?"⭐":"📊",n=new Date,i=`${n.getUTCHours().toString().padStart(2,"0")}:${n.getUTCMinutes().toString().padStart(2,"0")}`;let o=`🚨 <b>${e.grade}-GRADE 5M SETUP DETECTED!</b> 🚨
...
/help - Show this message`);return e.json({ok:!0})}catch(t){return console.error("[TELEGRAM WEBHOOK] Error:",t),e.json({ok:!0})}});const jt=new _e;jt.post("/market-analysis",async e=>await da(e));jt.get("/health",async e=>e.json({success:!0,status:"healthy",service:"ai-analysis",timestamp:new Date().toISOString()}));jt.get("/auto-ai-scan",async e=>await da(e));async function da(e){const{DB:t}=e.env;try{console.log("[AI-ANALYSIS] Starting comprehensive market analysis");const s=await t.prepare(`
    ...
    `).all();let a="";for(const R of s.results||[])R.setting_key==="twelve_data_api_key"&&(a=R.setting_value);let n=[];if(a&&a!=="your_api_key_here")try{const R=`https://api.twelvedata.com/time_series?symbol=XAU/USD&interval=1h&outputsize=100&apikey=${a}`,U=await(await fetch(R)).json();U.values&&U.values.length>=50&&(n=U.values.reverse().map(j=>({timestamp:j.datetime,open:parseFloat(j.open),high:parseFloat(j.high),low:parseFloat(j.low),close:parseFloat(j.close),volume:parseFloat(j.volume)||0})),console.log("[AI-ANALYSIS] Fresh data fetched:",n.length,"candles"))}catch{console.error("[AI-ANALYSIS] API fetch failed, falling back to database")}if(n.length===0){const R=await t.prepare(`
        ...
        ORDER BY timestamp DESC 
        LIMIT 100
      `).all();if(!R.results||R.results.length<50)return e.json({success:!1,error:"Not enough market data available"},400);n=R.results.reverse().map(L=>({timestamp:L.timestamp,open:L.open,high:L.high,low:L.low,close:L.close,volume:L.volume||0}))}const i=pe(n);if(!i)return e.json({success:!1,error:"Failed to calculate indicators"},400);const o=n[n.length-1].close,l=le(o,i,"day_trade");console.log("[AI-ANALYSIS] Current price:",o,"Signal:",l.signal_type,"Confidence:",l.confidence);const r={};for(const R of["5m","15m","1h","4h","daily"]){const L=await t.prepare(`
        SELECT * FROM multi_timeframe_indicators 
        WHERE timeframe = ?
        ORDER BY timestamp DESC 
        LIMIT 1
      `).bind(R).first();L&&(r[R]=L)}const d=Jt(r,o),c=n.slice(-50),u=c.map(R=>R.high).sort((R,L)=>L-R),_=c.map(R=>R.low).sort((R,L)=>R-L),m=[Math.max(...u.slice(0,10))],p=[Math.min(..._.slice(0,10))];o>i.sma_20?p.push(i.sma_20):m.push(i.sma_20),o>i.sma_50?p.push(i.sma_50):m.push(i.sma_50),o>i.vwap?p.push(i.vwap):m.push(i.vwap);const g=Math.round(o/10)*10;g>o?m.push(g):p.push(g);const f=[...new Set(m)].sort((R,L)=>R-L).filter(R=>R>o).slice(0,3),h=[...new Set(p)].sort((R,L)=>L-R).filter(R=>R<o).slice(0,3);console.log("[AI-ANALYSIS] Key levels - Support:",h,"Resistance:",f);const y=i.atr_14/o*100;let w="NORMAL";y>3?w="EXTREME":y>1.5?w="HIGH":y<.5&&(w="LOW");const E=[];let S=30,b=30,k=40;d.type==="ALL_BULLISH"?(S=60,b=20,k=20):d.type==="ALL_BEARISH"?(S=20,b=60,k=20):d.score>=4&&(d.trends.filter(R=>R.trend==="BULLISH").length>=4?(S=50,b=25,k=25):(S=25,b=50,k=25)),f.length>0&&E.push({name:"📈 BULLISH CONTINUATION",probability:S,description:`Price breaks above $${f[0].toFixed(2)} and rallies toward $${(f[f.length-1]||o*1.02).toFixed(2)}`,trigger:`Breakout above $${f[0].toFixed(2)} with volume`,target:f[f.length-1]||o*1.02}),h.length>0&&E.push({name:"📉 BEARISH CORRECTION",probability:b,description:`Price breaks below $${h[0].toFixed(2)} and drops toward $${(h[h.length-1]||o*.98).toFixed(2)}`,trigger:`Breakdown below $${h[0].toFixed(2)} with volume`,target:h[h.length-1]||o*.98}),E.push({name:"↔️ CONTINUED RANGING",probability:k,description:`Price oscillates between $${(h[0]||o*.99).toFixed(2)} and $${(f[0]||o*1.01).toFixed(2)} with choppy action`,trigger:w==="EXTREME"?"High volatility continues":"No clear breakout/breakdown",target:null}),E.sort((R,L)=>L.probability-R.probability);let v={action:"WAIT",reason:"Market conditions unclear - wait for better setup",entry_range:null,stop_loss:null};l.confidence>=65?l.signal_type==="BUY"?v={action:"BUY",reason:`Strong ${l.signal_type} signal with ${l.confidence}% confidence. ${d.type} MTF alignment.`,entry_range:`${(o-5).toFixed(2)}-${o.toFixed(2)}`,stop_loss:l.stop_loss.toFixed(2)}:l.signal_type==="SELL"&&(v={action:"SELL",reason:`Strong ${l.signal_type} signal with ${l.confidence}% confidence. ${d.type} MTF alignment.`,entry_range:`${o.toFixed(2)}-${(o+5).toFixed(2)}`,stop_loss:l.stop_loss.toFixed(2)}):w==="EXTREME"?v.reason=`⚠️ EXTREME volatility (ADX ${i.adx.toFixed(1)}) - Too risky to trade. Wait for market to calm down.`:(d.type==="MIXED"||d.type==="CONFLICTING")&&(v.reason=`⏰ Timeframes conflicting (${d.score}/5 aligned). Wait for ${f[0]?`breakout above $${f[0].toFixed(2)}`:h[0]?`breakdown below $${h[0].toFixed(2)}`:"clearer direction"}.`);let $=!1;if(l.confidence>=65)try{const R=await t.prepare(`
          SELECT setting_key, setting_value FROM user_settings
          WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
...
⏰ ${new Date().toLocaleString()}`,j=await q({botToken:i,chatId:o},U);k.day_trade_send_result=j,console.log("[AUTO-FETCH] Day trade alert result:",j),j?(S=!0,b.push("Day Trade"),R&&y.signal_type!=="HOLD"&&(console.log("[AUTO-FETCH] 🔥 HIGH CONVICTION signal detected! Sending reminder alerts..."),setTimeout(async()=>{const I=`${$} <b>⚠️ REMINDER: HIGH CONVICTION SIGNAL</b> ${$}
...
⏰ Don't miss this trade!`;await q({botToken:i,chatId:o},I),console.log("[AUTO-FETCH] First reminder sent")},120*1e3),setTimeout(async()=>{const I=`${$} <b>⚠️ FINAL REMINDER: HIGH CONVICTION</b> ${$}
...
⏰ ${new Date().toLocaleString()}`,j=await q({botToken:i,chatId:o},U);k.swing_trade_send_result=j,j&&(S=!0,b.push("Swing Trade"),R&&w.signal_type!=="HOLD"&&(console.log("[AUTO-FETCH] 🔥 HIGH CONVICTION swing signal! Sending reminder alerts..."),setTimeout(async()=>{const I=`${$} <b>⚠️ REMINDER: HIGH CONVICTION SWING</b> ${$}
...
⏰ Don't miss this swing trade!`;await q({botToken:i,chatId:o},I),console.log("[AUTO-FETCH] Swing first reminder sent")},180*1e3),setTimeout(async()=>{const I=`${$} <b>⚠️ FINAL REMINDER: HIGH CONVICTION</b> ${$}
...
📊 ${n(c)}`)&&(S.push("swing_trade"),E=!0));for(const b of[f,h])b.isValid&&await t.prepare(`
    ...
    `).all();let a="";for(const E of s.results||[])E.setting_key==="twelve_data_api_key"&&(a=E.setting_value);let n,i=!1;if(a&&a!=="your_api_key_here"){console.log("[GENERATE-NOW] Fetching FRESH data from Twelve Data API");try{const E=`https://api.twelvedata.com/time_series?symbol=XAU/USD&interval=1h&outputsize=100&apikey=${a}`,b=await(await fetch(E)).json();b.values&&b.values.length>=50?(n=b.values.reverse().map(k=>({timestamp:k.datetime,open:parseFloat(k.open)||0,high:parseFloat(k.high)||0,low:parseFloat(k.low)||0,close:parseFloat(k.close)||0,volume:parseFloat(k.volume)||0})),i=!0,console.log("[GENERATE-NOW] ✅ Fresh data fetched! Price:",n[n.length-1].close)):console.log("[GENERATE-NOW] ⚠️ API returned insufficient data, falling back to database")}catch(E){console.error("[GENERATE-NOW] API fetch failed:",E.message)}}if(!n||n.length===0){console.log("[GENERATE-NOW] Using database data (may be stale)");const E=await t.prepare(`
        ...
        ORDER BY timestamp DESC 
        LIMIT 200
      `).all();if(!E.results||E.results.length<50)return e.json({success:!1,error:"Not enough data. Please configure Twelve Data API key or fetch market data first."});n=E.results.reverse().map(S=>({timestamp:S.timestamp,open:S.open,high:S.high,low:S.low,close:S.close,volume:S.volume}))}const o=pe(n);if(!o)return e.json({success:!1,error:"Failed to calculate indicators"});let l=n[n.length-1].close,r=!1;if(a&&a!=="your_api_key_here")try{console.log("[GENERATE-NOW] Fetching real-time price...");const S=await(await fetch(`https://api.twelvedata.com/price?symbol=XAU/USD&apikey=${a}`,{signal:AbortSignal.timeout(5e3)})).json();if(S.price){const b=parseFloat(S.price),k=l,$=Math.abs(b-k)/b*100;console.log(`[GENERATE-NOW] Real-time: $${b}, Last candle: $${k}, Diff: ${$.toFixed(2)}%`),l=b,r=!0}}catch(E){console.log("[GENERATE-NOW] Real-time price fetch failed, using candle close:",E.message)}const d=le(l,o,"day_trade"),c=le(l,o,"swing_trade");console.log("[GENERATE-NOW] Signals generated - Day:",d.signal_type,"Swing:",c.signal_type);const u=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings
      WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
    ...
    `).first(),o=!a&&(i==null?void 0:i.count)>0;let l=0;if(o)l=0,n.steps[0].cached=!0;else{n.steps[0].name="Fetching Fresh MTF Data (5 timeframes × 100 candles)",n.steps[0].fetching=!0;const D=await t.prepare(`
        SELECT setting_value FROM user_settings 
        WHERE setting_key = 'twelve_data_api_key'
        ...
        `).first(),B=(D==null?void 0:D.setting_value)||"70140f57bea54c5e90768de696487d8f",P=[{interval:"5min",dbKey:"5m"},{interval:"15min",dbKey:"15m"},{interval:"1h",dbKey:"1h"},{interval:"4h",dbKey:"4h"},{interval:"1day",dbKey:"daily"}];for(const N of P)try{const F=`https://api.twelvedata.com/time_series?symbol=XAU/USD&interval=${N.interval}&apikey=${B}&outputsize=100`,V=new AbortController,Q=setTimeout(()=>V.abort(),1e4),ee=await fetch(F,{signal:V.signal});clearTimeout(Q);const te=await ee.json();if(te.values&&Array.isArray(te.values)){const se=[];for(const x of te.values)se.push({timestamp:x.datetime,open:parseFloat(x.open)||0,high:parseFloat(x.high)||0,low:parseFloat(x.low)||0,close:parseFloat(x.close)||0,volume:0});for(const x of se)await t.prepare(`
                ...
                `).bind(x.timestamp,x.open,x.high,x.low,x.close,x.volume,N.dbKey).run();if(se.length>=50){const{calculateIndicators:x}=await Promise.resolve().then(()=>rs),M=x(se.reverse());M&&await t.prepare(`
                    ...
                    (timestamp, timeframe, rsi_14, macd, macd_signal, macd_histogram,
                     ...
                     parabolic_sar, vwap, fib_382, fib_500, fib_618)
                    VALUES (datetime('now'), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                  `).bind(N.dbKey,M.rsi_14,M.macd,M.macd_signal,M.macd_histogram,M.sma_20,M.sma_50,M.sma_200,M.ema_12,M.ema_26,M.bb_upper,M.bb_middle,M.bb_lower,M.atr_14,M.stochastic_k,M.stochastic_d,M.adx,M.plus_di,M.minus_di,M.ichimoku_tenkan,M.ichimoku_kijun,M.ichimoku_senkou_a,M.ichimoku_senkou_b,M.parabolic_sar,M.vwap,M.fib_382,M.fib_500,M.fib_618).run()}i+=te.values.length}await new Promise(se=>setTimeout(se,100))}catch(F){console.error(`[MTF] Error fetching ${N.dbKey}:`,F)}}else i=0,n.steps[0].cached=!0}n.steps[0].status="completed",n.steps[0].data={totalCandles:i},n.steps.push({step:2,name:"Generate MTF Signal",status:"running"});const{analyzeTimeframeAlignment:o,validateMultiTimeframeSignal:l}=await Promise.resolve().then(()=>es),{generateSignal:r}=await Promise.resolve().then(()=>rs),d={};for(const I of["5m","15m","1h","4h","daily"]){const O=await t.prepare(`
        SELECT * FROM multi_timeframe_indicators 
        WHERE timeframe = ?
        ORDER BY timestamp DESC 
        LIMIT 1
