...
`;const he=candles1h.slice(-20),de=he.map(J=>J.high).sort((J,Te)=>Te-J),ae=he.map(J=>J.low).sort((J,Te)=>J-Te),It=de.slice(0,3),_t=ae.slice(0,3);if(A+=`📊 *Key Levels:*
...
`,v+=`⏰ ${k}`,console.log("[SIMPLE] Sending Telegram message, length:",v.length),f=await q({botToken:y.telegram_bot_token,chatId:y.telegram_chat_id},v),console.log("[SIMPLE] Telegram sent:",f),f||console.log("[SIMPLE] Telegram send failed - checking response")}else console.log("[SIMPLE] Telegram not configured")}catch(h){console.error("[SIMPLE] Telegram error:",h.message)}try{await t.prepare(`
    ...
    `).run()}catch(s){console.error("[DXY] Error storing DXY data:",s.message)}}async function Lo(e){try{const t=await e.prepare(`
      ...
      ORDER BY timestamp DESC
      LIMIT 10
    `).all();return!t.results||t.results.length===0?[]:t.results.map(s=>({close:s.close,timestamp:s.timestamp})).reverse()}catch(t){return console.error("[DXY] Error fetching cached DXY data:",t.message),[]}}async function $o(e,t,s=15){const a=await Lo(e);if(a.length>0){const o=new Date(a[a.length-1].timestamp),l=(new Date().getTime()-o.getTime())/6e4;if(l<s)return console.log(`[DXY] Using cached data (${l.toFixed(1)}min old)`),a}console.log("[DXY] Fetching fresh DXY data from API...");const n=await Ro(t);return n.length>0?(await Io(e,n),n):(console.log("[DXY] Fetch failed, using stale cache"),a)}function Ao(e,t,s){const a=ds("Silver (XAG/USD)",e),n=ds("Crude Oil (WTI)",t);let o=0;a&&Ft(a.trend,s)&&o++,n&&Ft(n.trend,s)&&o++;let i=0;const l=o>=1;o===2?i=95:o===1?i=70:i=30;const r=Do(a,n,o,s);return{silver:a,oil:n,aligned:l,alignmentCount:o,strength:i,description:r}}function ds(e,t){if(t.length<2)return null;const s=t[t.length-1],a=t[0],n=s.close,o=(s.close-a.close)/a.close*100;let i="FLAT";o>.2?i="UP":o<-.2&&(i="DOWN");const l=Math.abs(o);let r=0;return l>1?r=90:l>.5?r=75:l>.2?r=60:r=40,{symbol:e,price:n,change:o,trend:i,strength:r}}function Ft(e,t){return t==="HOLD"||e==="FLAT"?!1:t==="BUY"&&e==="UP"||t==="SELL"&&e==="DOWN"}function Do(e,t,s,a){if(s===2)return`Silver & Oil both ${a==="BUY"?"up":"down"} (strong confirmation)`;if(s===1){if(e&&Ft(e.trend,a))return`Silver ${e.trend.toLowerCase()} confirms Gold ${a}`;if(t&&Ft(t.trend,a))return`Oil ${t.trend.toLowerCase()} confirms Gold ${a}`}const n=e?`Silver ${e.trend.toLowerCase()}`:"Silver N/A",o=t?`Oil ${t.trend.toLowerCase()}`:"Oil N/A";return`${n}, ${o} (mixed signals)`}async function Mo(e){try{const t=`https://api.twelvedata.com/time_series?symbol=XAG/USD&interval=5min&outputsize=10&apikey=${e}`,a=await(await fetch(t)).json();return!a.values||a.values.length===0?(console.error("[SILVER] No data returned from API"),[]):a.values.map(o=>({close:parseFloat(o.close),timestamp:o.datetime})).reverse()}catch(t){return console.error("[SILVER] Error fetching Silver data:",t.message),[]}}async function No(e){try{const t=`https://api.twelvedata.com/time_series?symbol=WTI/USD&interval=5min&outputsize=10&apikey=${e}`,a=await(await fetch(t)).json();return!a.values||a.values.length===0?(console.error("[OIL] No data returned from API"),[]):a.values.map(o=>({close:parseFloat(o.close),timestamp:o.datetime})).reverse()}catch(t){return console.error("[OIL] Error fetching Oil data:",t.message),[]}}async function Oo(e,t,s){try{const a=t==="SILVER"?"silver_cache":"oil_cache";await e.prepare(`
...
`,h+=`Signal #${l} | ${f}`,h}ut.get("/scan",async e=>{var a;const{DB:t}=e.env,s={success:!1,message:"",data:{}};try{console.log("[MICRO] Starting 5-minute scan...");const n=await ri(t);if(!n.allowed)return console.log("[MICRO] Daily limit check failed:",n.reason),s.message=n.reason,s.data={signals_sent_today:n.signals_sent_today,max_signals:n.max_signals},e.json(s);console.log(`[MICRO] Daily limit check passed: ${n.signals_sent_today}/${n.max_signals} signals sent`);const o=await t.prepare(`
    ...
    `).all();if(!o.results||o.results.length<20)return s.message="Insufficient 5m candle data",e.json(s);const i=o.results.map(b=>({timestamp:b.timestamp,open:b.open,high:b.high,low:b.low,close:b.close,volume:b.volume||1})).reverse(),l=await t.prepare(`
      ...
      ORDER BY timestamp DESC 
      LIMIT 60
    `).all();if(!l.results||l.results.length<50)return s.message=`Insufficient 15m candle data (have ${((a=l.results)==null?void 0:a.length)||0}, need 50)`,e.json(s);const r=l.results.map(b=>({timestamp:b.timestamp,open:b.open,high:b.high,low:b.low,close:b.close,volume:b.volume||1})).reverse(),d=pe(i),c=pe(r);if(!d)return console.error("[MICRO] Failed to calculate 5m indicators - data length:",i.length),s.message="Failed to calculate 5m indicators",s.data={candles5m_count:i.length,first_candle:i[0],last_candle:i[i.length-1]},e.json(s);if(!c)return console.error("[MICRO] Failed to calculate 15m indicators - data length:",r.length),s.message="Failed to calculate 15m indicators",s.data={candles15m_count:r.length},e.json(s);const u=i[i.length-1].close,_=pa(i,d,r,c);if(!_)return console.log("[MICRO] No valid setup detected"),s.success=!0,s.message="No signal - waiting for setup",s.data={currentPrice:u},e.json(s);if(console.log(`[MICRO] Signal detected: ${_.signal_type} ${_.setup_type} (${_.confidence}%)`),_.confidence<60)return console.log(`[MICRO] Confidence too low: ${_.confidence}%`),s.success=!0,s.message=`Signal below threshold (${_.confidence}% < 60%)`,s.data={signal:_},e.json(s);const m=Ht(i);if(m.liquidity_score<60)return console.log(`[MICRO] Liquidity too low: ${m.liquidity_score}`),s.success=!0,s.message=`Low liquidity (${m.liquidity_score} < 60)`,s.data={signal:_,liquidity:m},e.json(s);const p=li(_,m.liquidity_score,m.optimal_for_trading),g=new Date(Date.now()+900*1e3).toISOString(),f=new Date().toISOString().split("T")[0],h=await t.prepare(`
...
`,p+=`Signal #${s} | ${_}`,p}Rt.get("/test-alert",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
    ...
    `).all(),y={};for(const E of h.results||[])y[E.setting_key]=E.setting_value;let w=!1;if(y.telegram_bot_token&&y.telegram_chat_id){const E=ga(f,m,g,85,"NEW_YORK");w=await q({botToken:y.telegram_bot_token,chatId:y.telegram_chat_id},E),w&&await t.prepare(`
      ...
      ORDER BY created_at DESC 
      LIMIT ?
    `).bind(s).all();return e.json({success:!0,signals:n.results||[],count:((a=n.results)==null?void 0:a.length)||0})}catch(n){return console.error("[HYBRID-MICRO] Error fetching signals:",n),e.json({success:!1,error:n.message,signals:[]},500)}});Rt.get("/test",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
      SELECT timestamp, open, high, low, close, volume 
      FROM market_data 
      WHERE timeframe = '5m'
      ORDER BY timestamp DESC 
      LIMIT 50
    `).all();if(!s.results||s.results.length<20)return e.json({success:!1,error:"Insufficient data for testing"});const a=s.results.map(l=>({timestamp:l.timestamp,open:l.open,high:l.high,low:l.low,close:l.close,volume:l.volume||1})).reverse(),n=await pe(a),o=a[a.length-1].close,i=await _a("BUY",o,a,n,{"5m":{trend:"BULLISH",indicators:n}});return e.json({success:!0,test_results:{grade:i.grade,filters_passed:i.filtersPassed,confidence:i.confidence,position_multiplier:i.positionMultiplier,filter_details:i.filterResults},current_price:o,indicators:{rsi:n.rsi,macd:n.macd,adx:n.adx}})}catch(s){return e.json({success:!1,error:s.message},500)}});const G=new _e;G.use("/api/*",ln());G.route("/api/signals/enhanced",zs);G.route("/api/signals/simple",Xs);G.route("/api/scanner",ct);G.route("/api/trades",we);G.route("/api/calendar",Fe);G.route("/api/backtest",pt);G.route("/api/telegram",ca);G.route("/api/ai",jt);G.route("/api/monitoring",Ue);G.route("/api/micro",ut);G.route("/api/hybrid-micro",Rt);G.get("/",e=>e.html(`
    <!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
