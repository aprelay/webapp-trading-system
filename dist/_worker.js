...
`;const he=candles1h.slice(-20),de=he.map(J=>J.high).sort((J,Te)=>Te-J),ae=he.map(J=>J.low).sort((J,Te)=>J-Te),Lt=de.slice(0,3),gt=ae.slice(0,3);if(A+=`📊 *Key Levels:*
...
`,i+="Next scan in 5 minutes...",i}async function dt(e){const t=await e.prepare(`
  ...
  `).first();return t||(await e.prepare(`
    ...
    `).bind(t,new Date().toISOString(),s,i,l,r,e).run();const c=(await dt(a)).current_balance+i;return await a.prepare(`
      ...
      SET current_balance = ?,
          updated_at = CURRENT_TIMESTAMP
      WHERE id = 1
    `).bind(c).run(),await sa(a),await zo(a),await Xo(a),{success:!0,profit_loss:i}}catch(n){return{success:!1,error:n.message}}}async function sa(e){const t=await dt(e),s=await e.prepare(`
  ...
  `).bind(t).all();if(s.results.length===0)return;const a=s.results,n=a.length,o=a.filter(m=>m.win===1).length,i=a.filter(m=>m.win===0).length,l=o/n*100,r=a.reduce((m,p)=>m+(p.profit_loss||0),0),d=Math.max(...a.map(m=>m.profit_loss||0)),c=Math.min(...a.map(m=>m.profit_loss||0)),u=a.reduce((m,p)=>m+(p.confidence||0),0)/n,_=a.reduce((m,p)=>m+(p.mtf_score||0),0)/n;await e.prepare(`
    ...
    ON CONFLICT(trade_date) DO UPDATE SET
      ...
      updated_at = CURRENT_TIMESTAMP
  `).bind(t,n,o,i,l,r,d,c,u,_).run()}async function Xo(e){const t=await dt(e),s=(t.starting_balance-t.current_balance)/t.starting_balance*100,a=new Date().toISOString().split("T")[0],n=await e.prepare(`
...
`)}const Jo=Object.freeze(Object.defineProperty({__proto__:null,formatBacktestResults:la,runBacktest:ra},Symbol.toStringTag,{value:"Module"})),pt=new _e;pt.post("/run",async e=>{try{const{DB:t}=e.env,s=await e.req.json(),a=await t.prepare(`
      SELECT timestamp, open, high, low, close, volume
      FROM market_data
      WHERE timeframe = '1h'
...
`,h+=`Signal #${l} | ${f}`,h}ut.get("/scan",async e=>{var a;const{DB:t}=e.env,s={success:!1,message:"",data:{}};try{console.log("[MICRO] Starting 5-minute scan...");const n=await li(t);if(!n.allowed)return console.log("[MICRO] Daily limit check failed:",n.reason),s.message=n.reason,s.data={signals_sent_today:n.signals_sent_today,max_signals:n.max_signals},e.json(s);console.log(`[MICRO] Daily limit check passed: ${n.signals_sent_today}/${n.max_signals} signals sent`);const o=await t.prepare(`
      SELECT timestamp, open, high, low, close, volume 
      FROM market_data 
      WHERE timeframe = '5m'
      ORDER BY timestamp DESC 
...
`,p+=`Signal #${s} | ${_}`,p}_t.get("/test-alert",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
    ...
    `).all(),y={};for(const E of h.results||[])y[E.setting_key]=E.setting_value;let w=!1;if(y.telegram_bot_token&&y.telegram_chat_id){const E=ga(f,m,g,85,"NEW_YORK");w=await q({botToken:y.telegram_bot_token,chatId:y.telegram_chat_id},E),w&&await t.prepare(`
      ...
      ORDER BY created_at DESC 
      LIMIT ?
    `).bind(s).all();return e.json({success:!0,signals:n.results||[],count:((a=n.results)==null?void 0:a.length)||0})}catch(n){return console.error("[HYBRID-MICRO] Error fetching signals:",n),e.json({success:!1,error:n.message,signals:[]},500)}});_t.get("/test",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
      SELECT timestamp, open, high, low, close, volume 
      FROM market_data 
      WHERE timeframe = '5m'
      ORDER BY timestamp DESC 
      LIMIT 50
    `).all();if(!s.results||s.results.length<20)return e.json({success:!1,error:"Insufficient data for testing"});const a=s.results.map(l=>({timestamp:l.timestamp,open:l.open,high:l.high,low:l.low,close:l.close,volume:l.volume||1})).reverse(),n=await pe(a),o=a[a.length-1].close,i=await _a("BUY",o,a,n,{"5m":{trend:"BULLISH",indicators:n}});return e.json({success:!0,test_results:{grade:i.grade,filters_passed:i.filtersPassed,confidence:i.confidence,position_multiplier:i.positionMultiplier,filter_details:i.filterResults},current_price:o,indicators:{rsi:n.rsi,macd:n.macd,adx:n.adx}})}catch(s){return e.json({success:!1,error:s.message},500)}});const Y=new _e;Y.use("/api/*",cn());Y.route("/api/signals/enhanced",zs);Y.route("/api/signals/simple",Xs);Y.route("/api/scanner",ct);Y.route("/api/trades",we);Y.route("/api/calendar",Fe);Y.route("/api/backtest",pt);Y.route("/api/telegram",ca);Y.route("/api/ai",jt);Y.route("/api/monitoring",Ue);Y.route("/api/micro",ut);Y.route("/api/hybrid-micro",_t);Y.get("/",e=>e.html(`
    <!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
      ...
      `).bind(m.timestamp,m.open,m.high,m.low,m.close,m.volume));await t.batch(u);const _=c.length;if(c.length>=50){const m=pe(c.reverse());if(m){await t.prepare(`
        ...
        `).bind(m.rsi_14,m.macd,m.macd_signal,m.macd_histogram,m.sma_20,m.sma_50,m.sma_200,m.ema_12,m.ema_26,m.bb_upper,m.bb_middle,m.bb_lower,m.atr_14,m.stochastic_k,m.stochastic_d,m.adx,m.plus_di,m.minus_di,m.ichimoku_tenkan,m.ichimoku_kijun,m.ichimoku_senkou_a,m.ichimoku_senkou_b,m.parabolic_sar,m.vwap,m.fib_382||0,m.fib_500||0,m.fib_618||0).run();const p=c[c.length-1].close,g=le(p,m,"day_trade"),f=le(p,m,"swing_trade"),h=70;for(const y of[g,f])if(y.confidence>=h&&y.signal_type!=="HOLD"){await t.prepare(`
            ...
            `).bind(y.signal_type,y.trading_style,y.price,y.stop_loss,y.take_profit_1,y.take_profit_2,y.take_profit_3,y.confidence,y.reason).run();const w=await t.prepare(`
              SELECT setting_key, setting_value FROM user_settings
              WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
            `).all(),E={};for(const S of w.results||[])E[S.setting_key]=S.setting_value;E.telegram_bot_token&&E.telegram_chat_id&&await q({botToken:E.telegram_bot_token,chatId:E.telegram_chat_id},lt(y))}}}return e.json({success:!0,count:_})}catch(s){return e.json({success:!1,error:s.message},500)}});Y.get("/api/cron/ping",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
      SELECT close, timestamp FROM market_data 
      WHERE timeframe = '1h' 
...
🌐 Dashboard: ${e.req.url.replace("/api/cron/hedge-fund","")}
    ...
    `).bind(s).all(),{calculatePortfolioMetrics:n}=await Promise.resolve().then(()=>mt),o=n(a.results);return e.json({success:!0,stats:o})}catch(a){return e.json({success:!1,error:a.message},500)}});Y.post("/api/trading/backtest",async e=>{var s;const{DB:t}=e.env;try{const a=await e.req.json(),n=await t.prepare(`
      SELECT timestamp, open, high, low, close, volume 
      FROM market_data 
      WHERE timeframe = ?
      ORDER BY timestamp ASC
