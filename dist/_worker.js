...
✅ Telegram alerts are active.`);return e.json({success:n})}catch(s){return e.json({success:!1,error:s.message},500)}});I.post("/api/news/fetch",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
    ...
    `).first();let a=(s==null?void 0:s.setting_value)||"70140f57bea54c5e90768de696487d8f";if(!a||a==="your_key_here"||a==="")return e.json({success:!1,error:"Twelve Data API key not configured. Please add it in settings.",count:0});const o=`https://api.twelvedata.com/time_series?symbol=XAU/USD&interval=1h&apikey=${a}&outputsize=100`,r=await(await fetch(o)).json();if(r.code&&r.status==="error")return e.json({success:!1,error:r.message||"Twelve Data API error",count:0});if(!r.values||!Array.isArray(r.values))return e.json({success:!1,error:"No data available from Twelve Data API",count:0});const c=r.values;let d=0;const p=[];for(const f of c){const g={timestamp:f.datetime,open:parseFloat(f.open),high:parseFloat(f.high),low:parseFloat(f.low),close:parseFloat(f.close),volume:0};p.push(g),await t.prepare(`
      ...
      `).bind(g.timestamp,g.open,g.high,g.low,g.close,g.volume).run(),d++}if(p.length>=50){const f=Oe(p.reverse());if(f){await t.prepare(`
        ...
        `).bind(f.rsi_14,f.macd,f.macd_signal,f.macd_histogram,f.sma_20,f.sma_50,f.sma_200,f.ema_12,f.ema_26,f.bb_upper,f.bb_middle,f.bb_lower,f.atr_14,f.stochastic_k,f.stochastic_d,f.adx,f.plus_di,f.minus_di,f.ichimoku_tenkan,f.ichimoku_kijun,f.ichimoku_senkou_a,f.ichimoku_senkou_b,f.parabolic_sar,f.vwap,f.fib_382||0,f.fib_500||0,f.fib_618||0).run();const g=p[p.length-1].close,w=K(g,f,"day_trade"),m=K(g,f,"swing_trade"),_=70;for(const h of[w,m])if(h.confidence>=_&&h.signal_type!=="HOLD"){await t.prepare(`
              INSERT INTO signals 
              (timestamp, signal_type, trading_style, price, stop_loss, 
               take_profit_1, take_profit_2, take_profit_3, confidence, reason)
              VALUES (datetime('now'), ?, ?, ?, ?, ?, ?, ?, ?, ?)
            `).bind(h.signal_type,h.trading_style,h.price,h.stop_loss,h.take_profit_1,h.take_profit_2,h.take_profit_3,h.confidence,h.reason).run();const F=await t.prepare(`
              SELECT setting_key, setting_value FROM user_settings
              WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
            `).all(),y={};for(const b of F.results||[])y[b.setting_key]=b.setting_value;y.telegram_bot_token&&y.telegram_chat_id&&await Se({botToken:y.telegram_bot_token,chatId:y.telegram_chat_id},Ne(h))}}}return e.json({success:!0,count:d})}catch(s){return e.json({success:!1,error:s.message},500)}});I.post("/api/market/fetch-mtf",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
      SELECT setting_value FROM user_settings 
      WHERE setting_key = 'twelve_data_api_key'
    `).first();let a=(s==null?void 0:s.setting_value)||"70140f57bea54c5e90768de696487d8f";if(!a||a==="your_key_here"||a==="")return e.json({success:!1,error:"Twelve Data API key not configured.",count:0});const n="XAU/USD",i=[{interval:"5min",dbKey:"5m",outputsize:100},{interval:"15min",dbKey:"15m",outputsize:100},{interval:"1h",dbKey:"1h",outputsize:100},{interval:"4h",dbKey:"4h",outputsize:100},{interval:"1day",dbKey:"daily",outputsize:100}];let o=0;const l={};for(const r of i){const c=`https://api.twelvedata.com/time_series?symbol=${n}&interval=${r.interval}&apikey=${a}&outputsize=${r.outputsize}`,p=await(await fetch(c)).json();if(p.code&&p.status==="error"){l[r.dbKey]={success:!1,error:p.message,count:0};continue}if(!p.values||!Array.isArray(p.values)){l[r.dbKey]={success:!1,error:"No data",count:0};continue}const f=p.values;let g=0;const w=[];for(const m of f){const _={timestamp:m.datetime,open:parseFloat(m.open),high:parseFloat(m.high),low:parseFloat(m.low),close:parseFloat(m.close),volume:0};w.push(_),await t.prepare(`
          INSERT INTO market_data (timestamp, open, high, low, close, volume, timeframe)
          VALUES (?, ?, ?, ?, ?, ?, ?)
          ON CONFLICT DO NOTHING
        `).bind(_.timestamp,_.open,_.high,_.low,_.close,_.volume,r.dbKey).run(),g++}if(w.length>=50){const m=Oe(w.reverse());m&&await t.prepare(`
    ...
    `).all();if(!s.results||s.results.length<50)return e.json({success:!1,error:"Not enough data. Please fetch market data first (need at least 50 candles)."});const a=s.results.reverse().map(r=>({timestamp:r.timestamp,open:r.open,high:r.high,low:r.low,close:r.close,volume:r.volume})),n=Oe(a);if(!n)return e.json({success:!1,error:"Failed to calculate indicators"});const i=a[a.length-1].close,o=K(i,n,"day_trade"),l=K(i,n,"swing_trade");return e.json({success:!0,signals:{day_trade:o,swing_trade:l}})}catch(s){return e.json({success:!1,error:s.message},500)}});I.post("/api/signals/generate-mtf",async e=>{const{DB:t}=e.env;try{const{analyzeTimeframeAlignment:s,validateMultiTimeframeSignal:a,formatAlignmentReport:n}=await Promise.resolve().then(()=>Kt),i=["5m","15m","1h","4h","daily"],o={};for(const R of i){const V=await t.prepare(`
      ...
      ORDER BY timestamp DESC 
      LIMIT 1
    `).first();if(!r)return e.json({success:!1,error:"No market data available"});const c=r.close,d=s(o,c),p=o["1h"],f=K(c,p,"day_trade"),g=K(c,p,"swing_trade"),w=a(f.signal_type,d),m=a(g.signal_type,d),_={...f,base_confidence:f.confidence,mtf_confidence:w.confidence,final_confidence:Math.min(95,w.confidence),isValid:w.isValid,mtf_reason:w.reason,alignment_score:d.score,alignment_type:d.type,reason:`${f.reason}, MTF: ${w.reason}`},h={...g,base_confidence:g.confidence,mtf_confidence:m.confidence,final_confidence:Math.min(95,m.confidence),isValid:m.isValid,mtf_reason:m.reason,alignment_score:d.score,alignment_type:d.type,reason:`${g.reason}, MTF: ${m.reason}`},F=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings
      WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
    `).all(),y={};for(const R of F.results||[])y[R.setting_key]=R.setting_value;let b=!1,A=[];y.telegram_bot_token&&y.telegram_chat_id&&(_.isValid&&_.signal_type!=="HOLD"&&await Se({botToken:y.telegram_bot_token,chatId:y.telegram_chat_id},`🎯 MULTI-TIMEFRAME CONFIRMED

${Ne({..._,timestamp:new Date().toISOString()})}

📊 ${n(d)}`)&&(A.push("day_trade"),b=!0),await new Promise(R=>setTimeout(R,1e3)),h.isValid&&h.signal_type!=="HOLD"&&await Se({botToken:y.telegram_bot_token,chatId:y.telegram_chat_id},`🎯 MULTI-TIMEFRAME CONFIRMED

${Ne({...h,timestamp:new Date().toISOString()})}

📊 ${n(d)}`)&&(A.push("swing_trade"),b=!0));for(const R of[_,h])R.isValid&&await t.prepare(`
          ...
          VALUES (datetime('now'), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).bind(R.signal_type,R.trading_style,R.price,R.stop_loss,R.take_profit_1,R.take_profit_2,R.take_profit_3,R.base_confidence,R.mtf_confidence,R.final_confidence,R.alignment_score,R.alignment_type,R.reason,b?1:0).run();return e.json({success:!0,signals:{day_trade:_,swing_trade:h},alignment:d,alignment_report:n(d),telegram_sent:b,sent_to_telegram:A,available_timeframes:Object.keys(o)})}catch(s){return e.json({success:!1,error:s.message,stack:s.stack},500)}});I.post("/api/signals/generate-now",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
      SELECT * FROM market_data 
      WHERE timeframe = '1h'
      ORDER BY timestamp DESC 
      LIMIT 200
    `).all();if(!s.results||s.results.length<50)return e.json({success:!1,error:"Not enough data. Please fetch market data first."});const a=s.results.reverse().map(f=>({timestamp:f.timestamp,open:f.open,high:f.high,low:f.low,close:f.close,volume:f.volume})),n=Oe(a);if(!n)return e.json({success:!1,error:"Failed to calculate indicators"});const i=a[a.length-1].close,o=K(i,n,"day_trade"),l=K(i,n,"swing_trade"),r=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings
      WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
    `).all(),c={};for(const f of r.results||[])c[f.setting_key]=f.setting_value;let d=!1,p=[];c.telegram_bot_token&&c.telegram_chat_id&&(await Se({botToken:c.telegram_bot_token,chatId:c.telegram_chat_id},Ne({...o,timestamp:new Date().toISOString()}))&&(p.push("day_trade"),d=!0),await new Promise(w=>setTimeout(w,1e3)),await Se({botToken:c.telegram_bot_token,chatId:c.telegram_chat_id},Ne({...l,timestamp:new Date().toISOString()}))&&(p.push("swing_trade"),d=!0));for(const f of[o,l])await t.prepare(`
        INSERT INTO signals 
        (timestamp, signal_type, trading_style, price, stop_loss, 
    ...
    `).bind(a).all(),{calculatePositionSize:l,formatPositionSize:r}=await Promise.resolve().then(()=>Ye),c=l(i,n,o.results);return e.json({success:!0,position:c,formatted:r(c)})}catch(s){return e.json({success:!1,error:s.message},500)}});I.post("/api/trading/execute",async e=>{const{DB:t}=e.env;try{const s=await e.req.json(),{account_id:a,signal_id:n,entry_price:i,stop_loss:o,take_profit_1:l,take_profit_2:r,take_profit_3:c,position_size:d,signal_type:p,trading_style:f,confidence:g}=s,w=await t.prepare(`
      SELECT * FROM trading_accounts WHERE id = ?
    `).bind(a).first();if(!w)return e.json({success:!1,error:"Account not found"},404);const m=new Date().toISOString().split("T")[0],_=await t.prepare(`
      SELECT profit_loss FROM trades 
      WHERE account_id = ? 
      AND date(entry_time) = ?
      AND status = 'CLOSED'
    `).bind(a,m).all(),{checkDailyLossLimit:h}=await Promise.resolve().then(()=>Ye),F=h(w,_.results);if(F.limit_exceeded)return e.json({success:!1,error:`Daily loss limit exceeded: ${F.current_loss_pct}% (max ${w.max_daily_loss_pct}%)`},400);const y=d*i,b=await t.prepare(`
      INSERT INTO trades (
    ...
    `).bind(a,n||null,p,f,i,d,y,o,l,r,c,g).run();return e.json({success:!0,trade_id:b.meta.last_row_id,message:"Trade executed successfully"})}catch(s){return e.json({success:!1,error:s.message},500)}});I.post("/api/trading/close/:trade_id",async e=>{const{DB:t}=e.env,s=e.req.param("trade_id");try{const a=await e.req.json(),{exit_price:n,exit_reason:i}=a,o=await t.prepare(`
    ...
    `).first(),n=(a==null?void 0:a.setting_value)||"70140f57bea54c5e90768de696487d8f",i=[{interval:"5min",dbKey:"5m"},{interval:"15min",dbKey:"15m"},{interval:"1h",dbKey:"1h"},{interval:"4h",dbKey:"4h"},{interval:"1day",dbKey:"daily"}];let o=0;for(const D of i){const Z=`https://api.twelvedata.com/time_series?symbol=XAU/USD&interval=${D.interval}&apikey=${n}&outputsize=100`,ee=await(await fetch(Z)).json();if(ee.values&&Array.isArray(ee.values)){const te=[];for(const E of ee.values){const k={timestamp:E.datetime,open:parseFloat(E.open),high:parseFloat(E.high),low:parseFloat(E.low),close:parseFloat(E.close),volume:0};te.push(k),await t.prepare(`
      ...
      ORDER BY timestamp DESC 
      LIMIT 1
    `).first(),f=(p==null?void 0:p.close)||0,g=l(d,f),w=d["1h"],m=K(f,w,"day_trade"),_=K(f,w,"swing_trade"),h=r(m.signal_type,g),F=r(_.signal_type,g),y={...m,final_confidence:Math.min(95,h.confidence),isValid:h.isValid,mtf_reason:h.reason,alignment_score:g.score,alignment_type:g.type},b={..._,final_confidence:Math.min(95,F.confidence),isValid:F.isValid,mtf_reason:F.reason,alignment_score:g.score,alignment_type:g.type};s.steps[1].status="completed",s.steps[1].data={dayTrade:y,swingTrade:b,alignment:g},s.steps.push({step:3,name:"Calculate Position Sizes",status:"running"});const A=await t.prepare(`
      SELECT * FROM trading_accounts WHERE id = 1
    `).first(),R=await t.prepare(`
      SELECT * FROM position_sizing_rules 
      WHERE account_id = 1 AND is_active = 1
      ORDER BY confidence_min DESC
    `).all(),{calculatePositionSize:V}=await Promise.resolve().then(()=>Ye),O=V(A,{entry_price:y.price,stop_loss:y.stop_loss,take_profit_1:y.take_profit_1,take_profit_2:y.take_profit_2,take_profit_3:y.take_profit_3,confidence:y.final_confidence,signal_type:y.signal_type,trading_style:y.trading_style},R.results),$=V(A,{entry_price:b.price,stop_loss:b.stop_loss,take_profit_1:b.take_profit_1,take_profit_2:b.take_profit_2,take_profit_3:b.take_profit_3,confidence:b.final_confidence,signal_type:b.signal_type,trading_style:b.trading_style},R.results);s.steps[2].status="completed",s.steps[2].data={dayPosition:O,swingPosition:$},s.steps.push({step:4,name:"Send Telegram Alert",status:"running"});const le=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings
      WHERE setting_key IN ('telegram_bot_token', 'telegram_chat_id')
...
🌐 Dashboard: ${e.req.url.replace("/api/automation/analyze-and-notify","")}
      `.trim();W=await Se({botToken:j.telegram_bot_token,chatId:j.telegram_chat_id},D)}if(s.steps[3].status=W?"completed":"failed",s.steps[3].data={telegramSent:W},y.isValid||b.isValid)for(const D of[y,b])D.isValid&&await t.prepare(`
            INSERT INTO mtf_signals 
            (timestamp, signal_type, trading_style, price, stop_loss, 
             take_profit_1, take_profit_2, take_profit_3, 
             base_confidence, mtf_confidence, final_confidence,
