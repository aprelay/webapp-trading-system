...
⏰ ${new Date().toLocaleString("en-US",{timeZone:"UTC"})} UTC
    ...
    ORDER BY timestamp DESC
    LIMIT 10
  `).all()).results.map(Q=>({rsi:parseFloat(String(Q.rsi))||50,macd:parseFloat(String(Q.macd))||0,macd_histogram:parseFloat(String(Q.macd_histogram))||0})).reverse(),U=nr(ie,a.slice(-10)),J=ir(U,q,u?"BULLISH":f?"BEARISH":"NEUTRAL");J&&U.strength>=70&&(o+=9,l++),i.push(lr(U,J));const Z=cr(t,s,n,r),qt=dr(Z,q);qt&&(Z.allAligned||Z.twoAligned)&&(o+=6,l++),i.push(ur(Z,qt));const Nt=await e.prepare(`
    SELECT setting_value FROM user_settings
    WHERE setting_key = 'twelve_data_api_key'
  `).first(),Mt=(Nt==null?void 0:Nt.setting_value)||"70140f57bea54c5e90768de696487d8f",Ks=await yr(e,Mt,15),ot=mr(Ks);pr(ot,q)&&ot.strength>=60?(o+=5,l++,i.push(`✅ Layer 18: ${ot.description}`)):ot.goldSignalSupport!=="NEUTRAL"?i.push(`⚠️ Layer 18: ${ot.description} (not aligned)`):i.push("ℹ️ Layer 18: DXY flat (neutral for Gold)");const Zs=await ss(e,Mt,"SILVER",15),Qs=await ss(e,Mt,"OIL",15),Ye=br(Zs,Qs,q);if(Ye.aligned&&Ye.alignmentCount>=1){const Q=Ye.alignmentCount===2?5:3;o+=Q,l++,i.push(`✅ Layer 19: ${Ye.description} (${Ye.strength}/100)`)}else i.push(`❌ Layer 19: ${Ye.description}`);const Js=await $r(e)||Nr(),Se=xr(Js);if(Lr(Se,q)&&Se.strength>=70){const Q=Se.positioning.includes("EXTREME")?7:4;o+=Q,l++,i.push(`✅ Layer 20: ${Se.description} (${Se.strength}/100)`)}else Se.goldSignalSupport!=="NEUTRAL"?i.push(`⚠️ Layer 20: ${Se.description} (not aligned)`):i.push(`ℹ️ Layer 20: ${Se.description}`);let bt="C";o>=162?bt="A+":o>=144?bt="A":o>=126&&(bt="B"),(u||f)&&l>=7&&(q=u?"BUY":"SELL");const xe=Math.max(ee*1.5,r*.003),en=q==="BUY"?r-xe:r+xe,tn=q==="BUY"?r+xe*2:r-xe*2,sn=q==="BUY"?r+xe*3:r-xe*3,nn=q==="BUY"?r+xe*4:r-xe*4;return{grade:bt,score:o,signal:q,confidence:o,layersPassed:l,layers:i,stopLoss:en,tp1:tn,tp2:sn,tp3:nn,liquidityScore:x,session:A,adx:d.adx,rsi:d.rsi,volumeRatio:w/y}}function Ns(e,t){const s=e.signal==="BUY"?"🟢":"🔴",n=e.grade==="A+"?"💎":e.grade==="A"?"⭐":"📊",a=new Date,r=`${a.getUTCHours().toString().padStart(2,"0")}:${a.getUTCMinutes().toString().padStart(2,"0")}`;let o=`🚨 <b>${e.grade}-GRADE 5M SETUP DETECTED!</b> 🚨
...
/help - Show this message`);return e.json({ok:!0})}catch(t){return console.error("[TELEGRAM WEBHOOK] Error:",t),e.json({ok:!0})}});const $t=new fe;$t.post("/market-analysis",async e=>await Gs(e));$t.get("/health",async e=>e.json({success:!0,status:"healthy",service:"ai-analysis",timestamp:new Date().toISOString()}));$t.get("/auto-ai-scan",async e=>await Gs(e));async function Gs(e){const{DB:t}=e.env;try{console.log("[AI-ANALYSIS] Starting comprehensive market analysis");const s=await t.prepare(`
      SELECT setting_key, setting_value FROM user_settings
      WHERE setting_key = 'twelve_data_api_key'
