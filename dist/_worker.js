...
⏰ ${new Date().toUTCString()}`;return await Z(p,l,r),!0}}return!1}catch(i){return console.error("[MONITORING] Failed to send alert:",i),!1}}He.get("/health-check",async e=>{const{DB:t}=e.env;try{const s=await ln(t),n=e.req.url.replace("/api/monitoring/health-check",""),a=new Date().toISOString(),o=await Go(t);console.log("[MONITORING] Starting comprehensive health check..."),console.log("[MONITORING] Tables exist:",o);const i=[{name:"auto-fetch",url:"/api/cron/auto-fetch"},{name:"ai-analysis",url:"/api/ai/health"},{name:"scanner",url:"/api/scanner/scan"},{name:"signals-recent",url:"/api/signals/recent"},{name:"indicators-latest",url:"/api/indicators/latest"}],l=[],r=s.telegram_alerts_enabled==="1",d=parseInt(s.slow_response_threshold_ms||"5000"),c=parseInt(s.max_failure_count||"3");console.log("[MONITORING] Fast mode: Checking 5 endpoints (MTF skipped)");for(const y of i){const _=await Vo(t,y.name,y.url,n);let b=0,E="unknown",v=_.status==="down"?1:0;if(o)try{const T=await t.prepare(`
        ...
        `).bind(m,u,f,g,l.reduce((y,_)=>y+_.response_time_ms,0)/l.length).run()}catch(y){console.log("[MONITORING] Could not save metrics:",y)}return console.log(`[MONITORING] Health check complete: ${h}`),console.log(`[MONITORING] Tables exist: ${o}, Alerts enabled: ${r}`),e.json({success:!0,timestamp:a,overall_status:h,summary:{endpoints:{healthy:m,degraded:u,down:f,total:l.length},data:{fresh:p.length-g,stale:g,total:p.length}},endpoints:l,data_freshness:p,config:{stale_threshold_minutes:s.data_stale_threshold_minutes,slow_response_threshold_ms:s.slow_response_threshold_ms,max_failure_count:s.max_failure_count,telegram_alerts_enabled:r}})}catch(s){return console.error("[MONITORING] Health check failed:",s),e.json({success:!1,error:s.message,timestamp:new Date().toISOString()},500)}});He.get("/status",async e=>{const{DB:t}=e.env;try{let s,n,a;try{s=await t.prepare(`
        SELECT 
          endpoint_name,
          status,
          response_time_ms,
