...
`,A+=`Next event: ${Nt(c.upcomingEvents[0])}
...
`;const he=candles1h.slice(-20),de=he.map(J=>J.high).sort((J,Te)=>Te-J),ne=he.map(J=>J.low).sort((J,Te)=>J-Te),Rt=de.slice(0,3),_t=ne.slice(0,3);if(A+=`📊 *Key Levels:*
`,A+=`🔴 *Resistance:* ${Rt.map(J=>`$${J.toFixed(2)}`).join(", ")}
...
`,K+="🤖 Powered by Hedge Fund Grade AI",console.log("[TELEGRAM] Message 2 length:",K.length,"characters");const Be=await q({botToken:C.telegram_bot_token,chatId:C.telegram_chat_id},K);fe=gt&&Be}}catch(H){console.error("[ENHANCED] Telegram error (optional):",H.message)}return e.json({success:!0,timestamp:new Date().toISOString(),current_price:l,telegram_sent:fe,day_trade:M,swing_trade:ue,alignment:{type:g.type,score:g.score,trends:g.trends},patterns:v.length>0?v.slice(0,3):null,regime:L?{trend:L.trend,volatility:L.volatility,should_trade:L.should_trade}:null,ml_prediction:I?{direction:I.overall_direction,predictions:I.predictions}:null,profit_probability:B?{tp1:B.tp1_probability,tp2:B.tp2_probability,tp3:B.tp3_probability,expected_value:B.expected_value}:null,liquidity:P?{score:P.liquidity_score,session:P.session,time_zone:P.time_of_day_zone,volume_trend:P.volume_trend,volume_percentile:P.volume_percentile,estimated_spread_pips:P.estimated_spread_pips,price_impact_bps:P.price_impact_bps,market_depth_score:P.market_depth_score,optimal_for_trading:P.optimal_for_trading,warnings:P.warnings,recommendation:P.recommendation}:null,risk_metrics:{var_95:V,var_99:Q,drawdown_pct:ee,portfolio_heat_pct:te}})}catch(n){return console.error("[ENHANCED] Error:",n.message,n.stack),e.json({success:!1,error:n.message,stack:n.stack},500)}});const Xs=new _e;Xs.post("/simple",async e=>{var s,n,a,o;const{DB:t}=e.env;try{console.log("[SIMPLE] Starting simple signal generation");const i=await t.prepare(`
    ...
    `).all()).results||[]).map(h=>({timestamp:h.timestamp,open:h.open,high:h.high,low:h.low,close:h.close,volume:h.volume||1})).reverse(),p=ze(d,u,m,"day_trade"),g=ze(d,u,m,"swing_trade");console.log("[SIMPLE] Generated signals with liquidity:",{day:{type:p.signal_type,confidence:p.confidence,liquidity_score:p.liquidity_score,session:p.session},swing:{type:g.signal_type,confidence:g.confidence,liquidity_score:g.liquidity_score,session:g.session}});let f=!1;try{const h=await t.prepare(`
      ...
      `).all(),y={};for(const w of h.results||[])y[w.setting_key]=w.setting_value;if(console.log("[SIMPLE] Telegram config:",{hasToken:!!y.telegram_bot_token,hasChat:!!y.telegram_chat_id,tokenLength:((o=y.telegram_bot_token)==null?void 0:o.length)||0,chatId:y.telegram_chat_id}),y.telegram_bot_token&&y.telegram_chat_id){const w=await t.prepare(`
          ...
          ORDER BY timestamp DESC 
          LIMIT 20
        `).all();let E=[],S=[];if(w.results&&w.results.length>=20){const j=w.results.map(O=>O.high).sort((O,D)=>D-O),I=w.results.map(O=>O.low).sort((O,D)=>O-D);E=j.slice(0,3),S=I.slice(0,3)}const b=p.signal_type==="BUY"?"🟢":p.signal_type==="SELL"?"🔴":"⚪",k=new Date().toLocaleString("en-US",{timeZone:"UTC"});let v=`${b} <b>GOLD/USD ${p.signal_type} SIGNAL</b> ${b}
...
`,h+=`Signal #${l} | ${f}`,h}ut.get("/scan",async e=>{var n;const{DB:t}=e.env,s={success:!1,message:"",data:{}};try{console.log("[MICRO] Starting 5-minute scan...");const a=await ri(t);if(!a.allowed)return console.log("[MICRO] Daily limit check failed:",a.reason),s.message=a.reason,s.data={signals_sent_today:a.signals_sent_today,max_signals:a.max_signals},e.json(s);console.log(`[MICRO] Daily limit check passed: ${a.signals_sent_today}/${a.max_signals} signals sent`);const o=await t.prepare(`
    ...
    `).all();if(!o.results||o.results.length<20)return s.message="Insufficient 5m candle data",e.json(s);const i=o.results.map(b=>({timestamp:b.timestamp,open:b.open,high:b.high,low:b.low,close:b.close,volume:b.volume||1})).reverse(),l=await t.prepare(`
      ...
      ORDER BY timestamp DESC 
      LIMIT 60
    `).all();if(!l.results||l.results.length<50)return s.message=`Insufficient 15m candle data (have ${((n=l.results)==null?void 0:n.length)||0}, need 50)`,e.json(s);const r=l.results.map(b=>({timestamp:b.timestamp,open:b.open,high:b.high,low:b.low,close:b.close,volume:b.volume||1})).reverse(),d=pe(i),c=pe(r);if(!d)return console.error("[MICRO] Failed to calculate 5m indicators - data length:",i.length),s.message="Failed to calculate 5m indicators",s.data={candles5m_count:i.length,first_candle:i[0],last_candle:i[i.length-1]},e.json(s);if(!c)return console.error("[MICRO] Failed to calculate 15m indicators - data length:",r.length),s.message="Failed to calculate 15m indicators",s.data={candles15m_count:r.length},e.json(s);const u=i[i.length-1].close,_=un(i,d,r,c);if(!_)return console.log("[MICRO] No valid setup detected"),s.success=!0,s.message="No signal - waiting for setup",s.data={currentPrice:u},e.json(s);if(console.log(`[MICRO] Signal detected: ${_.signal_type} ${_.setup_type} (${_.confidence}%)`),_.confidence<60)return console.log(`[MICRO] Confidence too low: ${_.confidence}%`),s.success=!0,s.message=`Signal below threshold (${_.confidence}% < 60%)`,s.data={signal:_},e.json(s);const m=Bt(i);if(m.liquidity_score<60)return console.log(`[MICRO] Liquidity too low: ${m.liquidity_score}`),s.success=!0,s.message=`Low liquidity (${m.liquidity_score} < 60)`,s.data={signal:_,liquidity:m},e.json(s);const p=li(_,m.liquidity_score,m.optimal_for_trading),g=new Date(Date.now()+900*1e3).toISOString(),f=new Date().toISOString().split("T")[0],h=await t.prepare(`
...
`,p+=`Signal #${s} | ${_}`,p}jt.get("/test-alert",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
    ...
    `).all(),y={};for(const E of h.results||[])y[E.setting_key]=E.setting_value;let w=!1;if(y.telegram_bot_token&&y.telegram_chat_id){const E=fn(f,m,g,85,"NEW_YORK");w=await q({botToken:y.telegram_bot_token,chatId:y.telegram_chat_id},E),w&&await t.prepare(`
          ...
          SET telegram_sent = 1
          WHERE id = (SELECT MAX(id) FROM micro_trade_signals)
        `).run()}return e.json({success:!0,message:`${m.grade}-grade signal generated and ${w?"sent":"saved"}`,signal:{...f,grade:m.grade,filters_passed:m.filtersPassed,position_multiplier:m.positionMultiplier},telegram_sent:w,execution_time_ms:Date.now()-s})}catch(r){return console.error("[HYBRID-MICRO] Error:",r),e.json({success:!1,error:r.message},500)}});jt.get("/test",async e=>{const{DB:t}=e.env;try{const s=await t.prepare(`
      SELECT timestamp, open, high, low, close, volume 
      FROM market_data 
      WHERE timeframe = '5m'
      ORDER BY timestamp DESC 
      LIMIT 50
    `).all();if(!s.results||s.results.length<20)return e.json({success:!1,error:"Insufficient data for testing"});const n=s.results.map(l=>({timestamp:l.timestamp,open:l.open,high:l.high,low:l.low,close:l.close,volume:l.volume||1})).reverse(),a=await pe(n),o=n[n.length-1].close,i=await gn("BUY",o,n,a,{"5m":{trend:"BULLISH",indicators:a}});return e.json({success:!0,test_results:{grade:i.grade,filters_passed:i.filtersPassed,confidence:i.confidence,position_multiplier:i.positionMultiplier,filter_details:i.filterResults},current_price:o,indicators:{rsi:a.rsi,macd:a.macd,adx:a.adx}})}catch(s){return e.json({success:!1,error:s.message},500)}});const G=new _e;G.use("/api/*",la());G.route("/api/signals/enhanced",zs);G.route("/api/signals/simple",Xs);G.route("/api/scanner",ct);G.route("/api/trades",we);G.route("/api/calendar",Fe);G.route("/api/backtest",pt);G.route("/api/telegram",dn);G.route("/api/ai",Pt);G.route("/api/monitoring",Ue);G.route("/api/micro",ut);G.route("/api/hybrid-micro",jt);G.get("/",e=>e.html(`
    <!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
...
⏰ ${new Date().toLocaleString()}`,j=await q({botToken:o,chatId:i},U);k.swing_trade_send_result=j,j&&(S=!0,b.push("Swing Trade"),R&&w.signal_type!=="HOLD"&&(console.log("[AUTO-FETCH] 🔥 HIGH CONVICTION swing signal! Sending reminder alerts..."),setTimeout(async()=>{const I=`${$} <b>⚠️ REMINDER: HIGH CONVICTION SWING</b> ${$}
...
🌐 Dashboard: ${e.req.url.replace("/api/automation/analyze-and-notify","")}
      `.trim();O=await q({botToken:I.telegram_bot_token,chatId:I.telegram_chat_id},N)}if(a.steps[3].status=O?"completed":"failed",a.steps[3].data={telegramSent:O},b.isValid||k.isValid)for(const D of[b,k])D.isValid&&await t.prepare(`
            INSERT INTO mtf_signals 
            (timestamp, signal_type, trading_style, price, stop_loss, 
             take_profit_1, take_profit_2, take_profit_3, 
             base_confidence, mtf_confidence, final_confidence,
...
🌐 Dashboard: ${e.req.url.replace("/api/automation/analyze-and-notify","")}
      `.trim(),{sendTelegramMessage:P}=await Promise.resolve().then(()=>Gs);j=await P({botToken:U.telegram_bot_token,chatId:U.telegram_chat_id},B)}if(a.steps[3].status=j?"completed":"failed",a.steps[3].data={telegramSent:j},E.isValid||S.isValid)for(const I of[E,S])I.isValid&&await t.prepare(`
            INSERT INTO mtf_signals 
            (timestamp, signal_type, trading_style, price, stop_loss, 
             take_profit_1, take_profit_2, take_profit_3, 
             base_confidence, mtf_confidence, final_confidence,
